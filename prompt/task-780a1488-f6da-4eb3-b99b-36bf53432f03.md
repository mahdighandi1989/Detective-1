---
task_id: 780a1488-f6da-4eb3-b99b-36bf53432f03
title: پیاده‌سازی ناسازگار کنترل دسترسی (RBAC) برای طبقه‌بندی محرمانگی در روت‌های API
type: security
priority: critical
...
created_at: '2026-06-13T18:04:48.168525+00:00'
updated_at: '2026-06-13T20:17:15.665026+00:00'
target_files:
- backend/app/api/routes/encyclopedia.py
- backend/app/api/routes/persons.py
---

# پیاده‌سازی ناسازگار کنترل دسترسی (RBAC) برای طبقه‌بندی محرمانگی در روت‌های API

## Raw Idea

روت‌های API `encyclopedia.py` و `persons.py` از توابع کمکی محلی `_can_access_classification` برای بررسی سطح دسترسی کاربر به محتوای طبقه‌بندی‌شده استفاده می‌کنند. این توابع محلی منطق دسترسی خود را دارند و از مکانیزم مرکزی `app.api.deps.can_access_classification` یا متد `current_user.has_clearance()` در مدل `User` استفاده نمی‌کنند. این ناسازگاری یک آسیب‌پذیری امنیتی جدی است که می‌تواند منجر به دور زدن کنترل دسترسی و افشای اطلاعات محرمانه به کاربران غیرمجاز شود.
...
- `backend/app/api/routes/encyclopedia.py:235-245` — `_can_access_classification` — تابع محلی کنترل دسترسی در encyclopedia.py
  ```python
  ...
  ```
- `backend/app/api/routes/persons.py:104-114` — `_can_access_classification` — تابع محلی کنترل دسترسی در persons.py
  ```python
...
FastAPI RBAC, SQLAlchemy User Model, Python Enums

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `backend/app/api/deps.py` (سطر 291) — این ماژول مکانیزم مرکزی can_access_classification را فراهم می‌کند.
- `backend/app/models/user.py` (سطر 212) — مدل User دارای متد has_clearance است که باید استفاده شود.
- `backend/app/models/article.py` — `encyclopedia.py` این فایل را import می‌کند
...
## 🌐 نقشهٔ وابستگی‌ها
این تغییر بر روی تمام روت‌های API که نیاز به بررسی سطح دسترسی دارند، تأثیر می‌گذارد و امنیت کلی سیستم را بهبود می‌بخشد.
...
- [ ] تمام توابع `_can_access_classification` محلی حذف شوند.
- [ ] روت‌های API از `app.api.deps` یا متدهای مدل `User` برای کنترل دسترسی استفاده کنند.
- [ ] تست‌های امنیتی برای اطمینان از عدم دسترسی کاربران با سطح محرمانگی پایین‌تر به محتوای طبقه‌بندی‌شده اضافه شود.
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. توابع کمکی محلی `_can_access_classification` در `encyclopedia.py` و `persons.py` حذف شوند. به جای آن، باید از مکانیزم‌های مرکزی و یکپارچهٔ RBAC که در `app.api.deps.py` و مدل `User` تعریف شده‌اند، استفاده شود. این امر شامل استفاده از `Depends(require_min_clearance(ClassificationLevel.SECRET))` یا فراخوانی `current_user.has_clearance()` است.

## 💡 نمونه‌های قبل/بعد
**قبل: استفاده از تابع محلی**

_قبل:_
```
if not _can_access_classification(current_user, person.classification):
    raise HTTPException(status.HTTP_403_FORBIDDEN, detail="سطح دسترسی کافی ندارید")
```

**بعد: استفاده از مکانیزم مرکزی**

_بعد:_
```
if not current_user.has_clearance(person.classification):
    raise HTTPException(status.HTTP_403_FORBIDDEN, detail="سطح دسترسی کافی ندارید")
# یا در دکوراتور روت:
# @router.get(..., dependencies=[Depends(require_min_clearance(ClassificationLevel.CONFIDENTIAL))])
```
...
## 🧪 دستورات اعتبارسنجی
- `pytest backend/tests/test_persons.py -k access_control`
- `pytest backend/tests/test_encyclopedia.py -k access_control`

## ⚠️ ریسک‌ها و موارد احتیاط
احتمال ایجاد خطاهای دسترسی (403 Forbidden) در صورت عدم پیاده‌سازی صحیح.
...
1. تمام توابع `_can_access_classification` محلی حذف شوند. _(verify: static)_
2. روت‌های API از `app.api.deps` یا متدهای مدل `User` برای کنترل دسترسی استفاده کنند. _(verify: static)_
3. تست‌های امنیتی برای اطمینان از عدم دسترسی کاربران با سطح محرمانگی پایین‌تر به محتوای طبقه‌بندی‌شده اضافه شود. _(verify: static)_

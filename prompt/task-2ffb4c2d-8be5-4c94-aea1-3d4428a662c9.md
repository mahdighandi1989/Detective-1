---
task_id: 2ffb4c2d-8be5-4c94-aea1-3d4428a662c9
title: افزودن تست‌های واحد برای منطق امتیازدهی اعتبار منبع (Source Credibility Scoring)
...
created_at: '2026-06-13T18:15:53.970799+00:00'
updated_at: '2026-06-13T20:20:26.460856+00:00'
target_files:
- backend/app/api/routes/sources.py
---

# افزودن تست‌های واحد برای منطق امتیازدهی اعتبار منبع (Source Credibility Scoring)

## Raw Idea

تابع `compute_credibility_score` در `app.api.routes.sources.py` منطق تجاری اصلی برای ارزیابی اعتبار منابع OSINT را پیاده‌سازی می‌کند. این امتیاز مستقیماً بر ارزیابی ریسک تأثیر می‌گذارد. به عنوان یک تابع خالص و قطعی، برای اطمینان از دقت، سازگاری و وزن‌دهی صحیح عوامل مختلف (نوع منبع، اعتبار دامنه، تعداد تأییدها، وجود نویسنده، منبع اولیه بودن) بسیار مناسب برای تست واحد است. پیاده‌سازی فعلی فاقد تست‌های اختصاصی است که می‌تواند منجر به محاسبات نادرست ریسک شود.
...
- `backend/app/api/routes/sources.py:53-135` — `compute_credibility_score` — توابع اصلی منطق امتیازدهی اعتبار منبع
  ```python
...
Python, توابع خالص، منطق تجاری

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `backend/app/api/routes/sources.py` (سطر 147) — از این منطق در `create_source` و `update_source` استفاده می‌کند
- `backend/app/services/risk_engine.py` — احتمالاً امتیازات اعتبار منبع را مصرف می‌کند
- `backend/app/models/source.py` (سطر 57) — انام `SourceType` را تعریف می‌کند
- `backend/app/api/deps.py` — `sources.py` این فایل را import می‌کند
- `backend/app/schemas/source.py` — `sources.py` این فایل را import می‌کند

## 🌐 نقشهٔ وابستگی‌ها
این منطق برای کیفیت داده‌های OSINT مرکزی است و مستقیماً بر قابلیت اطمینان ارزیابی‌های ریسک تأثیر می‌گذارد.
...
- [ ] `_domain_reputation_factor` امتیازات مورد انتظار را برای URLهای مختلف (مثلاً .gov، .com، blogspot) برگرداند.
- [ ] `_source_type_factor` وزن‌های صحیح را برای تمام انام‌های `SourceType` برگرداند.
- [ ] `compute_credibility_score` امتیازات دقیق (0.0-1.0) را برای طیف وسیعی از ترکیبات ورودی، از جمله موارد خاص (مثلاً تأیید صفر، بدون نویسنده، همه عوامل حداکثر/حداقل) برگرداند.
- [ ] امتیاز به درستی بین 0.0 و 1.0 محدود شود.
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. یک فایل تست جدید به نام `backend/tests/test_sources_logic.py` ایجاد کنید و تست‌های واحدی برای توابع `_domain_reputation_factor`، `_source_type_factor` و `compute_credibility_score` اضافه کنید.

## 📤 خروجی مورد انتظار
تغییر کد در فایل‌های مرتبط، commit یا PR جدید با پیام واضح، و عبور تمام معیارهای پذیرش.

## 🧪 دستورات اعتبارسنجی
- `pytest backend/tests/test_sources_logic.py`

## ⚠️ ریسک‌ها و موارد احتیاط
ریسک پایین، زیرا تست واحد ایزوله است.
...
1. `_domain_reputation_factor` امتیازات مورد انتظار را برای URLهای مختلف (مثلاً .gov، .com، blogspot) برگرداند. _(verify: static)_
2. `_source_type_factor` وزن‌های صحیح را برای تمام انام‌های `SourceType` برگرداند. _(verify: static)_
3. `compute_credibility_score` امتیازات دقیق (0.0-1.0) را برای طیف وسیعی از ترکیبات ورودی، از جمله موارد خاص (مثلاً تأیید صفر، بدون نویسنده، همه عوامل حداکثر/حداقل) برگرداند. _(verify: static)_
4. امتیاز به درستی بین 0.0 و 1.0 محدود شود. _(verify: static)_

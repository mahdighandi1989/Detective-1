---
task_id: 31629600-6250-4178-bb16-923a8d21ed94
title: '[منطق] Unaccounted Risk Assessment Change History Management'
type: logic_audit
priority: high
...
created_at: '2026-06-13T18:12:20.838341+00:00'
updated_at: '2026-06-13T20:19:40.236750+00:00'
---

# [منطق] Unaccounted Risk Assessment Change History Management
...
The `backend/app/models/risk_assessment.py` model explicitly includes 'تاریخچهٔ تغییرات' (change history) as a responsibility, indicating that historical records of assessments are crucial. However, the `backend/app/services/risk_engine.py` module, which generates the `RiskAssessmentResult`, does not mention producing or managing this history. There is no other component described that explicitly handles the creation, update, or tracking of this change history for `RiskAssessment` instances.

## 💥 پیامد (impact)
Without a clear mechanism for managing change history, the `RiskAssessment` model's `change_history` field will either remain empty, be manually updated inconsistently, or require complex logic embedded directly within the model or a yet-to-be-defined service. This compromises data integrity, audita

## 🛠 پیشنهاد رفع اولیه
Define a dedicated service or integrate explicit logic within the persistence layer (e.g., a `RiskAssessment` CRUD service or an event-driven mechanism) that is responsible for creating and updating the `change_history` field whenever a `RiskAssessment` is created or modified, potentially by comparing new results from the `risk_engine` with existing records.
...
### Step 1: Implement initial change history creation for new RiskAssessment instances
**Status:** `pending` (0%)
**Scope:** این مرحله بر تعریف و پیاده‌سازی منطق مورد نیاز برای مقداردهی اولیه فیلد 'change_history' تمرکز دارد، زمانی که یک نمونه جدید از 'RiskAssessment' برای اولین بار ایجاد و پایدار می‌شود. این کار تضمین می‌کند که اولین وضعیت ارزیابی ثبت شده و مبنایی برای تغییرات آینده ایجاد می‌شود. این مرحله شامل به‌روزرسانی‌های بعدی تاریخچه نمی‌شود، بلکه فقط رکورد اولیه را پوشش می‌دهد.
**Excerpt:**
```
Define a dedicated service or integrate explicit logic within the persistence layer (e.g., a `RiskAssessment` CRUD service or an event-driven mechanism) that is responsible for creating and updating the `change_history` field whenever a `RiskAssessment` is created or modified...
```

### Step 2: Implement change history update logic for modified RiskAssessment instances
**Status:** `pending` (0%)
**Scope:** این مرحله شامل توسعه منطق برای مقایسه یک 'RiskAssessmentResult' جدید (یا وضعیت به‌روز شده یک نمونه 'RiskAssessment') با وضعیت قبلی آن است. در صورت شناسایی تغییرات قابل توجه، یک ورودی جدید که این اصلاحات را منعکس می‌کند، به فیلد 'change_history' اضافه خواهد شد. این منطق در مکانی مناسب در لایه پایداری، احتمالاً به عنوان یک تابع کمکی یا متد مرتبط با مدل 'RiskAssessment' قرار خواهد گرفت.
**Excerpt:**
```
...responsible for creating and updating the `change_history` field whenever a `RiskAssessment` is created or modified, potentially by comparing new results from the `risk_engine` with existing records.
```

### Step 3: Integrate change history management into RiskAssessment persistence flow
**Status:** `pending` (0%)
**Scope:** این مرحله ادغام حیاتی تضمین می‌کند که منطق ایجاد و به‌روزرسانی 'change_history' (که در مراحل ۱ و ۲ توسعه یافته است) به طور خودکار هر زمان که یک نمونه 'RiskAssessment' در برنامه ایجاد یا اصلاح می‌شود، فراخوانی شود. این شامل شناسایی نقاط دقیق در لایه پایداری است که اشیاء 'RiskAssessment' در آن ذخیره یا به‌روزرسانی می‌شوند و درج فراخوانی‌ها به توابع مدیریت تاریخچه است. 'RiskAssessmentResult' تولید شده توسط 'backend/app/services/risk_engine.py' یک ورودی کلیدی برای فعال کردن به‌روزرسانی‌ها خواهد بود.
**Excerpt:**
```
Define a dedicated service or integrate explicit logic within the persistence layer (e.g., a `RiskAssessment` CRUD service or an event-driven mechanism) that is responsible for creating and updating the `change_history` field whenever a `RiskAssessment` is created or modified, potentially by comparing new results from the `risk_engine` with existing records.
```

### Step 4: Add unit and integration tests for change history functionality
**Status:** `pending` (0%)
**Scope:** این مرحله شامل نوشتن تست‌های واحد جامع برای منطق ایجاد و به‌روزرسانی 'change_history' و همچنین تست‌های یکپارچه‌سازی برای تأیید اینکه مدیریت تاریخچه به درستی در جریان کلی پایداری 'RiskAssessment' فعال می‌شود. تست‌ها باید سناریوهای مختلفی از جمله ایجاد اولیه، اصلاحات متعدد و موارد خاص برای تشخیص تغییر را پوشش دهند. این کار استحکام و صحت ردیابی تاریخچه پیاده‌سازی شده را تضمین می‌کند.
**Excerpt:**
```
Without a clear mechanism for managing change history, the `RiskAssessment` model's `change_history` field will either remain empty, be manually updated inconsistently, or require complex logic embedded directly within the model or a yet-to-be-defined service. This compromises data integrity, audita
```

---
task_id: a6045990-bfcb-40da-ba41-b3edcf5f3c0b
title: Add missing integration tests for 'risk_assessment' and 'sources' API routes
type: bug
priority: medium
...
updated_at: '2026-06-07T19:52:33.190555+00:00'
archived: true
archived_at: '2026-06-07T19:52:33.190538+00:00'
target_files:
- backend/app/api/routes/risk_assessment.py
- backend/app/api/routes/sources.py
---

# Add missing integration tests for 'risk_assessment' and 'sources' API routes

## Raw Idea

The `backend/tests` directory contains tests for several API routes (auth, encyclopedia, graph, persons, risk), but `backend/app/api/routes/risk_assessment.py` and `backend/app/api/routes/sources.py` are not explicitly covered. These routes likely expose critical functionality related to managing risk assessments and data sources. Without dedicated integration tests, changes to these endpoints or their underlying logic could introduce regressions that go unnoticed, potentially impacting data integrity or user experience.
...
- `backend/app/api/routes/risk_assessment.py:1-500` — `router` — Identify all API endpoints within this file that require integration testing.
  ```python
  router = APIRouter()
  
  @router.post('/')
  async def create_risk_assessment(...
      # ⚠️ This endpoint and others in this file need integration tests
      ...
  ```
- `backend/app/api/routes/sources.py:1-500` — `router` — Identify all API endpoints within this file that require integration testing.
  ```python
  router = APIRouter()
...
- `backend/tests/__init__.py` — New test files will be added to the backend test suite.
- `backend/tests/test_auth.py` — Provides an example structure for existing route tests to follow.
- `backend/app/models/risk_assessment.py` — The data model associated with the risk assessment routes.
- `backend/app/models/source.py` — The data model associated with the sources routes.

## 🌐 نقشهٔ وابستگی‌ها
These routes are part of the public API and are consumed by the frontend. Ensuring their correctness through tests is vital for application stability and data integrity.
...
- [ ] New test files `backend/tests/test_risk_assessment.py` and `backend/tests/test_sources.py` are created.
- [ ] Each new test file contains integration tests covering at least the primary GET, POST, PUT, and DELETE operations for their respective resources.
- [ ] Tests verify correct HTTP status codes (e.g., 200, 201, 400, 404, 403), response data structure, and error handling.
...
1. Create new test files, `backend/tests/test_risk_assessment.py` and `backend/tests/test_sources.py`. Implement integration tests to cover the main CRUD (Create, Read, Update, Delete) operations and any specific business logic exposed by these API endpoints. Ensure tests verify correct HTTP status codes, response data, and error handling.
...
- `pytest backend/tests/test_sources.py`

## ⚠️ ریسک‌ها و موارد احتیاط
Requires setting up appropriate test data and potentially mocking dependencies; might uncover existing bugs in the API logic.
...
### Step 1: تحلیل Endpointها و عملکرد `risk_assessment.py`
**Status:** `pending` (0%)
**Scope:** این مرحله شامل بررسی دقیق فایل `backend/app/api/routes/risk_assessment.py` برای شناسایی تمام endpointهای API (مانند GET، POST، PUT، DELETE) است که برای مدیریت ارزیابی‌های ریسک ارائه می‌دهد. هدف، درک کامل شمای درخواست/پاسخ مورد انتظار، پارامترها، و هرگونه منطق تجاری حیاتی است که این endpointها کپسوله می‌کنند. این تحلیل برای طراحی تست‌های یکپارچه‌سازی مؤثر و اطمینان از پوشش جامع ضروری است. این مرحله شامل نوشتن هیچ کدی یا تغییر فایل موجود نیست.
**Excerpt:**
```
The `backend/tests` directory contains tests for several API routes (auth, encyclopedia, graph, persons, risk), but `backend/app/api/routes/risk_assessment.py` and `backend/app/api/routes/sources.py` are not explicitly covered. These routes likely expose critical functionality related to managing risk assessments and data sources.
```

### Step 2: ایجاد فایل تست یکپارچه‌سازی برای `risk_assessment.py`
**Status:** `pending` (0%)
**Scope:** این مرحله به ایجاد یک فایل پایتون جدید اختصاص دارد که به طور خاص برای تست‌های یکپارچه‌سازی مربوط به `backend/app/api/routes/risk_assessment.py` طراحی شده است. این شامل ایجاد یک فایل جدید، احتمالاً با نام `backend/tests/integration/test_risk_assessment.py` (یا نامی مشابه بر اساس قراردادهای موجود در `backend/tests`) و راه‌اندازی کلاس تست پایه و هرگونه fixture لازم (مانند کلاینت تست، تنظیمات پایگاه داده) برای تست یکپارچه‌سازی است. فایل باید به صورت منطقی در دایرکتوری `backend/tests` قرار گیرد.
— [merged] این مرحله به ایجاد یک فایل پایتون جدید اختصاص دارد که به طور خاص برای تست‌های یکپارچه‌سازی مربوط به `backend/app/api/routes/sources.py` طراحی شده است. این شامل ایجاد یک فایل جدید، احتمالاً با نام `backend/tests/integration/test_sources.py` (یا نامی مشابه بر اساس قراردادهای موجود در `backend/tests`) و راه‌اندازی کلاس تست پایه و هرگونه fixture لازم (مانند کلاینت تست، تنظیمات پایگاه داده) برای تست یکپارچه‌سازی است. فایل باید به صورت منطقی در دایرکتوری `backend/tests` قرار گیرد.
**Excerpt:**
```
The `backend/tests` directory contains tests for several API routes (auth, encyclopedia, graph, persons, risk), but `backend/app/api/routes/risk_assessment.py` and `backend/app/api/routes/sources.py` are not explicitly covered.
```

### Step 3: پیاده‌سازی تست‌های یکپارچه‌سازی برای Endpointهای `risk_assessment.py`
**Status:** `pending` (0%)
**Scope:** این مرحله شامل پر کردن فایل تست تازه ایجاد شده با توابع تست واقعی است. هر تابع تست باید درخواست‌های کلاینت را به endpointهای API `risk_assessment` شبیه‌سازی کند، پاسخ‌های صحیح (کدهای وضعیت، ساختار داده، محتوا) را تأیید کند و هرگونه عوارض جانبی (مانند تغییرات پایگاه داده) را بررسی کند. هم سناریوهای موفق و هم موارد خاص (مانند ورودی نامعتبر، دسترسی غیرمجاز) باید پوشش داده شوند تا از جامعیت تست اطمینان حاصل شود.
— [merged] این مرحله شامل پر کردن فایل تست تازه ایجاد شده با توابع تست واقعی است. هر تابع تست باید درخواست‌های کلاینت را به endpointهای API `sources` شبیه‌سازی کند، پاسخ‌های صحیح (کدهای وضعیت، ساختار داده، محتوا) را تأیید کند و هرگونه عوارض جانبی (مانند تغییرات پایگاه داده) را بررسی کند. هم سناریوهای موفق و هم موارد خاص (مانند ورودی نامعتبر، دسترسی غیرمجاز) باید پوشش داده شوند تا از جامعیت تست اطمینان حاصل شود.
**Excerpt:**
```
Without dedicated integration tests, changes to these endpoints or their underlying logic could introduce regressions that go unnoticed, potentially impacting data integrity or user experience.
```

### Step 4: تحلیل Endpointها و عملکرد `sources.py`
**Status:** `pending` (0%)
**Scope:** این مرحله شامل بررسی دقیق فایل `backend/app/api/routes/sources.py` برای شناسایی تمام endpointهای API (مانند GET، POST، PUT، DELETE) است که برای مدیریت منابع داده ارائه می‌دهد. هدف، درک کامل شمای درخواست/پاسخ مورد انتظار، پارامترها، و هرگونه منطق تجاری حیاتی است که این endpointها کپسوله می‌کنند. این تحلیل برای طراحی تست‌های یکپارچه‌سازی مؤثر و اطمینان از پوشش جامع ضروری است. این مرحله شامل نوشتن هیچ کدی یا تغییر فایل موجود نیست.
**Excerpt:**
```
The `backend/tests` directory contains tests for several API routes (auth, encyclopedia, graph, persons, risk), but `backend/app/api/routes/risk_assessment.py` and `backend/app/api/routes/sources.py` are not explicitly covered. These routes likely expose critical functionality related to managing risk assessments and data sources.
```

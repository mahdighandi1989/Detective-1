---
task_id: 4be366ac-3643-409e-9eab-32fba256181d
title: پروژه فاقد فایل LICENSE است که ابهام قانونی ایجاد می‌کند.
type: security
priority: high
...
created_at: '2026-06-06T17:38:35.740252+00:00'
updated_at: '2026-06-07T19:53:30.927952+00:00'
archived: true
archived_at: '2026-06-07T19:53:30.927939+00:00'
target_files:
- README.md
---

# پروژه فاقد فایل LICENSE است که ابهام قانونی ایجاد می‌کند.

## Raw Idea

ریپازیتوری حاوی فایل `LICENSE` در ریشه خود نیست. این موضوع باعث می‌شود که نحوه استفاده، تغییر یا توزیع نرم‌افزار برای دیگران نامشخص باشد، که می‌تواند مانع مشارکت و پذیرش تجاری شود. همچنین، بررسی‌های خودکار سازگاری مجوز با وابستگی‌ها را غیرممکن می‌سازد.
...
- `README.md:1-1` — دایرکتوری ریشه فاقد فایل LICENSE است.
...
- `README.md` — باید به مجوز انتخاب شده اشاره کند.
- `backend/pyproject.toml` — وابستگی‌ها ممکن است دارای مجوزهایی باشند که نیاز به بررسی سازگاری دارند.
- `frontend/package.json` — وابستگی‌ها ممکن است دارای مجوزهایی باشند که نیاز به بررسی سازگاری دارند.

## 🌐 نقشهٔ وابستگی‌ها
مستقیماً بر وابستگی‌ها تأثیر نمی‌گذارد، اما جنبه‌های قانونی استفاده از همه وابستگی‌ها را تحت تأثیر قرار می‌دهد.
...
- [ ] فایل `LICENSE` در ریشه ریپازیتوری وجود دارد.
- [ ] فایل `README.md` برای اشاره به مجوز انتخاب شده به‌روزرسانی شده است.
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. یک فایل `LICENSE` در دایرکتوری ریشه ریپازیتوری ایجاد کنید. یک مجوز متن‌باز مناسب (مانند MIT، Apache 2.0، GPLv3) را انتخاب کنید که با اهداف پروژه و مجوزهای وابستگی‌های شخص ثالث سازگار باشد. همچنین فایل `README.md` را برای اشاره به مجوز به‌روزرسانی کنید.

## 💡 نمونه‌های قبل/بعد
**نمونه ایجاد فایل LICENSE**
...
_بعد:_
```
یک فایل `LICENSE` (مثلاً `LICENSE.md` یا `LICENSE`) با متن مجوز انتخاب شده به دایرکتوری ریشه اضافه می‌شود.
```
...
1. فایل `LICENSE` در ریشه ریپازیتوری وجود دارد. _(verify: static)_
2. فایل `README.md` برای اشاره به مجوز انتخاب شده به‌روزرسانی شده است. _(verify: static)_

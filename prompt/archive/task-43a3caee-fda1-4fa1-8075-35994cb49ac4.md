---
task_id: 43a3caee-fda1-4fa1-8075-35994cb49ac4
title: بررسی و به‌روزرسانی وابستگی‌های پروژه برای آسیب‌پذیری‌های امنیتی
type: security
priority: critical
...
created_at: '2026-06-06T17:37:04.883435+00:00'
updated_at: '2026-06-07T19:55:45.318872+00:00'
archived: true
archived_at: '2026-06-07T19:55:45.318858+00:00'
target_files:
...
# بررسی و به‌روزرسانی وابستگی‌های پروژه برای آسیب‌پذیری‌های امنیتی

## Raw Idea

پروژه به کتابخانه‌های شخص ثالث متعددی وابسته است که در فایل‌های `pyproject.toml`، `requirements.txt` و `package.json` فهرست شده‌اند. وابستگی‌های قدیمی اغلب حاوی آسیب‌پذیری‌های امنیتی شناخته‌شده‌ای هستند که می‌توانند مورد سوءاستفاده قرار گیرند و منجر به نقض داده‌ها، حملات DoS یا به خطر افتادن کامل سیستم شوند. اسکن و به‌روزرسانی منظم وابستگی‌ها یک اقدام امنیتی اساسی است.
---
[scan #2 at 2026-06-06T17:38:35.747077+00:00]
پروژه از وابستگی‌های متعددی (پایتون از طریق `pyproject.toml`/`requirements.txt`، Node.js از طریق `package.json`) استفاده می‌کند. بدون اسکن خودکار برای آسیب‌پذیری‌های شناخته شده (CVE) در این وابستگی‌ها، برنامه در معرض خطر استفاده از کتابخانه‌های ناامن است. این می‌تواند منجر به آسیب‌پذیری‌های حیاتی ما
...
ادامه (دور 1): بررسی و به‌روزرسانی وابستگی‌های پروژه برای آسیب‌پذیری‌های امنیتی
...
📋 وضعیت چک‌لیست مراحل (0/1 انجام‌شده) — پیشرفت کلی: **99%**:
  - [~] **مرحله 1: اسکن و به‌روزرسانی وابستگی‌های شخص ثالث برای رفع آسیب‌پذیری‌های امنیتی** — باقی‌مانده: شناسایی، اسکن و به‌روزرسانی تمام وابستگی‌های شخص ثالث در `pyproject.toml`، `requirements.txt` و `package.json` برای رفع آسیب‌پذیری‌ها انجام نشده است.
...
✅ بخش‌هایی که در دور قبل انجام شد:
  - تمام وابستگی‌های بک‌اند به آخرین نسخه‌های امن به‌روزرسانی شده‌اند.
  - تمام وابستگی‌های فرانت‌اند به آخرین نسخه‌های امن به‌روزرسانی شده‌اند.
  - یک ابزار اسکن وابستگی (مانند `pip-audit`، `npm audit`، Snyk) در CI/CD یا به صورت منظم اجرا می‌شود.
  - هیچ آسیب‌پذیری با شدت بحرانی یا بالا توسط اسکنرها گزارش نمی‌شود.
...
```json
{
  ...
  "done_parts": [],
  "remaining_parts": [
    "وابستگی‌های بک‌اند (pyproject.toml, requirements.txt) به آخرین نسخه‌های امن به‌روزرسانی نشده‌اند.",
    "وابستگی‌های فرانت‌اند (package.json) به آخرین نسخه‌های امن به‌روزرسانی نشده‌اند.",
    "ابزار اسکن وابستگی (مانند pip-audit, npm audit, Snyk) در CI/CD یا به صورت منظم پیاده‌سازی و اجرا نشده است.",
    "آسیب‌پذیری‌های با شدت بحرانی یا بالا همچنان در تسک‌های اخیر گزارش شده یا به آن‌ها اشاره شده است."
  ],
  "evide

🪜 اقدامات بعدی پیشنهادی verifier:
  - وابستگی‌های بک‌اند را در `backend/pyproject.toml` و `backend/requirements.txt` به آخرین نسخه‌های امن به‌روزرسانی کنید.
  - وابستگی‌های فرانت‌اند را در `frontend/package.json` به آخرین نسخه‌های امن به‌روزرسانی کنید.
  - یک ابزار اسکن وابستگی مناسب (مانند `pip-audit` برای پایتون و `npm audit` برای نود) را در CI/CD پیکربندی و اجرا کنید.
  - آسیب‌پذیری‌های امنیتی با شدت بحرانی و بالا را که در تسک‌ها ذکر شده‌اند، برطرف کنید.
...
## 🧪 دستورات اعتبارسنجی
- `cd backend && pip install -r requirements.txt && pip-audit`
...
- [~] **مرحله 1: اسکن و به‌روزرسانی وابستگی‌های شخص ثالث برای رفع آسیب‌پذیری‌های امنیتی** — این مرحله شامل شناسایی و اسکن تمام وابستگی‌های شخص ثالث پروژه است که در فایل‌های `pyproject.toml`، `requirements.txt` و `package.json` فهرست شده‌اند. هدف اصلی، یافتن آسیب‌پذیری‌های امنیتی شناخته‌شده در این وابستگی‌ها و سپس به‌روزرسانی آن‌ها به آخرین نسخه‌های پایدار و امن است. این به‌روزرسانی باید با
      _باقی‌مانده: شناسایی، اسکن و به‌روزرسانی تمام وابستگی‌های شخص ثالث در `pyproject.toml`، `requirements.txt` و `package.json` برای رفع آسیب‌پذیری‌ها انجام نشده است._

## Acceptance Criteria

1. [مرحله 1 — اسکن و به‌روزرسانی وابستگی‌های شخص ثالث برای رفع آسیب‌پذیری‌های امنیتی] باقی‌مانده: شناسایی، اسکن و به‌روزرسانی تمام وابستگی‌های شخص ثالث در `pyproject.toml`، `requirements.txt` و `package.json` برای رفع آسیب‌پذیری‌ها انجام نشده است. _(verify: manual_only)_
2. وابستگی‌های بک‌اند را در `backend/pyproject.toml` و `backend/requirements.txt` به آخرین نسخه‌های امن به‌روزرسانی کنید. _(verify: static)_
3. وابستگی‌های فرانت‌اند را در `frontend/package.json` به آخرین نسخه‌های امن به‌روزرسانی کنید. _(verify: static)_
4. یک ابزار اسکن وابستگی مناسب (مانند `pip-audit` برای پایتون و `npm audit` برای نود) را در CI/CD پیکربندی و اجرا کنید. _(verify: backend_test)_
5. آسیب‌پذیری‌های امنیتی با شدت بحرانی و بالا را که در تسک‌ها ذکر شده‌اند، برطرف کنید. _(verify: backend_test)_
6. هیچ تستی fail نمی‌شود (`npm run test` / `pytest`) _(verify: backend_test)_
7. linter بدون warning عبور می‌کند _(verify: backend_test)_
8. type-check موفق است (`tsc --noEmit` / `mypy`) _(verify: backend_test)_

## Task Steps

### Step 1: اسکن و به‌روزرسانی وابستگی‌های شخص ثالث برای رفع آسیب‌پذیری‌های امنیتی
**Status:** `partial` (99%)
**Scope:** این مرحله شامل شناسایی و اسکن تمام وابستگی‌های شخص ثالث پروژه است که در فایل‌های `pyproject.toml`، `requirements.txt` و `package.json` فهرست شده‌اند. هدف اصلی، یافتن آسیب‌پذیری‌های امنیتی شناخته‌شده در این وابستگی‌ها و سپس به‌روزرسانی آن‌ها به آخرین نسخه‌های پایدار و امن است. این به‌روزرسانی باید با دقت انجام شود تا از عدم سازگاری با کدهای موجود جلوگیری شود و پس از آن تست‌های لازم برای اطمینان از عملکرد صحیح سیستم اجرا شوند. این مرحله شامل پیاده‌سازی ابزارهای اسکن جدید یا تغییر ساختار کلی مدیریت وابستگی‌ها نیست، بلکه صرفاً به‌روزرسانی نسخه‌های موجود است.
**Excerpt:**
```
پروژه به کتابخانه‌های شخص ثالث متعددی وابسته است که در فایل‌های `pyproject.toml`، `requirements.txt` و `package.json` فهرست شده‌اند. وابستگی‌های قدیمی اغلب حاوی آسیب‌پذیری‌های امنیتی شناخته‌شده‌ای هستند که می‌توانند مورد سوءاستفاده قرار گیرند و منجر به نقض داده‌ها، حملات DoS یا به خطر افتادن کامل سیستم شوند. اسکن و به‌روزرسانی منظم وابستگی‌ها یک اقدام امنیتی اساسی است.
```

---
task_id: ac26503f-38e1-4519-8c05-34f87b03d824
title: کوکی‌های احراز هویت ممکن است فاقد پرچم‌های `Secure` و `HttpOnly` باشند.
type: security
priority: high
...
created_at: '2026-06-06T17:38:35.747230+00:00'
updated_at: '2026-06-07T19:53:20.676118+00:00'
archived: true
archived_at: '2026-06-07T19:53:20.676101+00:00'
target_files:
- backend/app/api/routes/auth.py
---

# کوکی‌های احراز هویت ممکن است فاقد پرچم‌های `Secure` و `HttpOnly` باشند.

## Raw Idea

اگر بک‌اند از کوکی‌ها برای مدیریت نشست یا احراز هویت استفاده می‌کند (که حتی با سیستم‌های مبتنی بر توکن برای refresh tokenها یا CSRF tokenها رایج است)، این کوکی‌ها باید با پرچم‌های `Secure` و `HttpOnly` تنظیم شوند. `Secure` تضمین می‌کند که کوکی‌ها فقط از طریق HTTPS ارسال می‌شوند و از استراق سمع جلوگیری می‌کند. `HttpOnly` از دسترسی اسکریپت‌های سمت کلاینت (جاوااسکریپت) به کوکی جلوگیری می‌کند و حملات XSS را کاهش می‌دهد. بدون این پرچم‌ها، آسیب‌پذیری‌های ربودن نشست و XSS به طور قابل توجهی افزایش می‌یابد.
...
- `backend/app/api/routes/auth.py:1-100` — `login` — بررسی کنید که کوکی‌های احراز هویت در کجا تنظیم می‌شوند.
  ```python
  response.set_cookie(key='session_id', value=token, httponly=False) # ⚠️ Missing HttpOnly and Secure
  ```
...
FastAPI `Response.set_cookie`, کوکی‌های HTTP، HTTPS.

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `backend/app/core/security.py` — ممکن است حاوی منطق تولید کوکی باشد.
- `frontend/src/lib/api.ts` — کلاینت فرانت‌اند ممکن است کوکی‌ها را مدیریت کند.

## 🌐 نقشهٔ وابستگی‌ها
بر امنیت نشست کاربر تأثیر می‌گذارد.
...
- [ ] همه کوکی‌های مربوط به احراز هویت و نشست دارای پرچم‌های `HttpOnly` و `Secure` هستند (هنگام اجرا بر روی HTTPS).
- [ ] جاوااسکریپت سمت کلاینت نمی‌تواند به این کوکی‌ها دسترسی پیدا کند.
- [ ] کوکی‌ها از طریق HTTP ساده ارسال نمی‌شوند.
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. مکانیزم تنظیم کوکی را در `backend/app/api/routes/auth.py` و `backend/app/core/security.py` بررسی کنید. اطمینان حاصل کنید که هر هدر `Set-Cookie` تولید شده توسط برنامه شامل `Secure=True` (فقط برای تولید/HTTPS) و `HttpOnly=True` باشد.

## 💡 نمونه‌های قبل/بعد
**تنظیم کوکی‌های secure و httponly**
...
_بعد:_
```
response.set_cookie(key='token', value=jwt_token, httponly=True, secure=True, samesite='Lax')
```
...
## 🧪 دستورات اعتبارسنجی
- `در ابزارهای توسعه‌دهنده مرورگر، پس از ورود به سیستم، درخواست‌های شبکه را برای بررسی پرچم‌های کوکی بررسی کنید.`

## ⚠️ ریسک‌ها و موارد احتیاط
جزئی، اما نیاز به آزمایش دقیق دارد تا اطمینان حاصل شود که عملکرد مبتنی بر کوکی موجود مختل نمی‌شود.
...
1. همه کوکی‌های مربوط به احراز هویت و نشست دارای پرچم‌های `HttpOnly` و `Secure` هستند (هنگام اجرا بر روی HTTPS). _(verify: backend_test)_
2. جاوااسکریپت سمت کلاینت نمی‌تواند به این کوکی‌ها دسترسی پیدا کند. _(verify: ui_interaction)_
3. کوکی‌ها از طریق HTTP ساده ارسال نمی‌شوند. _(verify: static)_

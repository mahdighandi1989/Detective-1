---
task_id: 17742f88-335b-47d9-96bc-45c7abcc7acc
title: Identify and refactor duplicated logic in frontend page components
type: refactor
priority: low
...
created_at: '2026-06-06T17:41:57.682577+00:00'
updated_at: '2026-06-07T19:50:43.872584+00:00'
archived: true
archived_at: '2026-06-07T19:50:43.872570+00:00'
target_files:
...
# Identify and refactor duplicated logic in frontend page components

## Raw Idea

The `frontend/src/app/` directory contains multiple `page.tsx` files (e.g., `encyclopedia/page.tsx`, `graph/page.tsx`, `persons/page.tsx`). In Next.js applications, it's common for these page components to share layout structures, data fetching patterns, loading states, or error handling logic. Without content, there's a risk of duplicating this common logic across multiple pages, leading to increased maintenance effort, potential inconsistencies, and larger bundle sizes. For example, common headers, footers, or authentication checks might be repeated.
...
- `frontend/src/app/encyclopedia/page.tsx:1-300` — `EncyclopediaPage` — Identify and extract common patterns into reusable components or hooks.
  ```tsx
  export default function EncyclopediaPage() {
    // ⚠️ Look for duplicated layout, data fetching, or state management logic here
    return <main>...</main>
  }
  ```
- `frontend/src/app/graph/page.tsx:1-300` — `GraphPage` — Identify and extract common patterns into reusable components or hooks.
  ```tsx
  ...
  ```
- `frontend/src/app/layout.tsx:1-200` — `RootLayout` — Ensure common UI elements and layout structures are centralized here.
  ```tsx
  export default function RootLayout({ children }: { children: React.ReactNode }) {
    // ⚠️ This is the ideal place for shared layout components like Header/Footer
...
- `frontend/src/components/Header.tsx` — A common component that should be integrated into a shared layout.
- `frontend/src/lib/api.ts` — Shared data fetching logic should be abstracted and reused.
- `frontend/src/lib/utils.ts` — General utility functions should be centralized and reused.

## 🌐 نقشهٔ وابستگی‌ها
Refactoring shared frontend logic will make the UI more consistent, easier to maintain, and potentially improve performance by reducing bundle size and improving caching.
...
- [ ] Common UI elements (e.g., header, navigation) are managed by a single shared layout component (e.g., `frontend/src/app/layout.tsx`).
- [ ] Repeated data fetching patterns are abstracted into custom hooks or utility functions.
- [ ] Reduced code duplication across `page.tsx` files, leading to cleaner and more concise page components.
- [ ] The application's UI remains consistent across all pages.
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. Review the `page.tsx` files for common patterns in layout, data fetching, state management, or UI elements. Extract shared logic into reusable components (e.g., `frontend/src/components/Layout.tsx`), custom hooks (e.g., for data fetching), or utilize Next.js's built-in layout features (e.g., `frontend/src/app/layout.tsx`) for shared UI across routes. Abstract common API calls into `frontend/src/lib/api.ts` or related files.

## 💡 نمونه‌های قبل/بعد
**Example: Centralizing layout**
...
## 🧪 دستورات اعتبارسنجی
- `Manual code review of `page.tsx` files and shared components.`
- `Run `npm run dev` and navigate through the application to visually confirm consistency.`

## ⚠️ ریسک‌ها و موارد احتیاط
Minor visual regressions if not carefully implemented; requires a good understanding of Next.js layout features.
...
1. Common UI elements (e.g., header, navigation) are managed by a single shared layout component (e.g., `frontend/src/app/layout.tsx`). _(verify: static)_
2. Repeated data fetching patterns are abstracted into custom hooks or utility functions. _(verify: static)_
3. Reduced code duplication across `page.tsx` files, leading to cleaner and more concise page components. _(verify: manual_only)_
4. The application's UI remains consistent across all pages. _(verify: ui_interaction)_

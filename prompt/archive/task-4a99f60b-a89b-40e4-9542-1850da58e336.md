...
title: افزودن تست‌های تأیید RBAC برای نقاط پایانی API
type: security
priority: critical
...
updated_at: '2026-06-07T19:55:14.545114+00:00'
archived: true
archived_at: '2026-06-07T19:55:14.545092+00:00'
target_files:
...
# افزودن تست‌های تأیید RBAC برای نقاط پایانی API
...
ادامه (دور 1): اطمینان از اعمال سازگار و قوی RBAC در تمام نقاط پایانی API

## 📍 موقعیت دقیق در پروژه
_(file:line — symbol — snippet)_

- `backend/app/core/rbac.py:1-30` — `RoleChecker`
- `backend/app/api/routes/persons.py:10-40` — `create_person`
- `backend/app/core/security.py` — از evidence verifier در دور 0
- `backend/app/core/enums.py` — از evidence verifier در دور 0
...
[auto-re-registered from github_import at 2026-06-06T18:03:11.644690+00:00]
...
✅ بخش‌هایی که در دور قبل انجام شد:
  - نقاط پایانی API در `persons.py` با وابستگی‌های RBAC (مانند `require_roles` و `_can_access_classification`) محافظت شده‌اند.
  - منطق RBAC برای بررسی نقش و سطح محرمانگی در `backend/app/core/rbac.py` پیاده‌سازی شده است.
  - در صورت عدم دسترسی کافی به منابع محافظت شده، پاسخ 403 Forbidden برگردانده می‌شود.
  - کاربران با مجوزهای صحیح می‌توانند به منابع محافظت شده دسترسی پیدا کنند.

⏳ بخش‌هایی که هنوز باقی مانده (تمرکز روی این‌ها):
  - تست‌های جدیدی برای تأیید RBAC برای نقاط پایانی کلیدی اضافه نشده‌اند یا قابل اجرا نیستند.
...
```json
{
  ...
  "remaining_parts": [
    "تست‌های جدیدی برای تأی

🪜 اقدامات بعدی پیشنهادی verifier:
  - فایل `tests/api/test_rbac.py` را ایجاد یا تکمیل کنید.
  - تست‌های واحد و یکپارچه‌سازی برای تأیید عملکرد RBAC در نقاط پایانی کلیدی بنویسید.
  - اطمینان حاصل کنید که تست‌های RBAC به درستی اجرا می‌شوند و نتایج مورد انتظار را برمی‌گردانند.
...
1. تست‌های جدیدی برای تأیید RBAC برای نقاط پایانی کلیدی اضافه نشده‌اند یا قابل اجرا نیستند. _(verify: static)_
2. هیچ تستی fail نمی‌شود (`npm run test` / `pytest`) _(verify: static)_
3. linter بدون warning عبور می‌کند _(verify: static)_
4. type-check موفق است (`tsc --noEmit` / `mypy`) _(verify: static)_

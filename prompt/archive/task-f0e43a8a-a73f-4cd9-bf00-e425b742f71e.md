---
task_id: f0e43a8a-a73f-4cd9-bf00-e425b742f71e
title: Refactor potentially large backend route and service files to improve modularity
type: refactor
priority: medium
...
created_at: '2026-06-06T17:39:44.192576+00:00'
updated_at: '2026-06-07T19:52:24.138203+00:00'
archived: true
archived_at: '2026-06-07T19:52:24.138188+00:00'
target_files:
...
# Refactor potentially large backend route and service files to improve modularity

## Raw Idea

Based on the project structure, several files in `backend/app/api/routes/` and `backend/app/services/` are prone to growing excessively large. For instance, `backend/app/api/routes/risk.py` or `backend/app/api/routes/encyclopedia.py` might embed complex business logic directly in route handlers, while `backend/app/services/risk_engine.py` or `backend/app/services/osint_agent.py` could accumulate extensive logic within single functions. This leads to reduced readability, maintainability, and increased cognitive load, making code harder to understand, test, and refactor.
...
- `backend/app/api/routes/risk.py:1-500` — `router` — Review route handlers for excessive logic; extract to services.
  ```python
  @router.post('/calculate')
  async def calculate_risk_endpoint(...
      # ⚠️ Check this endpoint for embedded complex business logic that should be in a service
      ...
  ```
- `backend/app/services/risk_engine.py:1-500` — `RiskEngineService` — Review the entire file for functions exceeding 80 lines and the file itself for exceeding 500 lines.
  ```python
  class RiskEngineService:
      def evaluate_comprehensive_risk(data: dict) -> dict:
          # ⚠️ Check this method for exceeding 80 lines or the file for exceeding 500 lines
          ...
  ```
- `backend/app/services/osint_agent.py:1-500` — `OSINTAgentService` — Review the entire file for functions exceeding 80 lines and the file itself for exceeding 500 lines.
  ```python
  class OSINTAgentService:
      def execute_multi_stage_osint_query(query: str) -> list:
          # ⚠️ Check this method for exceeding 80 lines or the file for exceeding 500 lines
          ...
  ```
...
- `backend/app/services/__init__.py` — New specialized services or methods might be added here.
- `backend/app/core/config.py` — Configuration might be related to service logic.

## 🌐 نقشهٔ وابستگی‌ها
Refactoring these files will improve the modularity and maintainability of the backend, benefiting all routes and tasks that depend on them. It will also make the code easier to test.
...
- [ ] No function in the targeted route or service files exceeds 80 lines (excluding docstrings and comments).
- [ ] No targeted file exceeds 500 lines.
- [ ] Business logic is clearly separated from API request/response handling.
- [ ] Code readability and maintainability are visibly improved.
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. Conduct a review of the identified files. For route files, extract complex business logic into dedicated service methods. For service files, refactor large functions (e.g., exceeding 80 lines) into smaller, more focused private helper methods or delegate parts of the logic to new, specialized service methods. Extract common utility functions into `backend/app/core/utils.py` if not already present.

## 💡 نمونه‌های قبل/بعد
**Example: Extracting logic from route to service**
...
async def create_item(item: Item):
    # Complex validation and DB logic here
    if item.price < 0: raise HTTPException(400, 'Invalid price')
    db.add(item)
    db.commit()
    return item
```
...
@router.post('/item')
async def create_item(item: Item, item_service: ItemService = Depends(get_item_service)):
    # Delegate to service
...
- `Run static analysis tools (e.g., `flake8 --max-line-length=80 --max-complexity=10 backend/app/api/routes/risk.py`)`

## ⚠️ ریسک‌ها و موارد احتیاط
Can introduce subtle bugs if refactoring is not done carefully and without adequate test coverage. Requires a good understanding of the existing logic.
...
1. No function in the targeted route or service files exceeds 80 lines (excluding docstrings and comments). _(verify: static)_
2. No targeted file exceeds 500 lines. _(verify: static)_
3. Business logic is clearly separated from API request/response handling. _(verify: manual_only)_
4. Code readability and maintainability are visibly improved. _(verify: manual_only)_

## Task Steps

### Step 1: شناسایی و مستندسازی پتانسیل افزایش حجم کد در ماژول‌های اصلی
**Status:** `pending` (0%)
**Scope:** این مرحله شامل تحلیل و درک دقیق مشکل مطرح شده توسط کاربر در مورد پتانسیل افزایش حجم کد در فایل‌های خاص بک‌اند است. تمرکز بر روی شناسایی رسمی این نگرانی معماری و پیامدهای آن بر خوانایی، قابلیت نگهداری و بار شناختی است. این مرحله شامل هیچ‌گونه بازسازی کد، تغییرات کد، یا طراحی معماری جزئی نیست. هدف اصلی، ایجاد یک درک روشن از مشکل شناسایی شده قبل از هرگونه اقدام عملی است.
**Excerpt:**
```
Based on the project structure, several files in `backend/app/api/routes/` and `backend/app/services/` are prone to growing excessively large. For instance, `backend/app/api/routes/risk.py` or `backend/app/api/routes/encyclopedia.py` might embed complex business logic directly in route handlers, while `backend/app/services/risk_engine.py` or `backend/app/services/osint_agent.py` could accumulate extensive logic within single functions. This leads to reduced readability, maintainability, and increased cognitive load, making code harder to understand, test, and refactor.
```

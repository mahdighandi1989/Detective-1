---
task_id: 6cd5783c-aa81-4cae-a278-b7b6bc93ab4a
title: تقویت اعتبارسنجی ورودی و اطمینان از رمزگذاری صحیح خروجی برای جلوگیری از XSS
type: security
priority: high
...
created_at: '2026-06-06T17:37:20.034896+00:00'
updated_at: '2026-06-07T19:54:24.374794+00:00'
archived: true
archived_at: '2026-06-07T19:54:24.374755+00:00'
target_files:
...
# تقویت اعتبارسنجی ورودی و اطمینان از رمزگذاری صحیح خروجی برای جلوگیری از XSS

## Raw Idea

در حالی که شمای Pydantic (`backend/app/schemas/`) اعتبارسنجی بک‌اند را فراهم می‌کند، فرانت‌اند (`frontend/src/app/`, `frontend/src/components/`) نیز باید تمام محتوای ارائه‌شده توسط کاربر را قبل از رندر کردن در مرورگر، به شدت پاکسازی و escape کند. بدون رمزگذاری صحیح خروجی، اسکریپت‌های مخرب تزریق‌شده به محتوای تولیدشده توسط کاربر (مانند متن مقاله، توضیحات افراد، جزئیات منبع) می‌توانند منجر به حملات Cross-Site Scripting (XSS) شوند و به مهاجمان اجازه دهند تا نشست‌های کاربر را سرقت کنند یا وب‌سایت را تخریب کنند. این امر به ویژه برای محتوایی که ممکن است در نماهای `encyclopedia` یا `graph` نمایش داده شود، حیاتی است.
...
- `backend/app/schemas/article.py` — `ArticleCreate` — شماها قوانین اعتبارسنجی ورودی را تعریف می‌کنند. محتوای واقعی در این فاز ارائه نشده است.
- `frontend/src/app/encyclopedia/page.tsx` — `EncyclopediaPage` — این صفحه احتمالاً محتوای تولیدشده توسط کاربر (مقالات) را نمایش می‌دهد. محتوای واقعی در این فاز ارائه نشده است.
- `frontend/src/app/graph/page.tsx` — `GraphPage` — این صفحه ممکن است برچسب‌ها یا توضیحات را از گره‌ها/لبه‌های گراف نمایش دهد. محتوای واقعی در این فاز ارائه نشده است.
...
FastAPI, Pydantic, Next.js, React

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `backend/app/models/article.py` — مدل‌های پایگاه داده که محتوای تولیدشده توسط کاربر را ذخیره می‌کنند.
- `backend/app/api/routes/encyclopedia.py` — API برای بازیابی/ذخیره مقالات.
- `frontend/src/lib/utils.ts` — ممکن است حاوی توابع کمکی برای پاکسازی باشد.

## 🌐 نقشهٔ وابستگی‌ها
بر یکپارچگی داده‌ها و امنیت رابط کاربری تأثیر می‌گذارد.
...
- [ ] تمام متن ارائه‌شده توسط کاربر که در فرانت‌اند نمایش داده می‌شود، به درستی escape یا پاکسازی شده است.
- [ ] تلاش برای تزریق `<script>alert('XSS')</script>` در فیلدهای متنی، اسکریپت را در مرورگر اجرا نمی‌کند.
- [ ] اعتبارسنجی بک‌اند ورودی‌هایی را که به وضوح مخرب هستند یا از محدودیت‌های طول معقول فراتر می‌روند، رد می‌کند.
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. 1. **بک‌اند**: شمای Pydantic و نقاط پایانی API را بررسی کنید تا از اعتبارسنجی جامع برای تمام ورودی‌های رشته‌ای، به ویژه آن‌هایی که ذخیره و بعداً نمایش داده می‌شوند، اطمینان حاصل کنید. در صورت مجاز بودن HTML/Markdown خام، استفاده از کتابخانه‌های پاکسازی را در نظر بگیرید. 2. **فرانت‌اند**: رمزگذاری/پاکسازی دقیق خروجی را برای تمام محتوای تولیدشده توسط کاربر قبل از رندر کردن آن در کامپوننت‌های React پیاده‌سازی کنید. از محافظت‌های داخلی React (به عنوان مثال، اجتناب از `dangerouslySetInnerHTML`) استفاده کنید و در صورت پشتیبانی از متن غنی، یک کتابخانه پاکسازی اختصاصی را در نظر بگیرید.
...
_بعد:_
```
<SanitizedHtml content={userContent} />
```

**اعتبارسنجی ورودی بک‌اند**

_قبل:_
```
class ArticleCreate(BaseModel): title: str; content: str
```

_بعد:_
```
class ArticleCreate(BaseModel): title: str; content: str = Field(..., max_length=10000); @validator('content') def validate_content(cls, v): return bleach.clean(v)
```
...
## 🧪 دستورات اعتبارسنجی
- `Manually test by submitting content like `<script>alert('XSS')</script>` through frontend forms.`
- `pytest backend/tests/test_encyclopedia.py`

## ⚠️ ریسک‌ها و موارد احتیاط
پاکسازی بیش از حد ممکن است قالب‌بندی قانونی را حذف کند؛ نیاز به تعادل دقیق دارد.
...
1. تمام متن ارائه‌شده توسط کاربر که در فرانت‌اند نمایش داده می‌شود، به درستی escape یا پاکسازی شده است. _(verify: static)_
2. تلاش برای تزریق `<script>alert('XSS')</script>` در فیلدهای متنی، اسکریپت را در مرورگر اجرا نمی‌کند. _(verify: static)_
3. اعتبارسنجی بک‌اند ورودی‌هایی را که به وضوح مخرب هستند یا از محدودیت‌های طول معقول فراتر می‌روند، رد می‌کند. _(verify: static)_

## Task Steps

### Step 1: پیاده‌سازی پاکسازی و Escape کردن محتوای فرانت‌اند برای جلوگیری از XSS
**Status:** `pending` (0%)
**Scope:** این مرحله شامل شناسایی تمام محتوای ارائه‌شده توسط کاربر است که در فرانت‌اند، به ویژه در مسیرهای `frontend/src/app/` و `frontend/src/components/`، رندر می‌شود. هدف اصلی اعمال تکنیک‌های مناسب پاکسازی (sanitization) و Escape کردن (escaping) قبل از رندر است تا از آسیب‌پذیری‌های Cross-Site Scripting (XSS) جلوگیری شود. این کار با خنثی‌سازی اسکریپت‌های مخرب تعبیه‌شده در محتوایی مانند متن مقاله، توضیحات افراد و جزئیات منبع انجام می‌شود. اطمینان از پردازش امن محتوا در نماهای حیاتی مانند `encyclopedia` و `graph` از اهمیت ویژه‌ای برخوردار است. این مرحله شامل اعتبارسنجی بک‌اند نمی‌شود، زیرا آن بخش قبلاً توسط شمای Pydantic مدیریت شده است.
**Excerpt:**
```
در حالی که شمای Pydantic (`backend/app/schemas/`) اعتبارسنجی بک‌اند را فراهم می‌کند، فرانت‌اند (`frontend/src/app/`, `frontend/src/components/`) نیز باید تمام محتوای ارائه‌شده توسط کاربر را قبل از رندر کردن در مرورگر، به شدت پاکسازی و escape کند. بدون رمزگذاری صحیح خروجی، اسکریپت‌های مخرب تزریق‌شده به محتوای تولیدشده توسط کاربر (مانند متن مقاله، توضیحات افراد، جزئیات منبع) می‌توانند منجر به حملات Cross-Site Scripting (XSS) شوند و به مهاجمان اجازه دهند تا نشست‌های کاربر را سرقت کنند یا وب‌سایت را تخریب کنند. این امر به ویژه برای محتوایی که ممکن است در نماهای `encyclopedia` یا `graph` نمایش داده شود، حیاتی است.
```

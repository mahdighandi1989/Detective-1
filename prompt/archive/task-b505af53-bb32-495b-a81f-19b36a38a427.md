---
task_id: b505af53-bb32-495b-a81f-19b36a38a427
title: استانداردسازی کلاینت API فرانت‌اند برای سازگاری و مدیریت خطای قوی
type: refactor
priority: medium
...
created_at: '2026-06-06T17:39:01.342622+00:00'
updated_at: '2026-06-07T19:52:43.336905+00:00'
archived: true
archived_at: '2026-06-07T19:52:43.336891+00:00'
target_files:
- frontend/src/lib/api.ts
- frontend/src/app/layout.tsx
---

# استانداردسازی کلاینت API فرانت‌اند برای سازگاری و مدیریت خطای قوی

## Raw Idea

فایل `frontend/src/lib/api.ts` به عنوان نقطه مرکزی برای ارتباط فرانت‌اند-بک‌اند عمل می‌کند. بسیار مهم است که این کلاینت یک رابط سازگار برای برقراری تماس‌های API فراهم کند، توکن‌های احراز هویت را مدیریت کند (مثلاً پیوست کردن JWT به درخواست‌ها، رفرش کردن توکن‌ها) و پاسخ‌های خطای بک‌اند را به طور یکنواخت پردازش کند. تماس‌های API ناسازگار یا نادیده گرفتن مدیریت خطا در فرانت‌اند می‌تواند منجر به تجربه کاربری پراکنده، وضعیت‌های UI مدیریت نشده و اشکال‌زدایی دشوار شود.
...
- `frontend/src/lib/api.ts:1-50` — `apiClient` — این کلاینت اصلی API است و باید تمام تعاملات بک‌اند را مدیریت کند.
  ```tsx
  import { API_BASE_URL } from '@/config'; // فرض بر وجود فایل config
  ...
  ```
- `frontend/src/app/layout.tsx:10-30` — `RootLayout` — زمینه سراسری برای احراز هویت ممکن است اینجا تنظیم شود و بر مدیریت توکن تأثیر بگذارد.
  ```tsx
  import { AuthProvider } from '@/context/AuthContext'; // فرض بر وجود AuthContext
...
Next.js, TypeScript, Fetch API (یا Axios).

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `frontend/src/app/encyclopedia/page.tsx` — صفحه نمونه‌ای که از کلاینت API استفاده می‌کند.
- `frontend/src/app/graph/page.tsx` — صفحه نمونه‌ای که از کلاینت API استفاده می‌کند.
- `frontend/src/components/Header.tsx` — ممکن است وضعیت ورود/خروج را مدیریت کند و با کلاینت API تعامل داشته باشد.

## 🌐 نقشهٔ وابستگی‌ها
بر تمام کامپوننت‌های فرانت‌اند که با بک‌اند تعامل دارند، تأثیر می‌گذارد و بر تجربه کاربری و پایداری برنامه اثر می‌گذارد.
...
- [ ] تمام فراخوانی‌های API از صفحات فرانت‌اند از کلاینت `api.ts` استفاده می‌کنند.
- [ ] کلاینت `api.ts` به طور خودکار توکن‌های احراز هویت را در درخواست‌ها قرار می‌دهد.
- [ ] پاسخ‌های خطا از بک‌اند به طور سازگار توسط `api.ts` تجزیه و تحلیل شده و به عنوان استثنا پرتاب می‌شوند.
- [ ] کامپوننت‌های UI فرانت‌اند به طور مناسب خطاها را که توسط کلاینت `api.ts` برگردانده می‌شوند، مدیریت می‌کنند.
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. 1. فایل `frontend/src/lib/api.ts` را بررسی کنید تا اطمینان حاصل شود که تمام الگوهای رایج تعامل API (GET, POST, PUT, DELETE) را کپسوله می‌کند. 2. مدیریت خطای قوی را در کلاینت پیاده‌سازی کنید، پاسخ‌های خطای بک‌اند را (همانطور که در یافته 3 تعریف شده است) تجزیه و تحلیل کنید و راهی سازگار برای واکنش کامپوننت‌های UI به خطاها فراهم کنید. 3. اطمینان حاصل کنید که توکن‌های احراز هویت (مثلاً JWT) به طور خودکار به درخواست‌های خروجی پیوست می‌شوند و به طور ایمن مدیریت می‌شوند (مثلاً در `localStorage` یا کوکی‌های `httpOnly` ذخیره می‌شوند). 4. در صورت لزوم، مکانیزم‌های تلاش مجدد یا منطق رفرش توکن را در نظر بگیرید.

## 💡 نمونه‌های قبل/بعد
**مثال یک فراخوانی API قوی با مدیریت خطا**
...
const fetchData = async () => {
    ...
    // No explicit error check, assumes success
    setData(data);
};
```
...
const fetchData = async () => {
    try {
        const data = await apiClient.get<DataType>('/data', userToken);
        setData(data);
    } catch (error) {
...
## 🧪 دستورات اعتبارسنجی
- `Manually test various frontend pages, observing network requests and error states.`
- `Trigger backend errors (e.g., 401, 403, 500) and verify frontend behavior.`
- `Check browser console for unhandled promise rejections.`

## ⚠️ ریسک‌ها و موارد احتیاط
شکستن فراخوانی‌های API موجود، معرفی اشکالات جدید احراز هویت در صورت عدم پیاده‌سازی دقیق.
...
1. تمام فراخوانی‌های API از صفحات فرانت‌اند از کلاینت `api.ts` استفاده می‌کنند. _(verify: static)_
2. کلاینت `api.ts` به طور خودکار توکن‌های احراز هویت را در درخواست‌ها قرار می‌دهد. _(verify: ui_interaction)_
3. پاسخ‌های خطا از بک‌اند به طور سازگار توسط `api.ts` تجزیه و تحلیل شده و به عنوان استثنا پرتاب می‌شوند. _(verify: ui_interaction)_
4. کامپوننت‌های UI فرانت‌اند به طور مناسب خطاها را که توسط کلاینت `api.ts` برگردانده می‌شوند، مدیریت می‌کنند. _(verify: ui_interaction)_

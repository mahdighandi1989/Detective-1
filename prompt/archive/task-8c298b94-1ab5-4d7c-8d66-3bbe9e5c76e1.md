---
task_id: 8c298b94-1ab5-4d7c-8d66-3bbe9e5c76e1
title: اطمینان از همگام‌سازی مدل‌های SQLAlchemy با مهاجرت‌های Alembic
type: bug
priority: high
...
created_at: '2026-06-06T17:37:20.041988+00:00'
updated_at: '2026-06-07T19:54:13.580870+00:00'
archived: true
archived_at: '2026-06-07T19:54:13.580854+00:00'
target_files:
- backend/alembic/env.py
- backend/app/models/__init__.py
---

# اطمینان از همگام‌سازی مدل‌های SQLAlchemy با مهاجرت‌های Alembic

## Raw Idea

پروژه از SQLAlchemy برای ORM و Alembic برای مهاجرت‌های پایگاه داده استفاده می‌کند. بسیار مهم است که وضعیت فعلی مدل‌های SQLAlchemy (در `backend/app/models/`) همیشه در آخرین اسکریپت‌های مهاجرت Alembic منعکس شود. ناهماهنگی‌ها می‌تواند منجر به خطاهای زمان اجرا، مشکلات یکپارچگی داده‌ها یا رفتار غیرمنتظره هنگام تعامل برنامه با پایگاه داده شود. این یک منبع رایج اشکال در پروژه‌های در حال توسعه است.
---
[scan #2 at 2026-06-06T17:37:41.175693+00:00]
پروژه به نظر می‌رسد از هر دو پایگاه داده رابطه‌ای (مدل‌های SQLAlchemy، Alembic) و پایگاه داده گرافی (با توجه به `neo4j_sync.py`, `graph.py`, `relationship.py`) استفاده می‌کند. حفظ یکپارچگی داده‌ها بین این دو سیستم ذخیره‌سازی متمایز حیاتی است. اگر یک `Person` در پایگاه داده SQL به‌روز شود، اما گره مر
...
- `backend/alembic/env.py:20-40` — `run_migrations_online` — این فایل نحوه تعامل Alembic با مدل‌های SQLAlchemy را هماهنگ می‌کند. `target_metadata` باید همه مدل‌ها را شامل شود.
  ```python
  def run_migrations_online():
      connectable = engine_from_config(
          config.get_section_option(section),
          prefix="sqlalchemy.",
      ...
      with connectable.connect() as connection:
          ...
          with context.begin_of_migration():
              context.run_migrations()
  ```
- `backend/app/models/__init__.py:1-15` — این فایل تضمین می‌کند که همه مدل‌ها برای Alembic قابل کشف هستند.
  ```python
  ...
  # اطمینان حاصل کنید که همه مدل‌ها اینجا import شده‌اند تا توسط Alembic کشف شوند.
  ```
...
Python, FastAPI, SQLAlchemy, Alembic, PostgreSQL (فرض شده)

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `backend/app/models/user.py` — یک فایل مدل نمونه که باید در مهاجرت‌ها منعکس شود.
- `backend/app/models/person.py` — یک فایل مدل نمونه که باید در مهاجرت‌ها منعکس شود.
- `backend/alembic/versions/0001_initial.py` — اسکریپت مهاجرت اولیه که باید با مدل‌های فعلی همگام باشد.

## 🌐 نقشهٔ وابستگی‌ها
بر تمام عملیات پایگاه داده و پایداری برنامه تأثیر می‌گذارد.
...
- [ ] اجرای `alembic revision --autogenerate -m "Check for pending changes"` روی کدبیس فعلی یک اسکریپت مهاجرت خالی (یا فقط تغییرات مورد انتظار) تولید می‌کند.
- [ ] یک تغییر جدید در مدل (مثلاً اضافه کردن یک ستون) به درستی یک اسکریپت مهاجرت تولید می‌کند که با موفقیت اعمال می‌شود.
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. یک فرآیند (دستی یا خودکار) برای بررسی منظم ناهماهنگی‌های مدل-مهاجرت پیاده‌سازی کنید. این کار معمولاً شامل اجرای `alembic revision --autogenerate -m "Check for pending changes"` و تأیید اینکه هیچ مهاجرت جدیدی تولید نمی‌شود مگر اینکه قصد شده باشد، است. در صورت یافتن تغییرات، یک مهاجرت جدید باید ایجاد و اعمال شود.

## 📤 خروجی مورد انتظار
تغییر کد در فایل‌های مرتبط، commit یا PR جدید با پیام واضح، و عبور تمام معیارهای پذیرش.

## 🧪 دستورات اعتبارسنجی
- `cd backend && alembic revision --autogenerate -m "Test sync"`

## ⚠️ ریسک‌ها و موارد احتیاط
احتمال از دست رفتن داده‌ها در صورت تولید یا اعمال نادرست مهاجرت‌ها.
...
1. اجرای `alembic revision --autogenerate -m "Check for pending changes"` روی کدبیس فعلی یک اسکریپت مهاجرت خالی (یا فقط تغییرات مورد انتظار) تولید می‌کند. _(verify: backend_test)_
2. یک تغییر جدید در مدل (مثلاً اضافه کردن یک ستون) به درستی یک اسکریپت مهاجرت تولید می‌کند که با موفقیت اعمال می‌شود. _(verify: backend_test)_

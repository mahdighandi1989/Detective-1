---
task_id: 7a90727a-e342-41b8-a672-4ea8251f0ce0
title: محدود کردن پیکربندی Cross-Origin Resource Sharing (CORS) به مبدأهای شناخته‌شده
type: security
priority: high
...
created_at: '2026-06-06T17:37:04.896244+00:00'
updated_at: '2026-06-07T19:54:45.117334+00:00'
archived: true
archived_at: '2026-06-07T19:54:45.117321+00:00'
target_files:
- backend/app/main.py
---

# محدود کردن پیکربندی Cross-Origin Resource Sharing (CORS) به مبدأهای شناخته‌شده

## Raw Idea

یک سیاست CORS بیش از حد مجاز (به عنوان مثال، اجازه دادن `*` برای `allow_origins`) می‌تواند API بک‌اند را در معرض حملات Cross-Site Request Forgery (CSRF) و سایر درخواست‌های مخرب cross-origin قرار دهد. اگرچه برای توسعه راحت است، اما در محیط تولید یک خطر امنیتی قابل توجه است. برنامه FastAPI باید به صراحت فقط مبدأهای فرانت‌اند مورد اعتماد را لیست کند.
...
- `backend/app/main.py` — `app` — CORS middleware معمولاً در طول مقداردهی اولیه برنامه FastAPI پیکربندی می‌شود. محتوای واقعی در این فاز ارائه نشده است.
...
- `frontend/src/lib/api.ts` — کلاینت فرانت‌اند که درخواست‌ها را به بک‌اند ارسال می‌کند.
- `docker-compose.yml` — نام‌های هاست سرویس را تعریف می‌کند که ممکن است برای مبدأهای CORS مرتبط باشند.

## 🌐 نقشهٔ وابستگی‌ها
بر تمام نقاط پایانی API و ارتباط فرانت‌اند-بک‌اند تأثیر می‌گذارد.
...
- [ ] لیست `allow_origins` در `CORSMiddleware` به صراحت فقط دامنه‌های فرانت‌اند مورد اعتماد را لیست می‌کند.
- [ ] درخواست‌ها از مبدأهای نامعتبر توسط بک‌اند با خطای CORS مسدود می‌شوند.
- [ ] برنامه فرانت‌اند می‌تواند با موفقیت تماس‌های API را به بک‌اند برقرار کند.
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. پیکربندی CORS را در `backend/app/main.py` (یا جایی که `CORSMiddleware` مقداردهی اولیه می‌شود) بررسی کنید و اطمینان حاصل کنید که `allow_origins` به جای `["*"]`، روی لیستی خاص از دامنه‌های مورد اعتماد (به عنوان مثال، `["http://localhost:3000", "https://your-frontend-domain.com"]`) تنظیم شده است.
...
_بعد:_
```
app.add_middleware(CORSMiddleware, allow_origins=["http://localhost:3000", "https://your-frontend.com"], allow_credentials=True, allow_methods=["*"])
```
...
## 🧪 دستورات اعتبارسنجی
- `curl -v -H "Origin: http://malicious-site.com" http://localhost:8000/api/v1/health`

## ⚠️ ریسک‌ها و موارد احتیاط
مسدود کردن تصادفی درخواست‌های قانونی فرانت‌اند در صورت عدم تعیین صحیح مبدأها.
...
1. لیست `allow_origins` در `CORSMiddleware` به صراحت فقط دامنه‌های فرانت‌اند مورد اعتماد را لیست می‌کند. _(verify: static)_
2. درخواست‌ها از مبدأهای نامعتبر توسط بک‌اند با خطای CORS مسدود می‌شوند. _(verify: api_response)_
3. برنامه فرانت‌اند می‌تواند با موفقیت تماس‌های API را به بک‌اند برقرار کند. _(verify: ui_interaction)_

---
task_id: 5922cc90-a80b-4989-beef-bcba79a4ef91
title: فایل‌های پیکربندی ممکن است حاوی اسرار hardcoded یا پیش‌فرض‌های حساس باشند.
type: security
priority: critical
...
created_at: '2026-06-06T17:37:04.890448+00:00'
updated_at: '2026-06-07T19:54:57.942235+00:00'
archived: true
archived_at: '2026-06-07T19:54:57.942222+00:00'
target_files:
- backend/app/core/config.py
- backend/app/core/security.py
---

# فایل‌های پیکربندی ممکن است حاوی اسرار hardcoded یا پیش‌فرض‌های حساس باشند.

## Raw Idea

در حالی که `backend/.env.example` استفاده صحیح از متغیرهای محیطی را نشان می‌دهد، معمول است که `backend/app/core/config.py` یا `backend/app/core/security.py` حاوی مقادیر پیش‌فرض برای اطلاعات حساس (مانند کلید مخفی JWT، اعتبارنامه پایگاه داده، کلیدهای API) باشند که در محیط تولید توسط متغیرهای محیطی override نمی‌شوند. hardcode کردن اسرار یک خطر امنیتی قابل توجه است، زیرا می‌توانند در کنترل سورس، آرتیفکت‌های بیلد یا لاگ‌ها افشا شوند.
...
- `backend/app/core/config.py:1-100` — `Settings` — مقادیر پیش‌فرض را که باید متغیرهای محیطی باشند، بررسی کنید.
  ```python
  class Settings(BaseSettings):
      SECRET_KEY: str = 'super-insecure-default-key'  # ⚠️ Hardcoded secret
      DATABASE_URL: str = 'postgresql://user:pass@host/db'
      ...
  ```
- `backend/app/core/security.py:1-50` — `ALGORITHM` — کلید مخفی JWT ممکن است در اینجا تعریف شده باشد.
  ```python
  SECRET_KEY = 'another-hardcoded-secret'
  ```
...
FastAPI, Pydantic BaseSettings, متغیرهای محیطی.

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `backend/.env.example` — باید همه متغیرهای محیطی مورد نیاز را لیست کند.
- `docker-compose.yml` — متغیرهای محیطی اغلب در اینجا منتقل می‌شوند.
- `backend/Dockerfile` — متغیرهای محیطی ممکن است در طول بیلد تنظیم یا استفاده شوند.

## 🌐 نقشهٔ وابستگی‌ها
بر امنیت برنامه به صورت جهانی، به ویژه احراز هویت و دسترسی به پایگاه داده، تأثیر می‌گذارد.
...
- [ ] همه مقادیر پیکربندی حساس از متغیرهای محیطی بارگذاری می‌شوند.
- [ ] هیچ secret hardcoded در فایل‌های کد منبع وجود ندارد.
- [ ] `backend/.env.example` جامع و به‌روز است.
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. بررسی کاملی از `backend/app/core/config.py`، `backend/app/core/security.py` و `docker-compose.yml` انجام دهید تا اطمینان حاصل شود که همه مقادیر حساس از متغیرهای محیطی بارگذاری می‌شوند و هیچ پیش‌فرض hardcoded ناامنی ندارند. `backend/.env.example` را برای شامل شدن همه متغیرهای محیطی لازم به‌روزرسانی کنید.

## 💡 نمونه‌های قبل/بعد
**بارگذاری secret از محیط**
...
## 🧪 دستورات اعتبارسنجی
- `grep -r "SECRET_KEY" backend/app/core/config.py backend/app/core/security.py`
- `docker-compose config | grep -i password`

## ⚠️ ریسک‌ها و موارد احتیاط
پیکربندی نادرست می‌تواند از شروع یا عملکرد صحیح برنامه جلوگیری کند.
...
1. همه مقادیر پیکربندی حساس از متغیرهای محیطی بارگذاری می‌شوند. _(verify: static)_
2. هیچ secret hardcoded در فایل‌های کد منبع وجود ندارد. _(verify: static)_
3. `backend/.env.example` جامع و به‌روز است. _(verify: static)_

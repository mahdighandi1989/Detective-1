---
task_id: 4d14927c-a1e7-4934-bde7-f97150cb403e
title: پیاده‌سازی مدیریت خطا و لاگ‌گیری متمرکز و سازگار
type: refactor
priority: medium
...
created_at: '2026-06-06T17:39:01.329903+00:00'
updated_at: '2026-06-07T19:52:52.385829+00:00'
archived: true
archived_at: '2026-06-07T19:52:52.385813+00:00'
target_files:
- backend/app/main.py
- backend/app/services/audit.py
---

# پیاده‌سازی مدیریت خطا و لاگ‌گیری متمرکز و سازگار

## Raw Idea

در حالی که `backend/app/services/audit.py` نشان‌دهنده برخی لاگ‌گیری‌ها است، یک استراتژی جامع، متمرکز برای مدیریت خطا و لاگ‌گیری برای یک API قوی حیاتی است. پاسخ‌های خطای ناسازگار (مثلاً فرمت‌های JSON مختلف، کدهای وضعیت) توسعه سمت کلاینت را دشوار می‌کند. عدم وجود لاگ‌گیری متمرکز برای استثناها و رویدادهای حیاتی، اشکال‌زدایی، نظارت و ممیزی امنیتی را مختل می‌کند. FastAPI مکانیزم‌هایی برای مدیریت استثنای سراسری فراهم می‌کند که باید از آنها استفاده شود.
...
- `backend/app/main.py:20-50` — `app.add_exception_handler` — اینجا جایی است که هندلرهای استثنای سراسری ثبت می‌شوند.
  ```python
  from fastapi import FastAPI, HTTPException, Request
  from fastapi.responses import JSONResponse
  from starlette.exceptions import HTTPException as StarletteHTTPException
  
  app = FastAPI(title="Detective-1 API")
  ...
  ```
- `backend/app/services/audit.py:1-20` — `log_event` — اطمینان حاصل کنید که این سرویس به طور سازگار برای لاگ‌گیری رویدادها/خطاهای حیاتی استفاده می‌شود.
  ```python
  import logging
  
  logger = logging.getLogger(__name__)
  
  def log_event(event_type: str, user_id: UUID = None, details: dict = None):
      # این سرویس باید با استراتژی لاگ‌گیری سراسری ادغام شود.
      logger.info(f"Audit Event: {event_type}", extra={"user_id": user_id, "details": details})
  ```
...
Python, FastAPI, ماژول `logging`

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `backend/app/api/routes/auth.py` — تمام مسیرهای API می‌توانند استثنا ایجاد کنند و باید از مدیریت خطای متمرکز بهره‌مند شوند.
- `backend/app/core/config.py` — برای پیکربندی لاگ‌گیری.

## 🌐 نقشهٔ وابستگی‌ها
بر تمام نقاط پایانی API و قابلیت نگهداری و مشاهده‌پذیری کلی بک‌اند تأثیر می‌گذارد.
...
- [ ] تمام نقاط پایانی API یک فرمت خطای JSON سازگار را برای خطاهای مورد انتظار (مثلاً 404، 400) و غیرمنتظره (مثلاً 500) برمی‌گردانند.
- [ ] خطاهای حیاتی و استثناها با ردیابی پشته و زمینه مربوطه لاگ می‌شوند.
- [ ] هیچ اطلاعات حساسی در پاسخ‌های خطا فاش نمی‌شود.
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. 1. یک فرمت پاسخ خطای استاندارد (مثلاً `{"detail": "Error message", "code": "ERROR_CODE"}`) تعریف کنید. 2. هندلرهای استثنای سراسری را در `backend/app/main.py` پیاده‌سازی کنید تا استثناهای رایج (مانند `HTTPException`, `RequestValidationError`, `Exception` عمومی) را گرفته و پاسخ‌های سازگار برگردانید. 3. یک راه‌حل لاگ‌گیری ساختاریافته (مانند ماژول `logging` با `structlog` یا مشابه) را در تمام سرویس‌ها و مسیرهای API ادغام کنید و اطمینان حاصل کنید که خطاها با زمینه کافی (شناسه درخواست، شناسه کاربر، ردیابی پشته) لاگ می‌شوند. 4. اطمینان حاصل کنید که `backend/app/services/audit.py` در این استراتژی سراسری برای رویدادهای حیاتی ادغام شده است.

## 💡 نمونه‌های قبل/بعد
**مثال هندلر استثنای عمومی در main.py**

_قبل:_
```
(بدون هندلر سراسری صریح، مسیرهای جداگانه خطاها را مدیریت می‌کنند)
```
...
## 🧪 دستورات اعتبارسنجی
- `curl -X GET http://localhost:8000/non-existent-path`
- `curl -X POST http://localhost:8000/api/auth/login -d '{"invalid_field": "value"}'`
- `Trigger an intentional server-side error and check backend logs.`

## ⚠️ ریسک‌ها و موارد احتیاط
لاگ‌گیری بیش از حد یا کمتر از حد، احتمال شکستن مدیریت خطای موجود در مسیرهای خاص در صورت عدم ادغام دقیق.
...
1. تمام نقاط پایانی API یک فرمت خطای JSON سازگار را برای خطاهای مورد انتظار (مثلاً 404، 400) و غیرمنتظره (مثلاً 500) برمی‌گردانند. _(verify: api_response)_
2. خطاهای حیاتی و استثناها با ردیابی پشته و زمینه مربوطه لاگ می‌شوند. _(verify: backend_test)_
3. هیچ اطلاعات حساسی در پاسخ‌های خطا فاش نمی‌شود. _(verify: api_response)_

---
task_id: 9a260e09-4966-4681-8b42-d412a6c82f1f
title: پیاده‌سازی محدودیت نرخ (Rate Limiting) در نقاط پایانی API برای جلوگیری از سوءاستفاده و حملات DoS
type: security
priority: high
...
created_at: '2026-06-06T17:37:04.899722+00:00'
updated_at: '2026-06-07T19:54:34.735227+00:00'
archived: true
archived_at: '2026-06-07T19:54:34.735215+00:00'
target_files:
- backend/app/main.py
- backend/app/api/routes/auth.py
---

# پیاده‌سازی محدودیت نرخ (Rate Limiting) در نقاط پایانی API برای جلوگیری از سوءاستفاده و حملات DoS

## Raw Idea

پروژه به نظر می‌رسد یک API عمومی با احراز هویت (`auth.py`) و نقاط پایانی داده مختلف باشد. بدون محدودیت نرخ، API در برابر حملات brute-force (به ویژه در نقاط پایانی ورود)، حملات denial-of-service (DoS) و مصرف منابع آسیب‌پذیر است. این می‌تواند منجر به عدم دسترسی به سرویس و افزایش هزینه‌های زیرساخت شود.
...
- `backend/app/main.py` — `app` — middleware محدودیت نرخ را می‌توان به صورت سراسری در اینجا اضافه کرد. محتوای واقعی در این فاز ارائه نشده است.
- `backend/app/api/routes/auth.py` — `login` — نقاط پایانی احراز هویت برای محدودیت نرخ حیاتی هستند. محتوای واقعی در این فاز ارائه نشده است.
...
FastAPI, potentially `fastapi-limiter` or custom middleware

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `backend/app/api/routes/__init__.py` — تمام مسیرهای API می‌توانند از محدودیت نرخ بهره‌مند شوند.
- `backend/app/core/config.py` — آستانه‌های محدودیت نرخ ممکن است قابل پیکربندی باشند.

## 🌐 نقشهٔ وابستگی‌ها
تمام نقاط پایانی API را در برابر درخواست‌های بیش از حد محافظت می‌کند.
...
- [ ] تلاش برای ورود بیش از N بار در یک بازه M دقیقه‌ای منجر به پاسخ `429 Too Many Requests` می‌شود.
- [ ] نقاط پایانی عمومی API دارای محدودیت نرخ معقولی هستند.
- [ ] مستندات مربوط به محدودیت نرخ به‌روزرسانی شده است.
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. یک مکانیزم محدودیت نرخ را در برنامه FastAPI ادغام کنید. این می‌تواند یک middleware باشد که به صورت سراسری اعمال می‌شود یا دکوراتورهایی که به نقاط پایانی خاص و حساس (مانند `/auth/login`، `/auth/register` یا نقاط پایانی جستجو/گراف با مصرف منابع بالا) اعمال می‌شوند.

## 💡 نمونه‌های قبل/بعد
**اعمال محدودیت نرخ به یک نقطه پایانی**
...
## 🧪 دستورات اعتبارسنجی
- `for i in $(seq 1 10); do curl -s -o /dev/null -w "%{\http_code}\n" -X POST http://localhost:8000/api/v1/auth/login -d '{"username":"test","password":"badpassword"}'; done`

## ⚠️ ریسک‌ها و موارد احتیاط
کاربران قانونی ممکن است در صورت سخت‌گیرانه بودن آستانه‌ها، محدود شوند؛ نیاز به تنظیم دقیق دارد.
...
1. تلاش برای ورود بیش از N بار در یک بازه M دقیقه‌ای منجر به پاسخ `429 Too Many Requests` می‌شود. _(verify: api_response)_
2. نقاط پایانی عمومی API دارای محدودیت نرخ معقولی هستند. _(verify: backend_test)_
3. مستندات مربوط به محدودیت نرخ به‌روزرسانی شده است. _(verify: static)_

...
title: جلوگیری افشای کلیدهای API/اسرار فرانت‌اند
type: security
priority: critical
...
updated_at: '2026-06-07T19:55:28.146734+00:00'
archived: true
archived_at: '2026-06-07T19:55:28.146720+00:00'
target_files:
...
# جلوگیری افشای کلیدهای API/اسرار فرانت‌اند
...
ادامه (دور 1): جلوگیری از افشای کلیدهای API حساس و اسرار در باندل فرانت‌اند
...
- `frontend/src/app/encyclopedia/page.tsx` — از evidence verifier در دور 0
- `backend/app/core/config.py` — از evidence verifier در دور 0
- `backend/app/main.py` — از evidence verifier در دور 0
...
✅ بخش‌هایی که در دور قبل انجام شد:
  - هیچ کلید API حساس بک‌اند (مانند LLM، سرویس‌های OSINT) در باندل جاوااسکریپت سمت کلاینت یافت نمی‌شود؛ `render.yaml` و `backend/app/core/config.py` این موضوع را تأیید می‌کنند.
  - تمام متغیرهای محیطی که در فرانت‌اند قابل دسترسی هستند، با پیشوند `NEXT_PUBLIC_` هستند؛ `frontend/next.config.js` و `frontend/src/lib/api.ts` این الگو را رعایت کرده‌اند.
  - فرانت‌اند با سرویس‌های حساس بک‌اند فقط از طریق API بک‌اند خود پروژه ارتباط برقرار می‌کند؛ `frontend/next.config.js`، `frontend/src/lib/api.ts` و `backend/app/main.py` این ارتباط متمرکز را نشان می‌دهند.
...
1. (auto-verify system probe) صفحه اصلی deployed قابل دسترسی است _(verify: static)_

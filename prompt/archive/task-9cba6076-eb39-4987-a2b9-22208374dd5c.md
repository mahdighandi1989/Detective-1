---
task_id: 9cba6076-eb39-4987-a2b9-22208374dd5c
title: پیکربندی امن اسرار از متغیرهای محیطی
type: security
priority: critical
...
created_at: '2026-06-06T17:37:04.884367+00:00'
updated_at: '2026-06-07T19:55:37.485157+00:00'
archived: true
archived_at: '2026-06-07T19:55:37.485141+00:00'
target_files:
- backend/app/core/config.py
---

# پیکربندی امن اسرار از متغیرهای محیطی

## Raw Idea

وجود `backend/.env.example` نشان‌دهنده استفاده از متغیرهای محیطی برای پیکربندی است. با این حال، بسیار مهم است که `backend/app/core/config.py` (یا منطق بارگذاری پیکربندی مشابه) تمام اطلاعات حساس (اعتبارنامه پایگاه داده، کلیدهای API، اسرار JWT و غیره) را به درستی از متغیرهای محیطی بارگذاری کند، بدون مقادیر پیش‌فرض hardcoded یا fallback که می‌توانند در محیط تولید استفاده شوند. hardcode کردن اسرار یا داشتن پیش‌فرض‌های ناامن می‌تواند منجر به افشای اعتبارنامه‌ها شود.
...
اطمینان از بارگذاری امن تمام تنظیمات حساس از متغیرهای محیطی

## 📍 موقعیت دقیق در پروژه
_(file:line — symbol — snippet)_

- `backend/app/core/config.py` — `Settings` — انتظار می‌رود این فایل مدیریت پیکربندی برنامه و بارگذاری اسرار را انجام دهد. محتوای واقعی در این فاز ارائه نشده است.
...
- `backend/.env.example` — فایل نمونه برای متغیرهای محیطی.
- `backend/Dockerfile` — ممکن است حاوی تعاریف متغیرهای محیطی باشد.
- `docker-compose.yml` — متغیرهای محیطی را برای سرویس‌ها تعریف می‌کند.
- `backend/app/db/session.py` — احتمالاً از اعتبارنامه‌های پایگاه داده از پیکربندی استفاده می‌کند.
- `backend/app/core/security.py` — احتمالاً از secret JWT از پیکربندی استفاده می‌کند.
- `backend/app/services/llm_adapter.py` — ممکن است از کلیدهای API خارجی از پیکربندی استفاده کند.
- `backend/app/services/osint_agent.py` — ممکن است از کلیدهای API خارجی از پیکربندی استفاده کند.

## 🌐 نقشهٔ وابستگی‌ها
پیکربندی اصلی بر اتصالات پایگاه داده، احراز هویت و ادغام سرویس‌های خارجی تأثیر می‌گذارد.
...
- [ ] تمام مقادیر پیکربندی حساس از متغیرهای محیطی بارگذاری می‌شوند.
- [ ] هیچ اسرار حساسی در `backend/app/core/config.py` یا هیچ فایل منبع دیگری hardcode نشده است.
- [ ] برنامه در صورت عدم وجود متغیرهای محیطی حیاتی در محیط‌های تولید، با خطا مواجه می‌شود.
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. فایل `backend/app/core/config.py` را بررسی کنید تا تأیید شود که تمام پارامترهای پیکربندی حساس منحصراً از متغیرهای محیطی بارگذاری می‌شوند (به عنوان مثال، با استفاده از `os.getenv` یا `pydantic.Settings`) و هیچ پیش‌فرض حساس برای تولید وجود ندارد. هرگونه اسرار hardcoded را حذف کنید.

## 💡 نمونه‌های قبل/بعد
**پیش‌فرض hardcoded ناامن**
...
عدم راه‌اندازی برنامه در صورت عدم تنظیم صحیح متغیرهای محیطی در زمان استقرار.
...
1. تمام مقادیر پیکربندی حساس از متغیرهای محیطی بارگذاری می‌شوند. _(verify: backend_test)_
2. هیچ اسرار حساسی در `backend/app/core/config.py` یا هیچ فایل منبع دیگری hardcode نشده است. _(verify: static)_
3. برنامه در صورت عدم وجود متغیرهای محیطی حیاتی در محیط‌های تولید، با خطا مواجه می‌شود. _(verify: backend_test)_

---
task_id: 9eb90964-0ff5-4fa4-996f-1ccd9732931e
title: Inconsistent Python Dependency Management Strategy
type: refactor
priority: medium
...
created_at: '2026-06-06T17:40:29.482572+00:00'
updated_at: '2026-06-07T19:51:43.944449+00:00'
archived: true
archived_at: '2026-06-07T19:51:43.944430+00:00'
target_files:
...
# Inconsistent Python Dependency Management Strategy

## Raw Idea

The backend project uses both `backend/pyproject.toml` (suggesting a modern dependency manager like Poetry or PDM) and `backend/requirements.txt` (for pip). This dual approach can lead to dependency drift, where versions specified in one file become inconsistent with the other, potentially causing build failures or runtime issues, especially if `requirements.txt` is used for Docker builds and `pyproject.toml` for local development. A single source of truth for dependencies is crucial for maintainability and reproducibility.
...
- `backend/pyproject.toml:N/A` — `N/A` — Primary dependency definition, likely for local development.
  ```
  N/A
  ```
- `backend/requirements.txt:N/A` — `N/A` — Secondary dependency definition, likely used in Docker builds.
  ```
  N/A
  ```
- `backend/Dockerfile:N/A` — `N/A` — The Docker build process needs to align with the chosen dependency management strategy.
  ```
  N/A
  ```
...
Python, Poetry/PDM (implied by `pyproject.toml`), Pip, Docker.

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `backend/docker-entrypoint.sh` — May interact with dependency installation or environment setup.

## 🌐 نقشهٔ وابستگی‌ها
This inconsistency affects all backend builds, deployments, and local development environments. Resolving it ensures consistent dependency resolution across all stages.
...
- [ ] The `backend/Dockerfile` successfully builds the image using a single, consistent dependency management approach (e.g., only `requirements.txt` or only `pyproject.toml` via Poetry).
- [ ] Local development setup (if using `pyproject.toml`) and Docker builds use the same dependency versions.
- [ ] Documentation (if any) reflects the chosen dependency management strategy.
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. Standardize the dependency management. If `pyproject.toml` is the intended source of truth, ensure `requirements.txt` is automatically generated from it (e.g., using `poetry export -f requirements.txt --output requirements.txt --without-hashes` or a similar command) and committed. Alternatively, modify the `backend/Dockerfile` to install dependencies directly from `pyproject.toml` using Poetry/PDM. If `requirements.txt` is preferred, `pyproject.toml` should be removed or its dependency section kept minimal.

## 💡 نمونه‌های قبل/بعد
**Example Dockerfile change for Poetry-based installation**
...
## 🧪 دستورات اعتبارسنجی
- `docker build -f backend/Dockerfile -t detective-backend-test .`
- `poetry install --check`

## ⚠️ ریسک‌ها و موارد احتیاط
Potential for breaking existing build pipelines or local development setups if not carefully migrated. Requires testing across environments.
...
1. The `backend/Dockerfile` successfully builds the image using a single, consistent dependency management approach (e.g., only `requirements.txt` or only `pyproject.toml` via Poetry). _(verify: backend_test)_
2. Local development setup (if using `pyproject.toml`) and Docker builds use the same dependency versions. _(verify: backend_test)_
3. Documentation (if any) reflects the chosen dependency management strategy. _(verify: manual_only)_

## Task Steps

### Step 1: بررسی و جمع‌آوری وابستگی‌های موجود از pyproject.toml و requirements.txt
**Status:** `pending` (0%)
**Scope:** این مرحله شامل بررسی دقیق محتوای فایل‌های `backend/pyproject.toml` و `backend/requirements.txt` برای شناسایی و لیست کردن تمامی وابستگی‌های پروژه است. هدف، درک وضعیت فعلی و شناسایی هرگونه تفاوت یا تکرار بین دو فایل است. این مرحله شامل هیچ‌گونه تغییر یا اصلاحی در فایل‌ها نیست، بلکه صرفاً جمع‌آوری اطلاعات است.
**Excerpt:**
```
The backend project uses both `backend/pyproject.toml` (suggesting a modern dependency manager like Poetry or PDM) and `backend/requirements.txt` (for pip). This dual approach can lead to dependency drift, where versions specified in one file become inconsistent with the other, potentially causing build failures or runtime issues, especially if `requirements.txt` is used for Docker builds and `pyproject.toml` for local development. A single source of truth for dependencies is crucial for maintainability and reproducibility.
```

### Step 2: انتقال وابستگی‌های تولیدی از requirements.txt به pyproject.toml
**Status:** `pending` (0%)
**Scope:** این مرحله بر روی انتقال تمامی وابستگی‌های مورد نیاز برای اجرای پروژه در محیط تولید از `backend/requirements.txt` به بخش `[project.dependencies]` در `backend/pyproject.toml` تمرکز دارد. هدف این است که `pyproject.toml` به عنوان منبع اصلی حقیقت برای وابستگی‌های زمان اجرا عمل کند. این مرحله شامل وابستگی‌های توسعه‌ای یا ابزارهای جانبی نمی‌شود.
— [merged] این مرحله به انتقال تمامی وابستگی‌های مربوط به توسعه، تست، linting و فرمت‌بندی که در `backend/requirements.txt` ذکر شده‌اند، به بخش مناسبی برای وابستگی‌های توسعه‌ای در `backend/pyproject.toml` (مانند `[project.optional-dependencies.dev]` یا `[tool.poetry.group.dev.dependencies]`) اختصاص دارد. هدف این است که تمامی وابستگی‌های پروژه، چه تولیدی و چه توسعه‌ای، تحت مدیریت `pyproject.toml` قرار گیرند.
**Excerpt:**
```
The backend project uses both `backend/pyproject.toml` (suggesting a modern dependency manager like Poetry or PDM) and `backend/requirements.txt` (for pip). This dual approach can lead to dependency drift, where versions specified in one file become inconsistent with the other, potentially causing build failures or runtime issues, especially if `requirements.txt` is used for Docker builds and `pyproject.toml` for local development. A single source of truth for dependencies is crucial for maintainability and reproducibility.
```

### Step 3: حذف فایل requirements.txt
**Status:** `pending` (0%)
**Scope:** پس از اطمینان از اینکه تمامی وابستگی‌های تولیدی و توسعه‌ای به `backend/pyproject.toml` منتقل شده‌اند، این مرحله شامل حذف کامل فایل `backend/requirements.txt` از پروژه است. این اقدام برای حذف منبع دوم حقیقت برای وابستگی‌ها و جلوگیری از 'dependency drift' در آینده ضروری است.
**Excerpt:**
```
The backend project uses both `backend/pyproject.toml` (suggesting a modern dependency manager like Poetry or PDM) and `backend/requirements.txt` (for pip). This dual approach can lead to dependency drift, where versions specified in one file become inconsistent with the other, potentially causing build failures or runtime issues, especially if `requirements.txt` is used for Docker builds and `pyproject.toml` for local development. A single source of truth for dependencies is crucial for maintainability and reproducibility.
```

### Step 4: به‌روزرسانی پیکربندی‌های Build/Deployment برای استفاده از pyproject.toml
**Status:** `pending` (0%)
**Scope:** این مرحله شامل شناسایی و اصلاح تمامی اسکریپت‌های build، Dockerfileها، یا پیکربندی‌های CI/CD است که قبلاً برای نصب وابستگی‌ها به `backend/requirements.txt` ارجاع می‌دادند. این ارجاعات باید به‌روزرسانی شوند تا به جای آن از `backend/pyproject.toml` (یا فایل lock مربوطه مانند `poetry.lock` یا `pdm.lock`) برای نصب وابستگی‌ها استفاده کنند. این امر تضمین می‌کند که فرآیندهای خودکار نیز از منبع واحد حقیقت جدید پیروی می‌کنند.
**Excerpt:**
```
This dual approach can lead to dependency drift, where versions specified in one file become inconsistent with the other, potentially causing build failures or runtime issues, especially if `requirements.txt` is used for Docker builds and `pyproject.toml` for local development. A single source of truth for dependencies is crucial for maintainability and reproducibility.
```

### Step 5: تأیید حل وابستگی‌ها و عملکرد پروژه
**Status:** `pending` (0%)
**Scope:** این مرحله نهایی شامل اجرای یک نصب تمیز وابستگی‌ها با استفاده از مدیر وابستگی (Poetry یا PDM) بر اساس `backend/pyproject.toml` و سپس اجرای تست‌های موجود پروژه است. هدف این است که اطمینان حاصل شود که تمامی وابستگی‌ها به درستی حل شده‌اند، پروژه بدون خطا build می‌شود و تمامی قابلیت‌ها طبق انتظار عمل می‌کنند. این مرحله تأیید می‌کند که فرآیند یکپارچه‌سازی هیچ رگرسیون یا مشکلی ایجاد نکرده است.
**Excerpt:**
```
This dual approach can lead to dependency drift, where versions specified in one file become inconsistent with the other, potentially causing build failures or runtime issues, especially if `requirements.txt` is used for Docker builds and `pyproject.toml` for local development. A single source of truth for dependencies is crucial for maintainability and reproducibility.
```

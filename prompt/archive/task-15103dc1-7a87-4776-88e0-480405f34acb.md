---
task_id: 15103dc1-7a87-4776-88e0-480405f34acb
title: Implement comprehensive unit and integration tests for backend services
type: refactor
priority: high
...
created_at: '2026-06-06T17:38:15.836711+00:00'
updated_at: '2026-06-07T19:54:04.970993+00:00'
archived: true
archived_at: '2026-06-07T19:54:04.970979+00:00'
target_files:
- backend/app/services/risk_engine.py
- backend/app/services/osint_agent.py
- backend/app/services/credibility.py
---

# Implement comprehensive unit and integration tests for backend services

## Raw Idea

The project's backend has a `tests` directory, but it primarily focuses on API routes. Critical business logic resides within `backend/app/services/` (e.g., `risk_engine`, `osint_agent`, `credibility`, `llm_adapter`, `neo4j_sync`). Lack of dedicated tests for these services makes refactoring risky, increases the likelihood of regressions, and hinders independent verification of their correctness. This impacts the overall reliability and maintainability of the application.
...
- `backend/app/services/risk_engine.py:1-500` — `RiskEngineService` — Identify key public methods within this service for thorough unit testing.
  ```python
  class RiskEngineService:
      def calculate_risk(data: dict) -> float:
          # ⚠️ This method and others in this service need unit tests
          ...
  ```
- `backend/app/services/osint_agent.py:1-500` — `OSINTAgentService` — Identify key public methods, especially those interacting with external systems, for testing.
  ```python
  class OSINTAgentService:
      def gather_intelligence(query: str) -> dict:
          # ⚠️ This method and others need unit/integration tests, especially due to external interactions
          ...
  ```
- `backend/app/services/credibility.py:1-500` — `CredibilityService` — Identify core logic methods for comprehensive unit testing.
  ```python
  class CredibilityService:
      def assess_credibility(entity_id: int) -> float:
          # ⚠️ Core business logic here needs robust unit tests
          ...
  ```
...
- `backend/tests/__init__.py` — Entry point for backend tests, new service test files will be added here.
- `backend/tests/conftest.py` — Test fixtures and configuration might need updates for service testing.
- `backend/pyproject.toml` — Defines project dependencies and test runner (pytest).

## 🌐 نقشهٔ وابستگی‌ها
These services are fundamental to the application's business logic and are called by various API routes and potentially Celery tasks. Improving their test coverage will enhance the reliability of the entire backend.
...
- [ ] New test files are created under `backend/tests/services/` for at least `risk_engine`, `osint_agent`, and `credibility` services.
- [ ] Key public methods of these services are covered by unit tests, including positive, negative, and edge cases.
- [ ] Test coverage reports show a significant increase in coverage for `backend/app/services/` directory.
...
1. Create new test files under `backend/tests/services/` (or a similar structure) to implement comprehensive unit and integration tests for the public methods of key services. Focus on covering various input scenarios, edge cases, and expected outputs. Mock external dependencies where necessary.
...
Time-consuming to write comprehensive tests for complex services; might uncover existing bugs or design flaws in the service logic.
...
1. New test files are created under `backend/tests/services/` for at least `risk_engine`, `osint_agent`, and `credibility` services. _(verify: static)_
2. Key public methods of these services are covered by unit tests, including positive, negative, and edge cases. _(verify: manual_only)_
3. Test coverage reports show a significant increase in coverage for `backend/app/services/` directory. _(verify: manual_only)_
4. Tests pass successfully without regressions. _(verify: backend_test)_

## Task Steps

### Step 1: Add Unit Tests for `risk_engine` Service Logic
**Status:** `pending` (0%)
**Scope:** این مرحله بر ایجاد تست‌های واحد جامع و اختصاصی برای منطق کسب‌وکار موجود در سرویس `risk_engine` واقع در مسیر `backend/app/services/` تمرکز دارد. هدف پوشش توابع حیاتی، موارد لبه‌ای و مدیریت خطا برای اطمینان از صحت و پایداری آن است. این مرحله شامل تست کردن endpointهای API یا کامپوننت‌های UI نمی‌شود.
— [merged] این مرحله شامل توسعه تست‌های واحد اختصاصی برای سرویس `osint_agent` است که بخشی از دایرکتوری `backend/app/services/` می‌باشد. تست‌ها باید منطق جمع‌آوری، تجزیه و پردازش داده‌های عامل را تأیید کنند و از استحکام و دقت آن در منابع و فرمت‌های مختلف داده اطمینان حاصل کنند. این مرحله به عملکرد داخلی `osint_agent` محدود می‌شود.
— [merged] این مرحله بر پیاده‌سازی تست‌های واحد برای سرویس `credibility` در `backend/app/services/` تمرکز دارد. تست‌ها الگوریتم‌ها و قوانین مورد استفاده برای ارزیابی اعتبار را تأیید می‌کنند و سناریوهای مختلفی را که بر امتیازات یا تعیین اعتبار تأثیر می‌گذارند، پوشش می‌دهند. هدف اطمینان از یکپارچگی و سازگاری منطق اعتبار است.
— [merged] این مرحله به ایجاد تست‌های واحد برای سرویس `llm_adapter`، واقع در `backend/app/services/` اختصاص دارد. تست‌ها منطق آداپتور برای تعامل با مدل‌های زبان بزرگ (LLM) را تأیید می‌کنند، از جمله ساختاردهی پرامپت، تجزیه پاسخ و مدیریت خطاهای خاص تعاملات LLM. تمرکز بر منطق داخلی آداپتور است، نه خود LLM.
— [merged] این مرحله بر توسعه تست‌های واحد برای سرویس `neo4j_sync` تمرکز دارد که مسئول همگام‌سازی داده‌ها با پایگاه داده گراف Neo4j است و در `backend/app/services/` قرار دارد. تست‌ها پوشش نگاشت داده، مدیریت تراکنش و منطق به‌روزرسانی گراف را برای اطمینان از یکپارچگی و سازگاری داده‌ها در Neo4j پوشش می‌دهند. این مرحله منطق همگام‌سازی را از تعاملات واقعی پایگاه داده در صورت امکان (با استفاده از mock) جدا می‌کند.
**Excerpt:**
```
The project's backend has a `tests` directory, but it primarily focuses on API routes. Critical business logic resides within `backend/app/services/` (e.g., `risk_engine`, `osint_agent`, `credibility`, `llm_adapter`, `neo4j_sync`). Lack of dedicated tests for these services makes refactoring risky, increases the likelihood of regressions, and hinders independent verification of their correctness. This impacts the overall reliability and maintainability of the application.
```

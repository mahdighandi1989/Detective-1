---
task_id: a544c24d-cbf0-4e8a-92cd-2bff9cfe30cd
title: 'endpoint بک‌اند بلااستفاده: PATCH /{person_id}'
type: audit
priority: medium
...
created_at: '2026-06-13T18:15:55.029355+00:00'
updated_at: '2026-06-13T20:20:54.273687+00:00'
target_files:
- backend/app/api/routes/persons.py
---

# endpoint بک‌اند بلااستفاده: PATCH /{person_id}

## Raw Idea

## 📋 شرح
endpoint `PATCH /{person_id}` در `backend/app/api/routes/persons.py` تعریف شده ولی هیچ `fetch`، `axios`، `apiClient.*`، یا frontend call به آن دیده نمی‌شود.

## 🔍 جزئیات
- method: `PATCH`
- path: `/{person_id}`
- فایل: `backend/app/api/routes/persons.py`
...
---
[scan #2 at 2026-06-13T18:15:55.096611+00:00]
## 📋 شرح
endpoint `POST /persons/{person_id}/evaluate` در `backend/app/api/routes/risk.py` تعریف شده ولی هیچ `fetch`، `axios`، `apiClient.*`، یا frontend call به آن دیده نمی‌شود.

## 🔍 جزئیات
- method: `POST`
- path: `/persons/{person_id}/evaluate`
- فایل: `backend/app/api/routes/risk.py`
- علت: no 
---
[scan #3 at 2026-06-13T18:15:55.202211+00:00]
## 📋 شرح
endpoint `POST /persons/{person_id}/override` در `backend/app/api/routes/risk.py` تعریف شده ولی هیچ `fetch`، `axios`، `apiClient.*`، یا frontend call به آن دیده نمی‌شود.

## 🔍 جزئیات
- method: `POST`
- path: `/persons/{person_id}/override`
- فایل: `backend/app/api/routes/risk.py`
- علت: no 
---
[scan #4 at 2026-06-13T18:15:55.272257+00:00]
## 📋 شرح
endpoint `PATCH /{source_id}` در `backend/app/api/routes/sources.py` تعریف شده ولی هیچ `fetch`، `axios`، `apiClient.*`، یا frontend call به آن دیده نمی‌شود.

## 🔍 جزئیات
- method: `PATCH`
- path: `/{source_id}`
- فایل: `backend/app/api/routes/sources.py`
...
- [ ] مشخص شد endpoint `PATCH /{person_id}` در کدام دسته است (orphan/internal/deprecated)
...
1. گام ۱: grep روی `/{person_id}` در frontend + scripts/ + docs/ — اگر caller هست، اتصال را drop شده اصلاح کن.
...
## 🧪 دستورات اعتبارسنجی
- `python -m py_compile backend/app/api/routes/persons.py`
- `ruff check backend/app/api/routes/persons.py`
- `pytest -x`
...
1. مشخص شد endpoint `PATCH /{person_id}` در کدام دسته است (orphan/internal/deprecated) _(verify: static)_

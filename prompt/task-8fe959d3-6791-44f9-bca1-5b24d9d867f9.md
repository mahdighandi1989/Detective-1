---
task_id: 8fe959d3-6791-44f9-bca1-5b24d9d867f9
title: 'endpoint بک‌اند بلااستفاده: GET /{person_id}/neighbors'
type: audit
priority: medium
...
created_at: '2026-06-13T18:15:54.962926+00:00'
updated_at: '2026-06-13T20:20:49.399197+00:00'
target_files:
- backend/app/api/routes/graph.py
---

# endpoint بک‌اند بلااستفاده: GET /{person_id}/neighbors

## Raw Idea

## 📋 شرح
endpoint `GET /{person_id}/neighbors` در `backend/app/api/routes/graph.py` تعریف شده ولی هیچ `fetch`، `axios`، `apiClient.*`، یا frontend call به آن دیده نمی‌شود.

## 🔍 جزئیات
- method: `GET`
- path: `/{person_id}/neighbors`
- فایل: `backend/app/api/routes/graph.py`
...
---
[scan #2 at 2026-06-13T18:15:55.029032+00:00]
## 📋 شرح
endpoint `GET /{person_id}` در `backend/app/api/routes/persons.py` تعریف شده ولی هیچ `fetch`، `axios`، `apiClient.*`، یا frontend call به آن دیده نمی‌شود.
...
---
[scan #3 at 2026-06-13T18:15:55.133115+00:00]
## 📋 شرح
endpoint `GET /persons/{person_id}/latest` در `backend/app/api/routes/risk.py` تعریف شده ولی هیچ `fetch`، `axios`، `apiClient.*`، یا frontend call به آن دیده نمی‌شود.

## 🔍 جزئیات
- method: `GET`
- path: `/persons/{person_id}/latest`
- فایل: `backend/app/api/routes/risk.py`
- علت: no fronte
---
[scan #4 at 2026-06-13T18:15:55.167302+00:00]
## 📋 شرح
endpoint `GET /persons/{person_id}/history` در `backend/app/api/routes/risk.py` تعریف شده ولی هیچ `fetch`، `axios`، `apiClient.*`، یا frontend call به آن دیده نمی‌شود.
...
- `backend/app/models/person.py` — `graph.py` این فایل را import می‌کند
...
- [ ] مشخص شد endpoint `GET /{person_id}/neighbors` در کدام دسته است (orphan/internal/deprecated)
...
1. گام ۱: grep روی `/{person_id}/neighbors` در frontend + scripts/ + docs/ — اگر caller هست، اتصال را drop شده اصلاح کن.
...
## 🧪 دستورات اعتبارسنجی
- `python -m py_compile backend/app/api/routes/graph.py`
- `ruff check backend/app/api/routes/graph.py`
- `pytest -x`
...
1. مشخص شد endpoint `GET /{person_id}/neighbors` در کدام دسته است (orphan/internal/deprecated) _(verify: static)_

---
task_id: 765eb40d-7d8f-4480-b18b-0a85916e9f12
title: Person Model Lacks Granular Current Position Fields
type: refactor
priority: medium
...
created_at: '2026-06-13T18:15:20.132408+00:00'
updated_at: '2026-06-13T20:19:58.097668+00:00'
target_files:
...
# Person Model Lacks Granular Current Position Fields

## Raw Idea

The `Person` model in `backend/app/models/person.py` stores the current position as a single field, `current_position_title` (line 105). However, the frontend `PersonSummary` in `frontend/src/app/persons/page.tsx` expects separate `current_position` (for the title) and `current_organization` fields. This forces either parsing on the frontend (which is brittle) or a less granular data representation in the backend, hindering flexible querying and data management.
...
- `backend/app/models/person.py:105-105` — `current_position_title` — Single field for current position title
  ```python
  current_position_title: Mapped[Optional[str]] = mapped_column(String, nullable=True)
  ```
- `frontend/src/app/persons/page.tsx:31-32` — `PersonSummary` — Frontend expects separate position and organization fields
  ```tsx
  current_position?: string | null;
    current_organization?: string | null;
  ```
- `frontend/src/app/persons/page.tsx:209-212` — `PersonCard` — Frontend rendering logic for separate position and organization
  ```tsx
  <p className="mt-0.5 truncate text-xs text-slate-400">
              ...
              {person.current_organization
                ? ` — ${person.current_organization}`
  ```
- `backend/app/api/routes/persons.py:214-215` — `create_person` — Person creation uses `current_position` (inferred from schema, but model has `current_position_title`)
  ```python
...
SQLAlchemy ORM, FastAPI, Pydantic, Alembic, Next.js

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `backend/app/schemas/person.py` — Pydantic schemas for Person CRUD operations (inferred)
- `backend/app/api/routes/graph.py` (سطر 96) — Graph API uses `current_position` for node labels
- `backend/alembic/versions/0001_initial.py` — Alembic migration will be needed to alter the table schema
...
## 🌐 نقشهٔ وابستگی‌ها
This change impacts the core `Person` model, its API schemas, and how position data is displayed in the frontend. An Alembic migration is required for existing databases.
...
- [ ] The `Person` model in `backend/app/models/person.py` has `current_position` and `current_organization` fields.
- [ ] Person creation and update API endpoints correctly handle these new fields.
- [ ] Frontend `PersonCard` displays `current_position` and `current_organization` correctly.
- [ ] An Alembic migration script is created to handle the schema change and data migration.
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. 1.  **Refactor `backend/app/models/person.py`:** Replace `current_position_title` with two distinct fields: `current_position` (for the job title) and `current_organization`.
2.  **Update Schemas:** Modify `backend/app/schemas/person.py` (specifically `PersonCreate`, `PersonUpdate`, `PersonRead`, `PersonListItem`) to reflect these new, granular fields.
3.  **Update API Routes:** Adjust `backend/app/api/routes/persons.py` to handle the new fields during person creation and update operations.
4.  **Alembic Migration:** Create an Alembic migration script to safely split existing data from the `current_position_title` column into the new `current_position` and `current_organization` columns.

## 💡 نمونه‌های قبل/بعد
**Person model with granular position fields**
...
_بعد:_
```
current_position: Mapped[Optional[str]] = mapped_column(String, nullable=True, comment="عنوان سمت فعلی")
    current_organization: Mapped[Optional[str]] = mapped_column(String, nullable=True, comment="سازمان فعلی")
```
...
## 🧪 دستورات اعتبارسنجی
- `alembic revision --autogenerate -m "Split current_position_title into current_position and current_organization"`
- `alembic upgrade head`
- `pytest backend/tests/test_persons.py`

## ⚠️ ریسک‌ها و موارد احتیاط
Data loss during migration if the Alembic script is not carefully designed and tested.
...
1. The `Person` model in `backend/app/models/person.py` has `current_position` and `current_organization` fields. _(verify: static)_
2. Person creation and update API endpoints correctly handle these new fields. _(verify: static)_
3. Frontend `PersonCard` displays `current_position` and `current_organization` correctly. _(verify: static)_
4. An Alembic migration script is created to handle the schema change and data migration. _(verify: static)_

## Task Steps

### Step 1: Add 'current_position' and 'current_organization' fields to Backend Person Model
**Status:** `pending` (0%)
**Scope:** This step involves modifying the `Person` model definition in `backend/app/models/person.py` to introduce two new fields: `current_position` for the job title and `current_organization` for the associated organization. These fields are intended to replace the less granular `current_position_title` field. The focus is solely on defining the new fields within the model structure, without affecting existing data, database schema, or API endpoints at this stage.
**Excerpt:**
```
The `Person` model in `backend/app/models/person.py` stores the current position as a single field, `current_position_title` (line 105). However, the frontend `PersonSummary` in `frontend/src/app/persons/page.tsx` expects separate `current_position` (for the title) and `current_organization` fields. This forces either parsing on the frontend (which is brittle) or a less granular data representation in the backend, hindering flexible querying and data management.
```

### Step 2: Remove 'current_position_title' field from Backend Person Model
**Status:** `pending` (0%)
**Scope:** This step focuses on removing the deprecated `current_position_title` field from the `Person` model definition in `backend/app/models/person.py`. This action ensures that the backend model no longer uses the single, combined field for position information, aligning with the new granular fields introduced in the previous step. This change is purely at the model definition level.
**Excerpt:**
```
The `Person` model in `backend/app/models/person.py` stores the current position as a single field, `current_position_title` (line 105). However, the frontend `PersonSummary` in `frontend/src/app/persons/page.tsx` expects separate `current_position` (for the title) and `current_organization` fields. This forces either parsing on the frontend (which is brittle) or a less granular data representation in the backend, hindering flexible querying and data management.
```

### Step 3: Generate and Apply Database Migration for Person Model Changes
**Status:** `pending` (0%)
**Scope:** This crucial step involves generating a database migration script that reflects the structural changes made to the `Person` model: the addition of `current_position` and `current_organization` fields, and the removal of `current_position_title`. After generation, the migration script must be applied to the database schema to physically update the table structure. This ensures the database schema matches the updated backend model.
**Excerpt:**
```
The `Person` model in `backend/app/models/person.py` stores the current position as a single field, `current_position_title` (line 105). However, the frontend `PersonSummary` in `frontend/src/app/persons/page.tsx` expects separate `current_position` (for the title) and `current_organization` fields. This forces either parsing on the frontend (which is brittle) or a less granular data representation in the backend, hindering flexible querying and data management.
```

### Step 4: Migrate Existing 'current_position_title' Data to New Fields
**Status:** `pending` (0%)
**Scope:** This step focuses on data integrity by creating and executing a one-time script to migrate existing data. For each `Person` record, the value from the now-removed `current_position_title` field (if any) needs to be parsed and used to populate the new `current_position` and `current_organization` fields. This ensures that historical data is preserved and correctly structured under the new model, preventing data loss during the transition.
**Excerpt:**
```
The `Person` model in `backend/app/models/person.py` stores the current position as a single field, `current_position_title` (line 105). However, the frontend `PersonSummary` in `frontend/src/app/persons/page.tsx` expects separate `current_position` (for the title) and `current_organization` fields. This forces either parsing on the frontend (which is brittle) or a less granular data representation in the backend, hindering flexible querying and data management.
```

### Step 5: Update Backend API Endpoints to Expose New Person Fields
**Status:** `pending` (0%)
**Scope:** This step involves modifying the backend API endpoints responsible for serving `Person` data. The serialization logic (e.g., Pydantic models, FastAPI response models, or similar ORM serialization) must be updated to include the newly added `current_position` and `current_organization` fields in API responses, and to exclude the now-removed `current_position_title`. This ensures that frontend consumers receive the correct, granular data.
**Excerpt:**
```
The `Person` model in `backend/app/models/person.py` stores the current position as a single field, `current_position_title` (line 105). However, the frontend `PersonSummary` in `frontend/src/app/persons/page.tsx` expects separate `current_position` (for the title) and `current_organization` fields. This forces either parsing on the frontend (which is brittle) or a less granular data representation in the backend, hindering flexible querying and data management.
```

### Step 6: Update Frontend 'PersonSummary' Component to Display New Fields
**Status:** `pending` (0%)
**Scope:** This step focuses on updating the `PersonSummary` component in `frontend/src/app/persons/page.tsx`. The component's rendering logic and data expectations must be adjusted to consume and display the new `current_position` and `current_organization` fields, which are now provided by the backend API. This ensures the UI accurately reflects the updated data structure and provides a better user experience.
**Excerpt:**
```
However, the frontend `PersonSummary` in `frontend/src/app/persons/page.tsx` expects separate `current_position` (for the title) and `current_organization` fields. This forces either parsing on the frontend (which is brittle) or a less granular data representation in the backend, hindering flexible querying and data management.
```

### Step 7: Update Frontend Data Fetching Logic for New Person Fields
**Status:** `pending` (0%)
**Scope:** This step involves modifying the frontend's data fetching logic to correctly retrieve the `current_position` and `current_organization` fields from the updated backend API. This might include adjusting API client calls, data transformation layers, or state management updates to ensure the `PersonSummary` component receives the new, granular data. This step bridges the gap between the updated backend API and the frontend UI.
**Excerpt:**
```
However, the frontend `PersonSummary` in `frontend/src/app/persons/page.tsx` expects separate `current_position` (for the title) and `current_organization` fields. This forces either parsing on the frontend (which is brittle) or a less granular data representation in the backend, hindering flexible querying and data management.
```

### Step 8: Add/Update Backend Tests for Person Model and API Changes
**Status:** `pending` (0%)
**Scope:** This step involves creating new unit and integration tests, or updating existing ones, for the backend `Person` model and its associated API endpoints. Tests should verify that the `current_position` and `current_organization` fields are correctly handled (e.g., creation, retrieval, validation) and that `current_position_title` is no longer present or used. This ensures the backend changes are robust and functional.
**Excerpt:**
```
The `Person` model in `backend/app/models/person.py` stores the current position as a single field, `current_position_title` (line 105). However, the frontend `PersonSummary` in `frontend/src/app/persons/page.tsx` expects separate `current_position` (for the title) and `current_organization` fields.
```

### Step 9: Add/Update Frontend Tests for PersonSummary Component
**Status:** `pending` (0%)
**Scope:** This step involves creating new unit and integration tests, or updating existing ones, for the `PersonSummary` component and its data fetching logic. Tests should verify that the component correctly receives, processes, and displays the `current_position` and `current_organization` fields, and that it no longer relies on `current_position_title`. This ensures the frontend changes are robust and provide the expected user experience.
**Excerpt:**
```
However, the frontend `PersonSummary` in `frontend/src/app/persons/page.tsx` expects separate `current_position` (for the title) and `current_organization` fields.
```

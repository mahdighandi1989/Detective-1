---
task_id: 16f46a6a-a114-45ab-a29d-5c175531798f
title: 'Critical: Unrestricted CORS Allowing Any Origin'
type: security
priority: critical
...
created_at: '2026-06-13T17:59:49.081761+00:00'
updated_at: '2026-06-13T20:16:29.862133+00:00'
target_files:
- backend/app/main.py
---

# Critical: Unrestricted CORS Allowing Any Origin

## Raw Idea

The FastAPI application in `backend/app/main.py` configures `CORSMiddleware`. The `cors_origins` variable, if not explicitly set in `settings.BACKEND_CORS_ORIGINS`, defaults to `["*"]` (line 173). This configuration allows requests from *any* origin, effectively disabling a fundamental browser-based security mechanism. This makes the backend vulnerable to cross-site request forgery (CSRF) attacks and other malicious interactions from arbitrary websites.
...
- `backend/app/main.py:173-179` — `create_application` — The default value `["*"]` is the issue.
  ```python
...
- `backend/app/core/config.py` — Where `BACKEND_CORS_ORIGINS` should be defined.
- `frontend/next.config.js` — Frontend configuration, might need to match backend CORS.
...
## 🌐 نقشهٔ وابستگی‌ها
This is a global middleware affecting all API endpoints.
...
- [ ] The `BACKEND_CORS_ORIGINS` setting is explicitly configured with trusted origins.
- [ ] Requests from untrusted origins are blocked by the CORS policy.
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. Restrict `BACKEND_CORS_ORIGINS` in `backend/app/core/config.py` (or its environment variable equivalent) to a specific list of trusted origins (e.g., the frontend URL). Avoid using `["*"]` in production.
...
_بعد:_
```
cors_origins = getattr(settings, "BACKEND_CORS_ORIGINS", None) or ["http://localhost:3000", "https://your-frontend-domain.com"]
```
...
## 🧪 دستورات اعتبارسنجی
- `Manual test: Attempt to make an AJAX request from a different domain (not in allowed origins) to a backend endpoint. It should fail with a CORS error in the browser console.`

## ⚠️ ریسک‌ها و موارد احتیاط
Accidentally blocking legitimate frontend requests if not configured correctly.
...
1. The `BACKEND_CORS_ORIGINS` setting is explicitly configured with trusted origins. _(verify: static)_
2. Requests from untrusted origins are blocked by the CORS policy. _(verify: static)_

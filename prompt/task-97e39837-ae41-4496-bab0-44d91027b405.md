...
updated_at: '2026-06-13T18:02:21.229952+00:00'
target_files:
...
1. `poetry install` and `pip install -r requirements.txt` result in the same set of installed packages and versions.
2. `pyproject.toml` contains all direct dependencies listed in `requirements.txt`.
3. The `requirements.txt` file can be successfully regenerated from `pyproject.toml` without introducing new discrepancies.

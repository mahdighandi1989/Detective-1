---
task_id: 97e39837-ae41-4496-bab0-44d91027b405
title: Python Dependency Mismatch and Incompleteness between pyproject.toml and requirements.txt
type: bug
priority: critical
...
created_at: '2026-06-13T18:02:21.229950+00:00'
updated_at: '2026-06-13T20:16:52.875761+00:00'
target_files:
- backend/pyproject.toml
- backend/requirements.txt
---

# Python Dependency Mismatch and Incompleteness between pyproject.toml and requirements.txt

## Raw Idea

The `requirements.txt` file, which claims to be the output of `poetry export` from `pyproject.toml`, is significantly out of sync. Many critical packages (e.g., `psycopg`, `PyJWT`, `flower`, `google-generativeai`, `selectolax`, `beautifulsoup4`, `lxml`, `boto3`, `minio`, `requests`, `aiofiles`, `python-dotenv`, `orjson`, `Pillow`, `python-slugify`, `tiktoken`) are listed in `requirements.txt` but are entirely missing from `pyproject.toml`. Additionally, for packages present in both, `requirements.txt` often pins to specific patch versions while `pyproject.toml` uses broader caret (`^`) ranges. This discrepancy will lead to inconsistent development and deployment environments, potential build failures, and unexpected runtime behavior depending on whether Poetry or Pip is used for dependency management.
...
- `backend/pyproject.toml:1-40` — The primary dependency definition file that needs to be updated.
  ```
  ...
  ...
  ```
- `backend/requirements.txt:1-70` — The generated dependency list, showing the current state of discrepancies.
  ```
  # SOURCE OF TRUTH: pyproject.toml
  # This file is kept in sync with pyproject.toml via:
...
Python 3.11+, Poetry, Pip, FastAPI

## 🌐 نقشهٔ وابستگی‌ها
This issue affects the entire backend's dependency resolution and environment setup, impacting all modules and services.
...
- [ ] `poetry install` and `pip install -r requirements.txt` result in the same set of installed packages and versions.
- [ ] `pyproject.toml` contains all direct dependencies listed in `requirements.txt`.
- [ ] The `requirements.txt` file can be successfully regenerated from `pyproject.toml` without introducing new discrepancies.
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. Update `pyproject.toml` to include all missing dependencies currently present in `requirements.txt`. For existing dependencies, align version constraints to be consistent (e.g., use `^` in `pyproject.toml` and then regenerate `requirements.txt` using `poetry export -f requirements.txt --output requirements.txt --without-hashes --with dev` to ensure strict pinning in `requirements.txt`).

## 💡 نمونه‌های قبل/بعد
**Example: Adding missing dependency to pyproject.toml**
...
**Example: Aligning version constraints**
...
- `cd backend && poetry export -f requirements.txt --output requirements.txt.new --without-hashes --with dev && diff requirements.txt requirements.txt.new`

## ⚠️ ریسک‌ها و موارد احتیاط
Potential for new dependency conflicts if versions are not carefully chosen; requires thorough testing of the build process.
...
1. `poetry install` and `pip install -r requirements.txt` result in the same set of installed packages and versions. _(verify: static)_
2. `pyproject.toml` contains all direct dependencies listed in `requirements.txt`. _(verify: static)_
3. The `requirements.txt` file can be successfully regenerated from `pyproject.toml` without introducing new discrepancies. _(verify: static)_

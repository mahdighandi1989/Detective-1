---
task_id: 87f4994c-143d-4c38-9be4-cded486ec275
title: افزودن تست‌های واحد برای منطق RBAC و سطح دسترسی مدل User
type: security
priority: high
...
created_at: '2026-06-13T18:10:52.028746+00:00'
updated_at: '2026-06-13T20:18:49.554275+00:00'
target_files:
- backend/app/models/user.py
---

# افزودن تست‌های واحد برای منطق RBAC و سطح دسترسی مدل User

## Raw Idea

مدل `User` در `app.models.user.py` انام‌های `UserRole` و `ClearanceLevel` را به همراه متدهای کمکی (`has_role`, `has_clearance`, `satisfies`, `can_access`) تعریف می‌کند که برای کنترل دسترسی برنامه حیاتی هستند. این متدها حاوی منطق تجاری مهمی برای مجوزدهی هستند. اطمینان از رفتار صحیح آن‌ها از طریق تست‌های واحد اختصاصی برای امنیت و جلوگیری از دسترسی غیرمجاز به داده‌ها بسیار مهم است.
...
- `backend/app/models/user.py:52-115, 205-213` — `UserRole, ClearanceLevel, User.has_role, User.has_clearance` — انام‌ها و متدهای RBAC که باید تست شوند
  ```python
...
SQLAlchemy ORM, Python Enums, RBAC

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `backend/app/api/deps.py` (سطر 34) — از `UserRole`, `ClearanceLevel` و منطق آن‌ها استفاده می‌کند
- `backend/app/api/routes/persons.py` (سطر 56) — از `ClassificationLevel` (که از `app.core.enums` بازصادر شده) استفاده می‌کند و منطق دسترسی را در `_can_access_classification` دارد که باید با `User.has_clearance` همسو باشد.
- `backend/app/api/routes/auth.py` (سطر 81) — کاربران را با نقش ایجاد می‌کند
...
## 🌐 نقشهٔ وابستگی‌ها
این انام‌ها و متدها برای تعریف و اعمال سیاست‌های دسترسی در سراسر برنامه اساسی هستند.
...
- [ ] `UserRole.satisfies` برای تمام ترکیبات نقش‌ها (مثلاً ADMIN، ANALYST) مقدار بولی صحیح را برگرداند.
- [ ] `ClearanceLevel.can_access` برای تمام ترکیبات سطوح دسترسی مقدار بولی صحیح را برگرداند.
- [ ] `User.has_role` و `User.has_clearance` منطق انام‌های زیرین را به درستی منعکس کنند، از جمله نادیده‌گرفتن `is_superuser`.
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. یک فایل تست جدید به نام `backend/tests/test_user_model.py` ایجاد کنید تا به طور خاص انام‌های `UserRole`، `ClearanceLevel` و متدهای `has_role` و `has_clearance` مدل `User` را تست کند.
...
ریسک پایین، زیرا تست واحد ایزوله منطق خالص است.
...
1. `UserRole.satisfies` برای تمام ترکیبات نقش‌ها (مثلاً ADMIN، ANALYST) مقدار بولی صحیح را برگرداند. _(verify: static)_
2. `ClearanceLevel.can_access` برای تمام ترکیبات سطوح دسترسی مقدار بولی صحیح را برگرداند. _(verify: static)_
3. `User.has_role` و `User.has_clearance` منطق انام‌های زیرین را به درستی منعکس کنند، از جمله نادیده‌گرفتن `is_superuser`. _(verify: static)_

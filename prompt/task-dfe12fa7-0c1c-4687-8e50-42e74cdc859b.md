---
task_id: dfe12fa7-0c1c-4687-8e50-42e74cdc859b
title: '[منطق] Missing Explicit RBAC Enforcement Functions'
type: logic_audit
priority: high
...
created_at: '2026-06-13T18:13:39.525599+00:00'
updated_at: '2026-06-13T20:19:48.126929+00:00'
---

# [منطق] Missing Explicit RBAC Enforcement Functions
...
While `backend/app/api/deps.py` and `backend/app/core/security.py` both mention RBAC and role definitions, neither module explicitly lists concrete functions or dependencies (e.g., `role_required(roles: list)`, `clearance_required(level: ClassificationLevel)`) as outputs that would be used directly by FastAPI endpoints to enforce specific role or classification requirements. The current outputs focus on resolving the *user*, but not on *checking* their permissions against an endpoint's requireme

## 💥 پیامد (impact)
Without explicit, reusable RBAC enforcement functions, developers are likely to implement ad-hoc permission checks at each endpoint. This leads to inconsistencies, missed checks on mutation paths, and potential security vulnerabilities. It also makes the application harder to audit for correct autho

## 🛠 پیشنهاد رفع اولیه
In `backend/app/core/security.py`, implement core functions like `has_role(user: User, required_roles: list)` and `has_clearance(user: User, required_level: ClassificationLevel)`. Then, in `backend/app/api/deps.py`, create FastAPI dependency functions that leverage these core functions, such as `role_required_dependency(roles: list) -> Callable[[User], User]` or `clearance_required_dependency(leve
...
### Step 1: Implement `has_role` core function in `backend/app/core/security.py`
**Status:** `pending` (0%)
**Scope:** This step focuses on creating the foundational `has_role` function within the `backend/app/core/security.py` module. Its primary responsibility is to check if a given `User` object possesses any of the `required_roles` provided. This function should encapsulate the core logic for role verification, independent of FastAPI's dependency injection system. It does not involve integrating with FastAPI endpoints or defining dependencies.
— [merged] This step involves creating the `has_clearance` function in `backend/app/core/security.py`. This function will determine if a `User` object's classification level meets or exceeds a specified `required_level` (of type `ClassificationLevel`). Similar to `has_role`, this function provides the core logic for clearance verification, isolated from the web framework's specifics. It does not include any FastAPI dependency definitions.
**Excerpt:**
```
In `backend/app/core/security.py`, implement core functions like `has_role(user: User, required_roles: list)`
```

### Step 2: Create `role_required_dependency` FastAPI dependency in `backend/app/api/deps.py`
**Status:** `pending` (0%)
**Scope:** This step focuses on defining the `role_required_dependency` function within `backend/app/api/deps.py`. This function will act as a FastAPI dependency, taking a list of `roles` as an argument. It will leverage the `has_role` core function (implemented in `backend/app/core/security.py`) to check the authenticated `User`'s roles. If the user does not possess the required roles, it should raise an appropriate HTTP exception. The return type should be `Callable[[User], User]`, indicating it returns the user if authorized.
— [merged] This step involves defining the `clearance_required_dependency` function in `backend/app/api/deps.py`. This FastAPI dependency will accept a `level` (representing a `ClassificationLevel`) as an argument. It will utilize the `has_clearance` core function (from `backend/app/core/security.py`) to verify if the authenticated `User` meets the specified clearance level. An HTTP exception should be raised if the user's clearance is insufficient.
**Excerpt:**
```
Then, in `backend/app/api/deps.py`, create FastAPI dependency functions that leverage these core functions, such as `role_required_dependency(roles: list) -> Callable[[User], User]`
```

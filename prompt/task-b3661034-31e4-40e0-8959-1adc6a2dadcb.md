---
task_id: b3661034-31e4-40e0-8959-1adc6a2dadcb
title: 'endpoint بک‌اند بلااستفاده: GET /admin'
type: audit
priority: medium
...
created_at: '2026-06-13T18:15:55.507535+00:00'
updated_at: '2026-06-13T20:21:12.098157+00:00'
target_files:
- backend/app/core/rbac.py
---

# endpoint بک‌اند بلااستفاده: GET /admin

## Raw Idea

## 📋 شرح
endpoint `GET /admin` در `backend/app/core/rbac.py` تعریف شده ولی هیچ `fetch`، `axios`، `apiClient.*`، یا frontend call به آن دیده نمی‌شود.

## 🔍 جزئیات
- method: `GET`
- path: `/admin`
- فایل: `backend/app/core/rbac.py`
...
---
[scan #2 at 2026-06-13T18:15:55.583344+00:00]
## 📋 شرح
endpoint `GET /secret` در `backend/app/core/rbac.py` تعریف شده ولی هیچ `fetch`، `axios`، `apiClient.*`، یا frontend call به آن دیده نمی‌شود.
...
- `backend/app/core/security.py` — `rbac.py` این فایل را import می‌کند
- `backend/alembic/env.py` — این فایل `rbac.py` را import می‌کند (caller)
...
- [ ] مشخص شد endpoint `GET /admin` در کدام دسته است (orphan/internal/deprecated)
...
1. گام ۱: grep روی `/admin` در frontend + scripts/ + docs/ — اگر caller هست، اتصال را drop شده اصلاح کن.
...
## 🧪 دستورات اعتبارسنجی
- `python -m py_compile backend/app/core/rbac.py`
...
1. مشخص شد endpoint `GET /admin` در کدام دسته است (orphan/internal/deprecated) _(verify: static)_

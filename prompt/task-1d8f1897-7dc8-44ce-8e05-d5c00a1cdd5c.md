---
task_id: 1d8f1897-7dc8-44ce-8e05-d5c00a1cdd5c
title: 'High: Unprivileged User Registration Defaults to Analyst Role'
type: security
priority: high
...
created_at: '2026-06-13T18:08:44.721368+00:00'
updated_at: '2026-06-13T20:18:09.018390+00:00'
target_files:
- backend/app/api/routes/auth.py
---

# High: Unprivileged User Registration Defaults to Analyst Role

## Raw Idea

In `backend/app/api/routes/auth.py`, the `/register` endpoint allows new users to be created. If the `role` field is not explicitly provided in the `UserCreate` payload, the user's role defaults to `"analyst"` (line 81: `role=getattr(payload, "role", None) or "analyst"`). The "analyst" role, as defined in `backend/app/models/user.py` and used in `backend/app/api/routes/encyclopedia.py` (`WRITE_ROLES`), grants significant write permissions, including ingesting and modifying encyclopedia articles and person profiles. This allows any unauthenticated user to register and immediately gain high privileges without administrative oversight, posing a severe security risk.
...
- `backend/app/api/routes/auth.py:78-83` — `register` — The default assignment of "analyst" role.
  ```python
  user = User(
...
- `backend/app/models/user.py` (سطر 52) — Defines `UserRole` enum and its privileges.
- `backend/app/schemas/auth.py` — `UserCreate` schema, might need to remove `role` from payload or default it to `VIEWER`.
- `backend/app/api/routes/encyclopedia.py` (سطر 76) — Uses `WRITE_ROLES` which includes "analyst".
- `backend/app/api/routes/persons.py` (سطر 195) — Uses `require_roles("analyst", "admin")` for create/update.
...
## 🌐 نقشهٔ وابستگی‌ها
This affects user registration and the initial privileges of new accounts.
...
- [ ] Newly registered users (without explicit role in payload) are assigned the `VIEWER` role.
- [ ] Users with `VIEWER` role cannot create or update encyclopedia articles or person profiles.
- [ ] Existing tests for user registration still pass.
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. Change the default role for new user registrations to the lowest privilege role, `UserRole.VIEWER`. Implement an administrative process for role upgrades, or require explicit role assignment by an existing `ADMIN` user.
...
_بعد:_
```
role=getattr(payload, "role", UserRole.VIEWER)
```
...
## 🧪 دستورات اعتبارسنجی
- `Manual test: Register a new user without specifying a role. Log in as that user and attempt to create an article or person profile. It should be denied with a 403 Forbidden error.`
- `Add an integration test for user registration to verify default role.`

## ⚠️ ریسک‌ها و موارد احتیاط
None, as it restricts privileges.
...
1. Newly registered users (without explicit role in payload) are assigned the `VIEWER` role. _(verify: static)_
2. Users with `VIEWER` role cannot create or update encyclopedia articles or person profiles. _(verify: static)_
3. Existing tests for user registration still pass. _(verify: static)_

---
task_id: 3e11e3c6-3158-4674-bc7e-e50dbae1e3e9
title: کامپوننت فرانت‌اند monolithic صفحه دانشنامه
type: refactor
priority: high
...
created_at: '2026-06-13T18:08:44.761212+00:00'
updated_at: '2026-06-13T20:18:16.924886+00:00'
target_files:
- frontend/src/app/encyclopedia/page.tsx
---

# کامپوننت فرانت‌اند monolithic صفحه دانشنامه

## Raw Idea

فایل `frontend/src/app/encyclopedia/page.tsx` با ۶۸۵ خط کد، یک کامپوننت monolithic است. این کامپوننت تمام جنبه‌های صفحه دانشنامه را مدیریت می‌کند، از جمله مدیریت وضعیت (state management)، فراخوانی‌های API (loadArticles, handleSemanticSearch, handleCreate, handleAnalyze, handleDelete)، منطق فرم و رندر کردن لیست مقالات، نتایج جستجو و فرم ایجاد. این رویکرد اصل مسئولیت واحد (Single Responsibility Principle) را نقض می‌کند و خواندن، درک، تست و نگهداری کامپوننت را دشوار می‌سازد.
...
- `frontend/src/app/encyclopedia/page.tsx:1-685` — کل فایل نیاز به بازسازی دارد.
  ```tsx
  'use client';
...
- `frontend/src/lib/api.ts` — می‌تواند برای فراخوانی‌های API استفاده شود یا الگویی برای آن باشد
- `frontend/src/components/Header.tsx` — کامپوننت اصلی layout
- `frontend/src/app/layout.tsx` — layout ریشه

## 🌐 نقشهٔ وابستگی‌ها
این بازسازی مختص فرانت‌اند است. تغییرات عمدتاً بر صفحه دانشنامه و زیرکامپوننت‌های جدید آن تأثیر می‌گذارد.
...
- [ ] `frontend/src/app/encyclopedia/page.tsx` از نظر اندازه و پیچیدگی به طور قابل توجهی کاهش یابد.
- [ ] فراخوانی‌های API به یک ماژول یا Hook جداگانه انتزاع شوند.
- [ ] منطق UI بین کامپوننت‌های کوچک‌تر و قابل استفاده مجدد توزیع شود.
- [ ] صفحه دانشنامه پس از بازسازی، عملکردی یکسان داشته باشد.
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. 1. فراخوانی‌های API به یک فایل ابزار اختصاصی (مثلاً `frontend/src/lib/encyclopediaApi.ts`) یا React Hookهای سفارشی منتقل شوند. 2. کامپوننت اصلی `EncyclopediaPage` به زیرکامپوننت‌های کوچک‌تر و متمرکزتر (مثلاً `ArticleList`, `ArticleSearch`, `ArticleForm`, `ArticleDetail`) تقسیم شود. 3. انواع و Enumهای مشترک (مانند `Classification`, `ContentType`, `Article`, `Source`) به یک فایل مشترک (مثلاً `frontend/src/lib/types.ts` یا `frontend/src/lib/enums.ts`) منتقل شوند.

## 💡 نمونه‌های قبل/بعد
**قبل: منطق API و UI در یک کامپوننت**
...
**بعد: جداسازی منطق API و UI**

_بعد:_
```
// frontend/src/lib/encyclopediaApi.ts
export async function fetchArticles() { ... }

// frontend/src/app/encyclopedia/page.tsx
export default function EncyclopediaPage() {
  const { articles, loadArticles } = useArticles(); // Custom hook
  return (<ArticleList articles={articles} /> <ArticleForm onCreate={handleCreate} />);
}
```
...
## 🧪 دستورات اعتبارسنجی
- `npm test (اگر تست‌های کامپوننت وجود دارد)`
- `npm run dev (و بررسی دستی تمام قابلیت‌های صفحه /encyclopedia)`

## ⚠️ ریسک‌ها و موارد احتیاط
احتمال رگرسیون‌های UI در صورت عدم مدیریت دقیق تقسیم کامپوننت و وضعیت.
...
1. `frontend/src/app/encyclopedia/page.tsx` از نظر اندازه و پیچیدگی به طور قابل توجهی کاهش یابد. _(verify: static)_
2. فراخوانی‌های API به یک ماژول یا Hook جداگانه انتزاع شوند. _(verify: static)_
3. منطق UI بین کامپوننت‌های کوچک‌تر و قابل استفاده مجدد توزیع شود. _(verify: static)_
4. صفحه دانشنامه پس از بازسازی، عملکردی یکسان داشته باشد. _(verify: static)_

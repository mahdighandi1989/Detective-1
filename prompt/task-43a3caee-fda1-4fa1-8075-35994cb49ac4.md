...
updated_at: '2026-06-06T17:44:37.410906+00:00'
target_files:
...
1. [مرحله 1 — اسکن و به‌روزرسانی وابستگی‌های شخص ثالث برای رفع آسیب‌پذیری‌های امنیتی] باقی‌مانده: شناسایی، اسکن و به‌روزرسانی تمام وابستگی‌های شخص ثالث در `pyproject.toml`، `requirements.txt` و `package.json` برای رفع آسیب‌پذیری‌ها انجام نشده است. _(verify: static)_
...
4. یک ابزار اسکن وابستگی مناسب (مانند `pip-audit` برای پایتون و `npm audit` برای نود) را در CI/CD پیکربندی و اجرا کنید. _(verify: static)_
5. آسیب‌پذیری‌های امنیتی با شدت بحرانی و بالا را که در تسک‌ها ذکر شده‌اند، برطرف کنید. _(verify: static)_
6. هیچ تستی fail نمی‌شود (`npm run test` / `pytest`) _(verify: static)_
7. linter بدون warning عبور می‌کند _(verify: static)_
8. type-check موفق است (`tsc --noEmit` / `mypy`) _(verify: static)_

---
task_id: 4a99f60b-a89b-40e4-9542-1850da58e336
title: اطمینان از اعمال سازگار و قوی RBAC در تمام نقاط پایانی API
type: security
priority: critical
...
created_at: '2026-06-06T17:37:04.886736+00:00'
updated_at: '2026-06-06T17:48:16.044910+00:00'
target_files:
- backend/app/core/rbac.py
- backend/app/api/routes/persons.py
---

# اطمینان از اعمال سازگار و قوی RBAC در تمام نقاط پایانی API

## Raw Idea

وجود `backend/app/core/rbac.py` نشان‌دهنده قصد پیاده‌سازی کنترل دسترسی مبتنی بر نقش (RBAC) است. با این حال، اعمال ناسازگار دکوراتورها یا منطق RBAC در مسیرهای مختلف API یک آسیب‌پذیری امنیتی رایج است. اگر یک نقطه پایانی که باید به نقش‌ها یا مجوزهای خاصی محدود شود، به درستی محافظت نشود، می‌تواند منجر به دسترسی غیرمجاز به داده‌ها یا عملکرد شود. یک بررسی کامل برای اطمینان از اینکه هر نقطه پایانی حساس به درستی مجوزهای مورد نیاز را اعمال می‌کند، ضروری است.
...
- `backend/app/core/rbac.py:1-30` — `RoleChecker` — این فایل منطق RBAC را تعریف می‌کند.
  ```python
  ...
  ```
- `backend/app/api/routes/persons.py:10-40` — `create_person` — مثالی از مسیری که احتمالاً به RBAC نیاز دارد و باید به درستی اعمال شود.
  ```python
  from fastapi import APIRouter, Depends, HTTPException
  from backend.app.schemas.person import PersonCreate, PersonRead
  from backend.app.api.deps import get_db
  from backend.app.core.rbac import RoleChecker
  
  router = APIRouter()
  
  @router.post("/", response_model=PersonRead, dependencies=[Depends(RoleChecker(["admin", "editor"]))])
...
Python, FastAPI, OAuth2/JWT, RBAC سفارشی.

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `backend/app/api/deps.py` — جایی که وابستگی‌های RBAC ممکن است تزریق شوند.
- `backend/app/models/user.py` — نقش‌ها/مجوزهای کاربر را تعریف می‌کند.
- `backend/app/api/routes/risk.py` — مثالی از مسیری که احتمالاً به RBAC نیاز دارد.

## 🌐 نقشهٔ وابستگی‌ها
برای امنیت و یکپارچگی کل API بک‌اند حیاتی است.
...
- [ ] تمام نقاط پایانی API در صورت لزوم به صراحت توسط وابستگی‌های RBAC محافظت می‌شوند.
- [ ] کاربران با مجوزهای ناکافی هنگام تلاش برای دسترسی به منابع محافظت شده، پاسخ 403 Forbidden دریافت می‌کنند.
- [ ] کاربران با مجوزهای صحیح می‌توانند به منابع محافظت شده دسترسی پیدا کنند.
- [ ] تست‌های جدیدی برای تأیید RBAC برای نقاط پایانی کلیدی اضافه می‌شوند.
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. 1. تمام مسیرهای API در `backend/app/api/routes/` را برای شناسایی نقش‌ها/مجوزهای مورد نیاز برای هر نقطه پایانی (GET, POST, PUT, DELETE) ممیزی کنید. 2. تأیید کنید که `backend/app/core/rbac.py` دکوراتورها یا توابع تزریق وابستگی لازم را برای اعمال این مجوزها فراهم می‌کند. 3. این مکانیزم‌های RBAC را به طور سازگار در تمام عملیات مسیر FastAPI مربوطه اعمال کنید. 4. اضافه کردن تست‌های یکپارچه‌سازی به طور خاص برای RBAC را در نظر بگیرید تا اطمینان حاصل شود که کاربران غیرمجاز نمی‌توانند به منابع محافظت شده دسترسی پیدا کنند.

## 💡 نمونه‌های قبل/بعد
**مثال اعمال وابستگی RBAC به یک مسیر**

_قبل:_
```
@router.get("/sensitive_data")
async def get_sensitive_data(current_user: User = Depends(get_current_active_user)):
    # Manual check here
...
@router.get("/sensitive_data", dependencies=[Depends(RoleChecker(["admin"]))])
async def get_sensitive_data(current_user: User = Depends(get_current_active_user)):
    # Logic is now handled by the dependency
    return {"data": "sensitive"}
```
...
- `Manual testing with different user roles (e.g., admin, regular user) via curl or Postman.`

## ⚠️ ریسک‌ها و موارد احتیاط
محدود کردن بیش از حد دسترسی، از دست دادن موارد خاص، احتمال پیچیدگی منطق RBAC که نگهداری آن را دشوار می‌کند.
...
1. تمام نقاط پایانی API در صورت لزوم به صراحت توسط وابستگی‌های RBAC محافظت می‌شوند. _(verify: static)_
2. کاربران با مجوزهای ناکافی هنگام تلاش برای دسترسی به منابع محافظت شده، پاسخ 403 Forbidden دریافت می‌کنند. _(verify: api_response)_
3. کاربران با مجوزهای صحیح می‌توانند به منابع محافظت شده دسترسی پیدا کنند. _(verify: api_response)_
4. تست‌های جدیدی برای تأیید RBAC برای نقاط پایانی کلیدی اضافه می‌شوند. _(verify: backend_test)_

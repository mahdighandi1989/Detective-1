---
task_id: e95449d9-56e9-4d83-a787-75bc6f2f8bd1
title: '[منطق] Authorization Logic Residing in Pydantic Schemas'
type: logic_audit
priority: high
...
created_at: '2026-06-13T18:13:05.144210+00:00'
updated_at: '2026-06-13T20:19:45.581785+00:00'
---

# [منطق] Authorization Logic Residing in Pydantic Schemas
...
`backend/app/schemas/auth.py` (a Pydantic schema module) lists 'Boolean results from access checks' as an expected output and mentions using `ClassificationLevel` enums for access checks. Pydantic schemas are primarily intended for data validation and structuring. Embedding authorization logic directly within schema models violates the Single Responsibility Principle.

## 💥 پیامد (impact)
Placing authorization logic in schemas blurs the lines between data models and business logic, making the system harder to understand, maintain, and test. It can lead to inconsistent application of authorization across mutation paths and potentially subtle information leaks if the schema's internal 

## 🛠 پیشنهاد رفع اولیه
Move all authorization logic (e.g., checking roles, classification levels) out of `backend/app/schemas/auth.py` and into dedicated service modules, specifically `backend/app/core/security.py` or `backend/app/api/deps.py` (as FastAPI dependencies). Schemas should only define the structure and validation rules for data.
...
3. integration test برای pipeline `auth` بدون شکست عبور می‌کند _(verify: static)_
4. PR description توضیح می‌دهد چرا این تصمیم گرفته شد _(verify: static)_

## Task Steps

### Step 1: تحلیل و شناسایی منطق احراز هویت موجود در backend/app/schemas/auth.py
**Status:** `pending` (0%)
**Scope:** این مرحله شامل بررسی دقیق فایل `backend/app/schemas/auth.py` برای شناسایی تمام بخش‌هایی است که به منطق احراز هویت (authorization logic) مربوط می‌شوند. این شامل یافتن مواردی مانند 'Boolean results from access checks' و استفاده از `ClassificationLevel` enums برای بررسی دسترسی است. هدف صرفاً درک و مستندسازی وضعیت فعلی است، بدون انجام هیچ‌گونه تغییری در کد.
**Excerpt:**
```
backend/app/schemas/auth.py (a Pydantic schema module) lists 'Boolean results from access checks' as an expected output and mentions using `ClassificationLevel` enums for access checks. Pydantic schemas are primarily intended for data validation and structuring. Embedding authorization logic directly within schema models violates the Single Responsibility Principle.
```

### Step 2: پیاده‌سازی توابع اصلی احراز هویت در backend/app/core/security.py
**Status:** `pending` (0%)
**Scope:** این مرحله شامل ایجاد یا به‌روزرسانی توابع و کلاس‌های اختصاصی در `backend/app/core/security.py` است که منطق احراز هویت شناسایی‌شده در مرحله ۱ را در خود جای می‌دهند. این توابع باید مسئول 'checking roles' و 'classification levels' باشند و به صورت مستقل از فریم‌ورک FastAPI قابل استفاده باشند. هدف، متمرکز کردن منطق کسب‌وکار احراز هویت در یک ماژول اختصاصی است.
**Excerpt:**
```
Move all authorization logic (e.g., checking roles, classification levels) out of `backend/app/schemas/auth.py` and into dedicated service modules, specifically `backend/app/core/security.py` or `backend/app/api/deps.py` (as FastAPI dependencies). Schemas should only define the structure and validation rules for data.
```

### Step 3: ایجاد یا به‌روزرسانی FastAPI dependencies برای احراز هویت در backend/app/api/deps.py
**Status:** `pending` (0%)
**Scope:** این مرحله شامل توسعه یا به‌روزرسانی توابع dependency در `backend/app/api/deps.py` است که از توابع اصلی احراز هویت پیاده‌سازی‌شده در `backend/app/core/security.py` استفاده می‌کنند. این dependencies باید قابلیت تزریق به endpointهای FastAPI را داشته باشند و مسئول استخراج اطلاعات مورد نیاز از درخواست (مانند کاربر فعلی) و اعمال بررسی‌های احراز هویت باشند. در صورت عدم احراز هویت، باید استثناهای HTTP مناسب را پرتاب کنند.
**Excerpt:**
```
Move all authorization logic (e.g., checking roles, classification levels) out of `backend/app/schemas/auth.py` and into dedicated service modules, specifically `backend/app/core/security.py` or `backend/app/api/deps.py` (as FastAPI dependencies). Schemas should only define the structure and validation rules for data.
```

### Step 4: حذف منطق احراز هویت از backend/app/schemas/auth.py
**Status:** `pending` (0%)
**Scope:** این مرحله شامل اصلاح فایل `backend/app/schemas/auth.py` برای حذف هرگونه کد مرتبط با منطق احراز هویت است. این شامل حذف 'Boolean results from access checks' و هرگونه استفاده مستقیم از `ClassificationLevel` enums برای کنترل دسترسی است. پس از این مرحله، `backend/app/schemas/auth.py` باید صرفاً شامل تعریف ساختار و قوانین اعتبارسنجی داده‌ها باشد.
**Excerpt:**
```
Move all authorization logic (e.g., checking roles, classification levels) out of `backend/app/schemas/auth.py` and into dedicated service modules, specifically `backend/app/core/security.py` or `backend/app/api/deps.py` (as FastAPI dependencies). Schemas should only define the structure and validation rules for data.
```

### Step 5: به‌روزرسانی کامپوننت‌های برنامه برای استفاده از مکانیزم‌های جدید احراز هویت
**Status:** `pending` (0%)
**Scope:** این مرحله شامل شناسایی و به‌روزرسانی تمام بخش‌های برنامه (مانند `mutation paths` یا endpointهای API) است که قبلاً به منطق احراز هویت جاسازی‌شده در `backend/app/schemas/auth.py` وابسته بودند. این کامپوننت‌ها باید اکنون برای اعمال بررسی‌های احراز هویت، از FastAPI dependencies جدید تعریف‌شده در `backend/app/api/deps.py` یا مستقیماً از توابع `backend/app/core/security.py` استفاده کنند. هدف، اطمینان از اعمال سازگار و صحیح احراز هویت در سراسر برنامه است.
**Excerpt:**
```
Placing authorization logic in schemas blurs the lines between data models and business logic, making the system harder to understand, maintain, and test. It can lead to inconsistent application of authorization across mutation paths and potentially subtle information leaks if the schema's internal
```

### Step 6: افزودن یا به‌روزرسانی تست‌ها برای منطق احراز هویت refactor شده
**Status:** `pending` (0%)
**Scope:** این مرحله شامل ایجاد تست‌های واحد (unit test) جدید برای توابع احراز هویت در `backend/app/core/security.py` و تست‌های یکپارچه‌سازی (integration test) برای FastAPI dependencies در `backend/app/api/deps.py` است. همچنین، هر تست موجود مرتبط با منطق احراز هویت در `backend/app/schemas/auth.py` باید به‌روزرسانی یا حذف شود. هدف، اطمینان از صحت و پایداری منطق احراز هویت پس از refactor است.
**Excerpt:**
```
Placing authorization logic in schemas blurs the lines between data models and business logic, making the system harder to understand, maintain, and test. It can lead to inconsistent application of authorization across mutation paths and potentially subtle information leaks if the schema's internal... coherence issue یعنی دو بخش کد فرض‌های ناسازگار دارند — معمولاً نشانه‌ی refactor ناتمام یا feature flag rot است. این کلاس bug ها در test معمولی پیدا نمی‌شوند چون unit test ها در silo اجرا می‌شوند.
```

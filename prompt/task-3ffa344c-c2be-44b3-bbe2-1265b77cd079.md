---
task_id: 3ffa344c-c2be-44b3-bbe2-1265b77cd079
title: '[منطق] Ambiguous Ownership of Authentication and RBAC Logic'
type: logic_audit
priority: critical
...
created_at: '2026-06-13T18:05:10.293467+00:00'
updated_at: '2026-06-13T18:05:10.293470+00:00'
---

# [منطق] Ambiguous Ownership of Authentication and RBAC Logic
...
Both `backend/app/api/deps.py` and `backend/app/core/security.py` claim responsibility for 'FastAPI authentication dependencies' and 'RBAC'. `deps.py` lists `get_current_user` and `get_current_active_user` as outputs, while `security.py` lists 'FastAPI authentication dependencies' and 'Authenticated user objects' as outputs, and 'RBAC with classification' as its purpose. This creates an unclear division of labor regarding where the core logic for JWT validation, user resolution, and role-based a

## 💥 پیامد (impact)
This ambiguity leads to confusion for developers, potentially resulting in inconsistent implementations, duplicated code, and increased risk of security vulnerabilities due to scattered or incomplete authorization logic. It also complicates auditing and testing of critical security components.

## 🛠 پیشنهاد رفع اولیه
Clearly define responsibilities: `backend/app/core/security.py` should contain the *core logic* (e.g., password hashing, JWT creation/validation, fundamental RBAC functions like `check_user_roles`). `backend/app/api/deps.py` should then *wrap* or *use* these core functions to create FastAPI `Depends` functions (e.g., `get_current_user`, `get_current_active_user`, `role_required_dependency`) that a
...
### Step 1: Consolidate Core Authentication and RBAC Logic in `backend/app/core/security.py`
**Status:** `pending` (0%)
**Scope:** این مرحله شامل بازبینی و بازآرایی فایل `backend/app/core/security.py` است تا اطمینان حاصل شود که تمامی عملیات‌های امنیتی بنیادی و هسته‌ای به صورت انحصاری در آن قرار دارند. این مسئولیت‌ها شامل منطق هش کردن رمز عبور، ایجاد و اعتبارسنجی توکن‌های JWT، و توابع اصلی کنترل دسترسی مبتنی بر نقش (RBAC) مانند `check_user_roles` می‌شود. هرگونه منطق هسته‌ای مرتبط با این مسئولیت‌ها که در حال حاضر خارج از این فایل قرار دارد، باید به `backend/app/core/security.py` منتقل شود. این مرحله شامل هیچ‌گونه تغییر در `backend/app/api/deps.py` یا توابع `FastAPI Depends` نیست.
**Excerpt:**
```
Clearly define responsibilities: `backend/app/core/security.py` should contain the *core logic* (e.g., password hashing, JWT creation/validation, fundamental RBAC functions like `check_user_roles`).
```

### Step 2: Update `backend/app/api/deps.py` to Utilize Core Security Functions from `security.py`
**Status:** `pending` (0%)
**Scope:** این مرحله بر اصلاح فایل `backend/app/api/deps.py` متمرکز است تا اطمینان حاصل شود که این فایل به درستی توابع احراز هویت و RBAC هسته‌ای را که در `backend/app/core/security.py` تجمیع شده‌اند، فراخوانی و استفاده می‌کند. به طور خاص، توابع `FastAPI Depends` مانند `get_current_user`، `get_current_active_user` و `role_required_dependency` باید به گونه‌ای به‌روزرسانی شوند که منطق هسته‌ای مناسب را از `backend/app/core/security.py` فراخوانی کنند، نه اینکه خودشان آن منطق را پیاده‌سازی یا تکرار کنند. این کار `deps.py` را به عنوان لایه تزریق وابستگی FastAPI تثبیت می‌کند که بر `security.py` برای مکانیزم‌های امنیتی زیربنایی تکیه دارد.
**Excerpt:**
```
`backend/app/api/deps.py` should then *wrap* or *use* these core functions to create FastAPI `Depends` functions (e.g., `get_current_user`, `get_current_active_user`, `role_required_dependency`) that a
```

---
task_id: 461be010-12b9-4153-a3bc-b110f3710c98
title: "دکمه‌ی UI بدون handler: setShowForm((s) => !s)}\n              className=\"rounded-md bg-blue-600 px-4 py-"
type: bug_fix
priority: medium
...
created_at: '2026-06-13T18:15:54.034007+00:00'
updated_at: '2026-06-13T20:20:33.958741+00:00'
target_files:
- frontend/src/app/encyclopedia/page.tsx
---

# دکمه‌ی UI بدون handler: setShowForm((s) => !s)}
              className="rounded-md bg-blue-600 px-4 py-

## Raw Idea

## 📋 شرح
یک دکمه/کنترل UI در فایل `frontend/src/app/encyclopedia/page.tsx` پیدا شد که هیچ event handler معنادار به آن متصل نیست (onClick، onChange، form submit، router push، یا API call شناسایی نشد).

## 🔍 جزئیات
- label/متن دکمه: `setShowForm((s) => !s)}
              className="rounded-md bg-blue-600 px-4 py-`
- فایل: `frontend/src/app/encyclopedia/page.tsx`
...
---
[scan #2 at 2026-06-13T18:15:54.101466+00:00]
## 📋 شرح
یک دکمه/کنترل UI در فایل `frontend/src/app/encyclopedia/page.tsx` پیدا شد که هیچ event handler معنادار به آن متصل نیست (onClick، onChange، form submit، router push، یا API call شناسایی نشد).
...
- label/متن دکمه: `{
                  ...
                  setSea
---
[scan #3 at 2026-06-13T18:15:54.136248+00:00]
## 📋 شرح
یک دکمه/کنترل UI در فایل `frontend/src/app/encyclopedia/page.tsx` پیدا شد که هیچ event handler معنادار به آن متصل نیست (onClick، onChange، form submit، router push، یا API call شناسایی نشد).
...
- label/متن دکمه: `{
                  ...
                  se
---
[scan #4 at 2026-06-13T18:15:54.168791+00:00]
## 📋 شرح
یک دکمه/کنترل UI در فایل `frontend/src/app/encyclopedia/page.tsx` پیدا شد که هیچ event handler معنادار به آن متصل نیست (onClick، onChange، form submit، router push، یا API call شناسایی نشد).
...
- label/متن دکمه: `{
                      ...
                      h
---
[scan #5 at 2026-06-13T18:15:54.201844+00:00]
## 📋 شرح
یک دکمه/کنترل UI در فایل `frontend/src/app/encyclopedia/page.tsx` پیدا شد که هیچ event handler معنادار به آن متصل نیست (onClick، onChange، form submit، router push، یا API call شناسایی نشد).
...
- [ ] git blame مشخص می‌کند چرا این دکمه `setShowForm((s) => !s)}
              className="rounded-md bg-blue-600 px-4 py-` فاقد handler است
...
1. git blame مشخص می‌کند چرا این دکمه `setShowForm((s) => !s)}
              className="rounded-md bg-blue-600 px-4 py-` فاقد handler است _(verify: static)_

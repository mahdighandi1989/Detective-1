---
task_id: f08d4a78-7cf1-4c5b-806f-77f68546f75e
title: '[منطق] Inconsistent Risk Classification Terminology'
type: logic_audit
priority: high
...
created_at: '2026-06-13T18:11:43.062834+00:00'
updated_at: '2026-06-13T20:19:35.407349+00:00'
---

# [منطق] Inconsistent Risk Classification Terminology
...
The concept of risk classification is referred to as 'RiskCategory' (enum) in `backend/app/schemas/person.py` and 'RiskLevel' (enum) in `backend/app/models/risk_assessment.py` and `backend/app/schemas/graph.py`. Additionally, `backend/app/services/risk_engine.py` mentions 'risk level' and 'risk categories'. While likely representing the same underlying concept, this inconsistent naming across core components can lead to confusion, mapping errors, and make the codebase harder to maintain and unde

## 💥 پیامد (impact)
Potential for data type mismatches, incorrect filtering/sorting, and errors when passing risk classification data between components (e.g., from `risk_engine` output to `risk_assessment` model, or from `person` schemas to `graph` schemas). This directly impacts the 'signal with risk model' coherence

## 🛠 پیشنهاد رفع اولیه
Standardize the naming convention for risk classification (e.g., consistently use `RiskLevel` or `RiskCategory` as an Enum) across all relevant models, schemas, and service descriptions.
...
3. integration test برای pipeline `business` بدون شکست عبور می‌کند _(verify: static)_
4. PR description توضیح می‌دهد چرا این تصمیم گرفته شد _(verify: static)_

## Task Steps

### Step 1: تایید استاندارد نام‌گذاری برای طبقه‌بندی ریسک
**Status:** `pending` (0%)
**Scope:** این مرحله اولیه شامل تایید نام دقیق enum است که قرار است به طور یکپارچه در سراسر کدبیس برای طبقه‌بندی ریسک استفاده شود. با توجه به استفاده موجود، `RiskLevel` به عنوان استاندارد پذیرفته خواهد شد. این مرحله شامل هیچ تغییر کدی نیست، بلکه استاندارد نام‌گذاری هدف را برای مراحل بازسازی بعدی تعیین می‌کند.
**Excerpt:**
```
Standardize the naming convention for risk classification (e.g., consistently use `RiskLevel` or `RiskCategory` as an Enum) across all relevant models, schemas, and service descriptions.
```

### Step 2: بازسازی `backend/app/schemas/person.py` برای استفاده از `RiskLevel`
**Status:** `pending` (0%)
**Scope:** این مرحله بر روی اصلاح فایل `backend/app/schemas/person.py` تمرکز دارد. enum `RiskCategory` به `RiskLevel` تغییر نام داده خواهد شد و تمام ارجاعات داخلی در این فایل برای استفاده از enum جدید `RiskLevel` به‌روزرسانی می‌شوند. این کار تضمین می‌کند که شمای `person` با استاندارد انتخاب شده همسو می‌شود.
**Excerpt:**
```
The concept of risk classification is referred to as 'RiskCategory' (enum) in `backend/app/schemas/person.py`... Standardize the naming convention for risk classification (e.g., consistently use `RiskLevel` or `RiskCategory` as an Enum) across all relevant models, schemas, and service descriptions.
```

### Step 3: همسوسازی `backend/app/models/risk_assessment.py` با استاندارد `RiskLevel`
**Status:** `pending` (0%)
**Scope:** این مرحله شامل بررسی فایل `backend/app/models/risk_assessment.py` است. در حالی که این فایل قبلاً از `RiskLevel` استفاده می‌کند، وظیفه این است که اطمینان حاصل شود تعریف و استفاده آن کاملاً با enum استاندارد `RiskLevel` (به عنوان مثال، مقادیر، ترتیب، نوع) سازگار است. هرگونه ناهماهنگی یا فرصت برای همسوسازی برطرف خواهد شد.
**Excerpt:**
```
The concept of risk classification is referred to as ... 'RiskLevel' (enum) in `backend/app/models/risk_assessment.py`... Standardize the naming convention for risk classification (e.g., consistently use `RiskLevel` or `RiskCategory` as an Enum) across all relevant models, schemas, and service descriptions.
```

### Step 4: همسوسازی `backend/app/schemas/graph.py` با استاندارد `RiskLevel`
**Status:** `pending` (0%)
**Scope:** این مرحله بر روی `backend/app/schemas/graph.py` تمرکز دارد. مشابه مدل `risk_assessment`، این فایل قبلاً از `RiskLevel` استفاده می‌کند. وظیفه این است که اطمینان حاصل شود تعریف و استفاده آن کاملاً با enum استاندارد `RiskLevel` سازگار است و هرگونه ناهماهنگی احتمالی در مقادیر یا ساختار را برطرف می‌کند.
**Excerpt:**
```
The concept of risk classification is referred to as ... 'RiskLevel' (enum) in ... `backend/app/schemas/graph.py`. Additionally, `backend/app/services/risk_engine.py` mentions 'risk level' and 'risk categories'. While likely representing the same underlying concept, this inconsistent naming across core components can lead to confusion, mapping errors, and make the codebase harder to maintain and unde... Standardize the naming convention for risk classification (e.g., consistently use `RiskLevel` or `RiskCategory` as an Enum) across all relevant models, schemas, and service descriptions.
```

### Step 5: به‌روزرسانی `backend/app/services/risk_engine.py` برای سازگاری `RiskLevel`
**Status:** `pending` (0%)
**Scope:** این مرحله شامل اصلاح فایل `backend/app/services/risk_engine.py` است. تمام ارجاعات به 'risk level' و 'risk categories' در منطق این سرویس برای ارجاع یکپارچه به enum `RiskLevel` به‌روزرسانی خواهند شد. این شامل به‌روزرسانی نام متغیرها، پارامترهای تابع، راهنمایی‌های نوع (type hints) و هرگونه رشته متنی است که مفهوم طبقه‌بندی ریسک را نشان می‌دهد.
**Excerpt:**
```
Additionally, `backend/app/services/risk_engine.py` mentions 'risk level' and 'risk categories'. While likely representing the same underlying concept, this inconsistent naming across core components can lead to confusion, mapping errors, and make the codebase harder to maintain and unde... Standardize the naming convention for risk classification (e.g., consistently use `RiskLevel` or `RiskCategory` as an Enum) across all relevant models, schemas, and service descriptions.
```

### Step 6: به‌روزرسانی جریان داده بین‌کامپوننتی و راهنمایی‌های نوع برای `RiskLevel`
**Status:** `pending` (0%)
**Scope:** این مرحله حیاتی شامل شناسایی و به‌روزرسانی تمام نقاطی است که داده‌های طبقه‌بندی ریسک بین کامپوننت‌های تحت تأثیر (`risk_engine`، مدل `risk_assessment`، شمای `person`، شمای `graph`) منتقل می‌شوند. این شامل به‌روزرسانی امضای توابع، بارهای API، اشیاء انتقال داده (DTOs) و راهنمایی‌های نوع برای اطمینان از استفاده یکپارچه از enum `RiskLevel` و جلوگیری از عدم تطابق نوع داده است.
**Excerpt:**
```
Potential for data type mismatches, incorrect filtering/sorting, and errors when passing risk classification data between components (e.g., from `risk_engine` output to `risk_assessment` model, or from `person` schemas to `graph` schemas). This directly impacts the 'signal with risk model' coherence.
```

### Step 7: به‌روزرسانی تست‌های واحد برای تغییرات `RiskLevel`
**Status:** `pending` (0%)
**Scope:** این مرحله شامل بررسی و به‌روزرسانی تست‌های واحد موجود برای `backend/app/schemas/person.py`، `backend/app/models/risk_assessment.py`، `backend/app/schemas/graph.py` و `backend/app/services/risk_engine.py` است. تست‌هایی که به `RiskCategory` یا نمایش‌های رشته‌ای خاصی از سطوح ریسک متکی بودند، برای استفاده از enum جدید `RiskLevel` به‌روزرسانی خواهند شد. ممکن است تست‌های واحد جدیدی برای پوشش خاص استفاده صحیح و رفتار enum `RiskLevel` در هر کامپوننت اضافه شود.
**Excerpt:**
```
coherence issue یعنی دو بخش کد فرض‌های ناسازگار دارند — معمولاً نشانه‌ی refactor ناتمام یا feature flag rot است. این کلاس bug ها در test معمولی پیدا نمی‌شوند چون unit test ها در silo اجرا می‌شوند.
```

### Step 8: پیاده‌سازی/به‌روزرسانی تست‌های یکپارچه‌سازی برای انسجام `RiskLevel`
**Status:** `pending` (0%)
**Scope:** این مرحله بر روی ایجاد یا به‌روزرسانی تست‌های یکپارچه‌سازی تمرکز دارد تا به طور خاص جریان صحیح و تفسیر داده‌های `RiskLevel` را در مرزهای `risk_engine`، مدل `risk_assessment`، شمای `person` و شمای `graph` تأیید کند. این تست‌ها سناریوهای تبادل داده در دنیای واقعی را شبیه‌سازی می‌کنند تا 'مسائل انسجام' را که تست‌های واحد ممکن است از دست بدهند، شناسایی کنند.
**Excerpt:**
```
Potential for data type mismatches, incorrect filtering/sorting, and errors when passing risk classification data between components (e.g., from `risk_engine` output to `risk_assessment` model, or from `person` schemas to `graph` schemas). This directly impacts the 'signal with risk model' coherence... این کلاس bug ها در test معمولی پیدا نمی‌شوند چون unit test ها در silo اجرا می‌شوند.
```

### Step 9: به‌روزرسانی مستندات و کامنت‌های کد برای استاندارد `RiskLevel`
**Status:** `pending` (0%)
**Scope:** این مرحله نهایی شامل به‌روزرسانی هرگونه مستندات داخلی مرتبط، فایل‌های README، مشخصات API و کامنت‌های کد درون‌خطی است که به `RiskCategory`، `risk categories` یا اصطلاحات قدیمی `risk level` اشاره می‌کنند. تمام این ارجاعات برای استفاده یکپارچه از `RiskLevel` به‌روزرسانی خواهند شد تا استاندارد جدید را منعکس کرده و قابلیت نگهداری و درک کدبیس در آینده را تضمین کنند.
**Excerpt:**
```
Standardize the naming convention for risk classification (e.g., consistently use `RiskLevel` or `RiskCategory` as an Enum) across all relevant models, schemas, and service descriptions.
```

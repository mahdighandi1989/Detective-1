---
task_id: 40bc3f67-8c9b-4629-b7c8-ea7e48f6ad29
title: جلوگیری از افشای کلیدهای API حساس و اسرار در باندل فرانت‌اند
type: security
priority: critical
...
created_at: '2026-06-06T17:37:04.885413+00:00'
updated_at: '2026-06-06T17:48:03.701042+00:00'
target_files:
- frontend/next.config.js
- frontend/src/lib/api.ts
---

# جلوگیری از افشای کلیدهای API حساس و اسرار در باندل فرانت‌اند

## Raw Idea

برنامه‌های فرانت‌اند، به ویژه آن‌هایی که با فریم‌ورک‌هایی مانند Next.js ساخته شده‌اند، در صورت عدم پیکربندی صحیح، می‌توانند به طور ناخواسته متغیرهای محیطی یا کلیدهای API را در کد سمت کلاینت بسته‌بندی کنند. این می‌تواند منجر به افشای اعتبارنامه‌های حساس بک‌اند (مانند کلیدهای API LLM، کلیدهای سرویس OSINT) به هر کاربری که کد منبع مرورگر را بررسی می‌کند، شود. Next.js نیاز دارد که متغیرهای محیطی که برای مرورگر در نظر گرفته شده‌اند، با پیشوند `NEXT_PUBLIC_` باشند.
...
- `frontend/next.config.js` — `module.exports` — این فایل نحوه افشای متغیرهای محیطی به فرانت‌اند را پیکربندی می‌کند. محتوای واقعی در این فاز ارائه نشده است.
- `frontend/src/lib/api.ts` — `fetcher` — این فایل احتمالاً تماس‌های API را برقرار می‌کند و ممکن است به متغیرهای محیطی دسترسی داشته باشد. محتوای واقعی در این فاز ارائه نشده است.
...
Next.js, Environment Variables, Client-Side Bundling

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `backend/app/core/config.py` — اسرار بک‌اند را تعریف می‌کند که نباید افشا شوند.
- `backend/app/services/llm_adapter.py` — از کلیدهای API LLM استفاده می‌کند.
- `backend/app/services/osint_agent.py` — از کلیدهای API سرویس OSINT استفاده می‌کند.

## 🌐 نقشهٔ وابستگی‌ها
بر امنیت ادغام سرویس‌های خارجی و زیرساخت بک‌اند تأثیر می‌گذارد.
...
- [ ] هیچ کلید API حساس بک‌اند (مانند LLM، سرویس‌های OSINT) در باندل جاوااسکریپت سمت کلاینت یافت نمی‌شود.
- [ ] تمام متغیرهای محیطی که در فرانت‌اند قابل دسترسی هستند، با پیشوند `NEXT_PUBLIC_` هستند.
- [ ] فرانت‌اند با سرویس‌های حساس بک‌اند فقط از طریق API بک‌اند خود پروژه ارتباط برقرار می‌کند.
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. 1. فایل `frontend/next.config.js` و هر فایلی در `frontend/src/lib/api.ts` یا `frontend/src/app/**` که به متغیرهای محیطی دسترسی دارند را بررسی کنید. 2. اطمینان حاصل کنید که فقط متغیرهای دارای پیشوند `NEXT_PUBLIC_` در سمت کلاینت قابل دسترسی هستند. 3. تأیید کنید که هیچ کلید API یا اسرار خاص بک‌اند به طور مستقیم در codebase فرانت‌اند وارد یا hardcode نشده است. اگر فرانت‌اند نیاز به تعامل با سرویسی دارد که به یک secret نیاز دارد، باید این کار را از طریق API بک‌اند انجام دهد که سپس از اعتبارنامه‌های امن خود استفاده می‌کند.

## 💡 نمونه‌های قبل/بعد
**دسترسی ناامن سمت کلاینت**
...
- `Inspect the generated JavaScript bundles in `frontend/.next/static/chunks/` for sensitive strings.`

## ⚠️ ریسک‌ها و موارد احتیاط
شکست تصادفی عملکرد فرانت‌اند در صورت تغییر نادرست دسترسی به متغیرهای محیطی.
...
1. هیچ کلید API حساس بک‌اند (مانند LLM، سرویس‌های OSINT) در باندل جاوااسکریپت سمت کلاینت یافت نمی‌شود. _(verify: static)_
2. تمام متغیرهای محیطی که در فرانت‌اند قابل دسترسی هستند، با پیشوند `NEXT_PUBLIC_` هستند. _(verify: static)_
3. فرانت‌اند با سرویس‌های حساس بک‌اند فقط از طریق API بک‌اند خود پروژه ارتباط برقرار می‌کند. _(verify: static)_

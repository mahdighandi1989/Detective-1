---
task_id: 89d0d2e0-07a8-42d8-96a7-fefb7b3e6572
title: عدم تطابق فیلدهای رابط `Article` فرانت‌اند و مدل `Article` بک‌اند (به‌ویژه منابع)
type: bug
priority: high
...
created_at: '2026-06-13T18:06:36.314644+00:00'
updated_at: '2026-06-13T20:17:43.448495+00:00'
target_files:
...
# عدم تطابق فیلدهای رابط `Article` فرانت‌اند و مدل `Article` بک‌اند (به‌ویژه منابع)

## Raw Idea

رابط `Article` در `frontend/src/app/encyclopedia/page.tsx` (خط 16-29) شامل `sources: Source[]` (آرایه‌ای از منابع) است، در حالی که مدل `Article` در بک‌اند (`backend/app/models/article.py`) دارای `source_id` (کلید خارجی منفرد) و یک رابطهٔ `source` (یک به چند از مقاله به منبع) است. این بدان معناست که فرانت‌اند انتظار لیستی از منابع را دارد، اما بک‌اند فقط به یک منبع لینک می‌دهد. همچنین، فرم ایجاد مقاله در فرانت‌اند (`EMPTY_FORM` در خط 97) دارای `sourceUrls` (رشتهٔ چندگانه) است که در `handleCreate` (خط 181) به `source_urls` (آرایه) تبدیل می‌شود، اما API بک‌اند (`ArticleIngestRequest` در `backend/app/api/routes/encyclopedia.py`:86) فقط `source_url` (مفرد) را می‌پذیرد. این یک عدم تطابق حیاتی در نحوهٔ مدیریت منابع است.
...
- `frontend/src/app/encyclopedia/page.tsx:25` — `Article` — تعریف `sources` به عنوان آرایه در رابط Article فرانت‌اند
  ```tsx
  sources: Source[];
  ```
- `frontend/src/app/encyclopedia/page.tsx:104` — `EMPTY_FORM` — فیلد `sourceUrls` در فرم ایجاد مقاله
  ```tsx
  sourceUrls: '',
  ```
- `frontend/src/app/encyclopedia/page.tsx:181-184` — `handleCreate` — پردازش `sourceUrls` به عنوان آرایه برای payload
  ```tsx
  source_urls: form.sourceUrls
            ...
            .filter(Boolean),
  ```
- `backend/app/models/article.py:155-162` — `Article` — تعریف `source_id` (مفرد) و رابطهٔ `source` در مدل Article بک‌اند
  ```python
  ...
  ```
- `backend/app/api/routes/encyclopedia.py:103` — `ArticleIngestRequest` — فیلد `source_url` (مفرد) در درخواست ingest API
  ```python
...
FastAPI, SQLAlchemy ORM, Next.js React, TypeScript

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `backend/app/models/source.py` (سطر 76) — تعریف مدل Source و جداول ارتباطی
- `backend/app/schemas/article.py` (سطر -1) — اسکیماهای Pydantic برای Article (احتمالاً نیاز به به‌روزرسانی)
...
## 🌐 نقشهٔ وابستگی‌ها
این مشکل بر نحوهٔ ذخیره و نمایش منابع برای مقالات دانشنامه تأثیر می‌گذارد و نیاز به هماهنگی بین مدل‌های دیتابیس، API schemas و رابط کاربری فرانت‌اند دارد.
...
- [ ] رابط `Article` در فرانت‌اند یک فیلد `source` (مفرد) یا `source_id` را انتظار دارد.
- [ ] فرم ایجاد مقاله در فرانت‌اند یک فیلد `sourceUrl` (مفرد) را برای ارسال به API دارد.
- [ ] مقالات جدید با یک منبع واحد به‌درستی در بک‌اند ذخیره می‌شوند و در فرانت‌اند نمایش داده می‌شوند.
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. رابطهٔ `Article` با `Source` را در بک‌اند استانداردسازی کنید. اگر رابطه یک به چند است (یک مقاله یک منبع دارد)، فرانت‌اند را به‌روز کنید تا `source?: Source` (مفرد) را در رابط `Article` انتظار داشته باشد و فرم ایجاد مقاله را برای ارسال یک `source_url` واحد اصلاح کنید. اگر رابطه چند به چند است (یک مقاله چندین منبع دارد)، مدل `Article` در بک‌اند را به‌روز کنید تا از `article_source_association_table` استفاده کند و API را برای پذیرش `source_ids` (آرایه‌ای از UUIDها) یا `source_urls` (آرایه‌ای از رشته‌ها) اصلاح کنید. با توجه به `source_id` در مدل `Article`، فرض بر این است که رابطه فعلی یک به چند است و فرانت‌اند باید با آن هماهنگ شود.

## 💡 نمونه‌های قبل/بعد
**قبل (frontend/src/app/encyclopedia/page.tsx)**
...
interface Article {
  // ...
  source?: Source; // یا source_id?: string;
}
```
...
_قبل:_
```
source_urls: form.sourceUrls.split('\n')...
```
...
## 🧪 دستورات اعتبارسنجی
- `مقاله جدیدی را با یک URL منبع در UI ایجاد کنید و صحت ذخیره و نمایش آن را بررسی کنید.`
- `بررسی پاسخ API از `/api/encyclopedia/articles` برای ساختار فیلد منبع.`

## ⚠️ ریسک‌ها و موارد احتیاط
عدم تطابق داده‌ها و از دست رفتن اطلاعات منبع در صورت عدم اصلاح.
...
1. رابط `Article` در فرانت‌اند یک فیلد `source` (مفرد) یا `source_id` را انتظار دارد. _(verify: static)_
2. فرم ایجاد مقاله در فرانت‌اند یک فیلد `sourceUrl` (مفرد) را برای ارسال به API دارد. _(verify: static)_
3. مقالات جدید با یک منبع واحد به‌درستی در بک‌اند ذخیره می‌شوند و در فرانت‌اند نمایش داده می‌شوند. _(verify: static)_

## Task Steps

### Step 1: به‌روزرسانی مدل Article در بک‌اند برای پشتیبانی از چندین منبع
**Status:** `pending` (0%)
**Scope:** این مرحله شامل اصلاح مدل `Article` در `backend/app/models/article.py` است تا به‌جای `source_id` منفرد، بتواند چندین منبع را مدیریت کند. این تغییر ممکن است شامل حذف `source_id` و ایجاد یک رابطهٔ چند به چند (many-to-many) با مدل `Source` یا یک مدل واسط باشد. هدف این است که مدل داده‌ای بک‌اند با انتظار فرانت‌اند برای `sources: Source[]` هم‌راستا شود. این مرحله شامل تغییرات API یا UI نمی‌شود.
**Excerpt:**
```
رابط `Article` در `frontend/src/app/encyclopedia/page.tsx` (خط 16-29) شامل `sources: Source[]` (آرایه‌ای از منابع) است، در حالی که مدل `Article` در بک‌اند (`backend/app/models/article.py`) دارای `source_id` (کلید خارجی منفرد) و یک رابطهٔ `source` (یک به چند از مقاله به منبع) است. این بدان معناست که فرانت‌اند انتظار لیستی از منابع را دارد، اما بک‌اند فقط به یک منبع لینک می‌دهد.
```

### Step 2: به‌روزرسانی API بک‌اند برای دریافت چندین URL منبع در زمان ایجاد مقاله
**Status:** `pending` (0%)
**Scope:** این مرحله بر اصلاح `ArticleIngestRequest` در `backend/app/api/routes/encyclopedia.py` تمرکز دارد. `ArticleIngestRequest` باید به‌جای `source_url` (مفرد)، یک آرایه از رشته‌ها به نام `source_urls` را بپذیرد. این تغییر، عدم تطابق بین داده‌های ارسالی از فرانت‌اند و انتظار API بک‌اند را برطرف می‌کند و امکان ایجاد مقالات با چندین منبع را فراهم می‌آورد. منطق پردازش این `source_urls` در بک‌اند نیز باید پیاده‌سازی شود تا منابع جدید ایجاد یا به مقاله موجود مرتبط شوند.
**Excerpt:**
```
همچنین، فرم ایجاد مقاله در فرانت‌اند (`EMPTY_FORM` در خط 97) دارای `sourceUrls` (رشتهٔ چندگانه) است که در `handleCreate` (خط 181) به `source_urls` (آرایه) تبدیل می‌شود، اما API بک‌اند (`ArticleIngestRequest` در `backend/app/api/routes/encyclopedia.py`:86) فقط `source_url` (مفرد) را می‌پذیرد. این یک عدم تطابق حیاتی در نحوهٔ مدیریت منابع است.
```

### Step 3: به‌روزرسانی API بک‌اند برای بازیابی چندین منبع مقاله
**Status:** `pending` (0%)
**Scope:** این مرحله شامل اصلاح endpointهای بازیابی مقاله در `backend/app/api/routes/encyclopedia.py` است. پس از اینکه مدل `Article` در بک‌اند برای پشتیبانی از چندین منبع به‌روزرسانی شد (مرحله ۱)، این API باید به‌گونه‌ای تغییر کند که هنگام بازیابی یک مقاله، تمام منابع مرتبط با آن را به‌صورت یک آرایه از اشیاء `Source` (یا فرمت معادل) برگرداند. این کار تضمین می‌کند که فرانت‌اند داده‌های مورد انتظار خود (`sources: Source[]`) را دریافت می‌کند.
**Excerpt:**
```
رابط `Article` در `frontend/src/app/encyclopedia/page.tsx` (خط 16-29) شامل `sources: Source[]` (آرایه‌ای از منابع) است، در حالی که مدل `Article` در بک‌اند (`backend/app/models/article.py`) دارای `source_id` (کلید خارجی منفرد) و یک رابطهٔ `source` (یک به چند از مقاله به منبع) است. این بدان معناست که فرانت‌اند انتظار لیستی از منابع را دارد، اما بک‌اند فقط به یک منبع لینک می‌دهد.
```

### Step 4: تنظیم منطق `handleCreate` در فرانت‌اند برای API جدید بک‌اند
**Status:** `pending` (0%)
**Scope:** این مرحله شامل بررسی و تنظیم تابع `handleCreate` در `frontend/src/app/encyclopedia/page.tsx` است. اگرچه فرانت‌اند در حال حاضر `sourceUrls` را به `source_urls` (آرایه) تبدیل می‌کند، اما باید اطمینان حاصل شود که ساختار نهایی payload ارسالی به API بک‌اند (که در مرحله ۲ به‌روزرسانی شد) دقیقاً با انتظار `ArticleIngestRequest` جدید مطابقت دارد. این ممکن است شامل تغییرات جزئی در نام فیلد یا ساختار شیء ارسالی باشد.
**Excerpt:**
```
فرم ایجاد مقاله در فرانت‌اند (`EMPTY_FORM` در خط 97) دارای `sourceUrls` (رشتهٔ چندگانه) است که در `handleCreate` (خط 181) به `source_urls` (آرایه) تبدیل می‌شود، اما API بک‌اند (`ArticleIngestRequest` در `backend/app/api/routes/encyclopedia.py`:86) فقط `source_url` (مفرد) را می‌پذیرد.
```

### Step 5: به‌روزرسانی تعریف رابط Article در فرانت‌اند برای چندین منبع
**Status:** `pending` (0%)
**Scope:** این مرحله شامل بررسی و تأیید تعریف رابط `Article` در `frontend/src/app/encyclopedia/page.tsx` (خطوط 16-29) است. باید اطمینان حاصل شود که این رابط به‌درستی `sources: Source[]` را تعریف می‌کند و ساختار `Source` نیز با داده‌هایی که از API بازیابی مقاله در بک‌اند (مرحله ۳) دریافت می‌شود، مطابقت دارد. این یک مرحلهٔ تایپ‌اسکریپت برای تضمین سازگاری داده‌ها در سمت فرانت‌اند است.
**Excerpt:**
```
رابط `Article` در `frontend/src/app/encyclopedia/page.tsx` (خط 16-29) شامل `sources: Source[]` (آرایه‌ای از منابع) است
```

### Step 6: به‌روزرسانی کامپوننت‌های UI فرانت‌اند برای نمایش چندین منبع مقاله
**Status:** `pending` (0%)
**Scope:** این مرحله شامل اصلاح کامپوننت‌های رابط کاربری در `frontend/src/app/encyclopedia/page.tsx` یا هر کامپوننت مرتبط دیگری است که جزئیات مقاله را نمایش می‌دهد. این کامپوننت‌ها باید به‌جای نمایش یک منبع منفرد، قابلیت تکرار و نمایش تمام آیتم‌های موجود در آرایه `sources: Source[]` را داشته باشند. این شامل طراحی UI برای نمایش لیست منابع به‌شکلی کاربرپسند است.
**Excerpt:**
```
رابط `Article` در `frontend/src/app/encyclopedia/page.tsx` (خط 16-29) شامل `sources: Source[]` (آرایه‌ای از منابع) است، در حالی که مدل `Article` در بک‌اند (`backend/app/models/article.py`) دارای `source_id` (کلید خارجی منفرد) و یک رابطهٔ `source` (یک به چند از مقاله به منبع) است. این بدان معناست که فرانت‌اند انتظار لیستی از منابع را دارد، اما بک‌اند فقط به یک منبع لینک می‌دهد.
```

### Step 7: پیاده‌سازی و به‌روزرسانی تست‌های جامع برای قابلیت چند-منبعی
**Status:** `pending` (0%)
**Scope:** این مرحله شامل افزودن یا به‌روزرسانی تست‌های واحد (unit tests) و تست‌های یکپارچه‌سازی (integration tests) برای اطمینان از صحت عملکرد تمام تغییرات اعمال‌شده در بک‌اند و فرانت‌اند است. این تست‌ها باید پوشش‌دهندهٔ موارد زیر باشند: ذخیره‌سازی چندین منبع در مدل `Article`، دریافت چندین URL منبع توسط API ایجاد مقاله، بازیابی صحیح چندین منبع توسط API بازیابی مقاله، و نمایش صحیح چندین منبع در UI فرانت‌اند. این مرحله تضمین می‌کند که قابلیت جدید به‌درستی کار می‌کند و هیچ رگرسیونی ایجاد نشده است.
**Excerpt:**
```
رابط `Article` در `frontend/src/app/encyclopedia/page.tsx` (خط 16-29) شامل `sources: Source[]` (آرایه‌ای از منابع) است، در حالی که مدل `Article` در بک‌اند (`backend/app/models/article.py`) دارای `source_id` (کلید خارجی منفرد) و یک رابطهٔ `source` (یک به چند از مقاله به منبع) است. این بدان معناست که فرانت‌اند انتظار لیستی از منابع را دارد، اما بک‌اند فقط به یک منبع لینک می‌دهد. همچنین، فرم ایجاد مقاله در فرانت‌اند (`EMPTY_FORM` در خط 97) دارای `sourceUrls` (رشتهٔ چندگانه) است که در `handleCreate` (خط 181) به `source_urls` (آرایه) تبدیل می‌شود، اما API بک‌اند (`ArticleIngestRequest` در `backend/app/api/routes/encyclopedia.py`:86) فقط `source_url` (مفرد) را می‌پذیرد. این یک عدم تطابق حیاتی در نحوهٔ مدیریت منابع است.
```

---
task_id: 8ebe2d4d-7322-4f43-a928-526d9433db53
title: 'Anti-pattern: Under-engineering / Conditional inconsistency'
type: bug_fix
priority: high
...
created_at: '2026-06-13T18:10:52.531900+00:00'
updated_at: '2026-06-13T20:19:15.205725+00:00'
target_files:
- backend/app/core/config.py
---

# Anti-pattern: Under-engineering / Conditional inconsistency

## Raw Idea

The Neo4j configuration fields (`NEO4J_URI`, `NEO4J_USERNAME`, `NEO4J_PASSWORD`) are all optional. If `NEO4J_URI` is provided, `NEO4J_USERNAME` and `NEO4J_PASSWORD` should likely be mandatory to ensure a functional connection. Without this, the application might attempt to connect with incomplete credentials, leading to runtime errors or silent failures. A `model_validator` is needed to enforce th

📁 file: backend/app/core/config.py (line 80)
...
- `backend/app/api/deps.py` — این فایل `config.py` را import می‌کند (caller)
- `backend/app/main.py` — این فایل `config.py` را import می‌کند (caller)
- `backend/app/services/llm_adapter.py` — این فایل `config.py` را import می‌کند (caller)
- `backend/app/services/embeddings.py` — این فایل `config.py` را import می‌کند (caller)

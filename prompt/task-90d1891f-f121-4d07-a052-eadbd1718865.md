...
updated_at: '2026-06-13T18:02:52.524381+00:00'
target_files:
...
1. All ORM model files import `Base` directly from `app.db.base_class`.
2. No `try/except` blocks or local `DeclarativeBase` fallbacks are used for `Base` imports in model files.
3. Alembic migrations run successfully and detect schema changes as expected.

---
task_id: 90d1891f-f121-4d07-a052-eadbd1718865
title: Inconsistent SQLAlchemy Base Class Imports Leading to Potential Alembic Failures
type: bug
priority: critical
...
created_at: '2026-06-13T18:02:52.524378+00:00'
updated_at: '2026-06-13T20:16:57.776295+00:00'
target_files:
...
# Inconsistent SQLAlchemy Base Class Imports Leading to Potential Alembic Failures

## Raw Idea

Multiple SQLAlchemy ORM model files (`article.py`, `audit_log.py`, `person.py`, `risk_assessment.py`, `source.py`) employ `try/except` blocks to import the `Base` declarative class, including a fallback to a local `DeclarativeBase` if the shared `app.db.base_class.Base` is not found. This directly contradicts the explicit warning in `backend/app/models/user.py` (lines 45-49) which states that `Base` *must* be a single, shared source of truth for the entire backend to ensure Alembic migrations function correctly. This inconsistency can lead to models being registered with different `MetaData` objects, causing Alembic to fail to detect schema changes or attempt to create already existing tables, resulting in critical database migration issues.
...
- `backend/app/models/user.py:45-49` — The explicit warning against the current problematic pattern.
  ```python
  ...
  ```
- `backend/app/models/person.py:29-42` — Example of a model file using the problematic fallback logic.
  ```python
  try:
      ...
      try:
          ...
          class Base(DeclarativeBase):  # type: ignore
              ...
              pass
  ```
- `backend/app/models/__init__.py:36-49` — The `__init__.py` also attempts to defensively import Base, which should be removed if all models import directly.
  ```python
  Base: Any = None
...
SQLAlchemy 2.0, Alembic, Python ORM

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `backend/app/models/article.py` (سطر 42) — Uses try/except fallback for Base
- `backend/app/models/audit_log.py` (سطر 39) — Uses try/except fallback for Base
- `backend/app/models/risk_assessment.py` (سطر 46) — Uses try/except fallback for Base
- `backend/app/models/source.py` (سطر 36) — Uses try/except fallback for Base
- `backend/alembic/env.py` (سطر 20) — Alembic relies on a single MetaData object from Base
- `backend/app/db/base_class.py` — `user.py` این فایل را import می‌کند
- `backend/app/api/deps.py` — این فایل `user.py` را import می‌کند (caller)
- `backend/app/api/routes/auth.py` — این فایل `user.py` را import می‌کند (caller)
- `backend/app/api/routes/persons.py` — این فایل `user.py` را import می‌کند (caller)
...
## 🌐 نقشهٔ وابستگی‌ها
This is a core architectural issue affecting all ORM models and the database migration system (Alembic).
...
- [ ] All ORM model files import `Base` directly from `app.db.base_class`.
- [ ] No `try/except` blocks or local `DeclarativeBase` fallbacks are used for `Base` imports in model files.
- [ ] Alembic migrations run successfully and detect schema changes as expected.
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. Refactor all ORM model files to import `Base` directly and exclusively from `app.db.base_class` without any `try/except` fallbacks. Ensure `app.db.base_class.py` is always available and correctly defines the shared `Base` instance. Remove the `Base` fallback in `app.models/__init__.py` as well.
...
## 🧪 دستورات اعتبارسنجی
- `grep -r 'from app.db.base_class import Base' backend/app/models/`
- `grep -r 'DeclarativeBase' backend/app/models/`
- `cd backend && alembic revision --autogenerate -m 'Check for consistent Base'`

## ⚠️ ریسک‌ها و موارد احتیاط
High risk of breaking existing database migrations if not handled carefully; requires thorough testing of schema changes.
...
1. All ORM model files import `Base` directly from `app.db.base_class`. _(verify: static)_
2. No `try/except` blocks or local `DeclarativeBase` fallbacks are used for `Base` imports in model files. _(verify: static)_
3. Alembic migrations run successfully and detect schema changes as expected. _(verify: static)_

## Task Steps

### Step 1: بررسی و درک مشکل ناسازگاری کلاس Base در مدل‌های ORM
**Status:** `pending` (0%)
**Scope:** این مرحله شامل بررسی دقیق شرح مشکل، شناسایی الگوهای وارد کردن `Base` با `try/except` در فایل‌های مدل مشخص‌شده، و درک هشدار صریح در `backend/app/models/user.py` است. هدف اصلی این مرحله، درک ریشهٔ اصلی مشکلات احتمالی مهاجرت Alembic ناشی از استفاده از چندین شیء `MetaData` است. در این مرحله هیچ تغییر کدی انجام نمی‌شود.
**Excerpt:**
```
Multiple SQLAlchemy ORM model files (`article.py`, `audit_log.py`, `person.py`, `risk_assessment.py`, `source.py`) employ `try/except` blocks to import the `Base` declarative class, including a fallback to a local `DeclarativeBase` if the shared `app.db.base_class.Base` is not found. This directly contradicts the explicit warning in `backend/app/models/user.py` (lines 45-49) which states that `Base` *must* be a single, shared source of truth for the entire backend to ensure Alembic migrations function correctly. This inconsistency can lead to models being registered with different `MetaData` objects, causing Alembic to fail to detect schema changes or attempt to create already existing tables, resulting in critical database migration issues.
```

### Step 2: بازسازی `article.py` برای استفاده از کلاس Base مشترک
**Status:** `pending` (0%)
**Scope:** این مرحله بر اصلاح فایل `article.py` تمرکز دارد. هدف این است که هر بلوک `try/except` مربوط به وارد کردن `Base` حذف شود و اطمینان حاصل شود که `Base` به طور مستقیم و منحصراً از `app.db.base_class.Base` وارد می‌شود. این کار تضمین می‌کند که `article.py` به اصل منبع واحد حقیقت برای کلاس Base اعلامی پایبند است.
— [merged] این مرحله بر اصلاح فایل `audit_log.py` تمرکز دارد. هدف این است که هر بلوک `try/except` مربوط به وارد کردن `Base` حذف شود و اطمینان حاصل شود که `Base` به طور مستقیم و منحصراً از `app.db.base_class.Base` وارد می‌شود. این کار تضمین می‌کند که `audit_log.py` به اصل منبع واحد حقیقت برای کلاس Base اعلامی پایبند است.
— [merged] این مرحله بر اصلاح فایل `person.py` تمرکز دارد. هدف این است که هر بلوک `try/except` مربوط به وارد کردن `Base` حذف شود و اطمینان حاصل شود که `Base` به طور مستقیم و منحصراً از `app.db.base_class.Base` وارد می‌شود. این کار تضمین می‌کند که `person.py` به اصل منبع واحد حقیقت برای کلاس Base اعلامی پایبند است.
— [merged] این مرحله بر اصلاح فایل `risk_assessment.py` تمرکز دارد. هدف این است که هر بلوک `try/except` مربوط به وارد کردن `Base` حذف شود و اطمینان حاصل شود که `Base` به طور مستقیم و منحصراً از `app.db.base_class.Base` وارد می‌شود. این کار تضمین می‌کند که `risk_assessment.py` به اصل منبع واحد حقیقت برای کلاس Base اعلامی پایبند است.
— [merged] این مرحله بر اصلاح فایل `source.py` تمرکز دارد. هدف این است که هر بلوک `try/except` مربوط به وارد کردن `Base` حذف شود و اطمینان حاصل شود که `Base` به طور مستقیم و منحصراً از `app.db.base_class.Base` وارد می‌شود. این کار تضمین می‌کند که `source.py` به اصل منبع واحد حقیقت برای کلاس Base اعلامی پایبند است.
**Excerpt:**
```
Multiple SQLAlchemy ORM model files (`article.py`, `audit_log.py`, `person.py`, `risk_assessment.py`, `source.py`) employ `try/except` blocks to import the `Base` declarative class, including a fallback to a local `DeclarativeBase` if the shared `app.db.base_class.Base` is not found.
```

### Step 3: تأیید عملکرد مهاجرت Alembic
**Status:** `pending` (0%)
**Scope:** این مرحله حیاتی شامل تأیید این است که تغییرات اعمال شده در فایل‌های مدل، مشکل اساسی با مهاجرت‌های Alembic را حل کرده است. این شامل اجرای دستورات Alembic برای تشخیص تغییرات شمای پایگاه داده (`alembic revision --autogenerate`) و اعمال مهاجرت‌ها (`alembic upgrade head`) است تا اطمینان حاصل شود که هیچ خطایی مربوط به ناسازگاری `MetaData` یا تلاش برای ایجاد جداول موجود رخ نمی‌دهد. این مرحله اثربخشی اصلاح را تأیید می‌کند.
**Excerpt:**
```
This inconsistency can lead to models being registered with different `MetaData` objects, causing Alembic to fail to detect schema changes or attempt to create already existing tables, resulting in critical database migration issues.
```

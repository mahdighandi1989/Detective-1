...
updated_at: '2026-06-06T17:37:20.034901+00:00'
target_files:
...
1. تمام متن ارائه‌شده توسط کاربر که در فرانت‌اند نمایش داده می‌شود، به درستی escape یا پاکسازی شده است.
2. تلاش برای تزریق `<script>alert('XSS')</script>` در فیلدهای متنی، اسکریپت را در مرورگر اجرا نمی‌کند.
3. اعتبارسنجی بک‌اند ورودی‌هایی را که به وضوح مخرب هستند یا از محدودیت‌های طول معقول فراتر می‌روند، رد می‌کند.

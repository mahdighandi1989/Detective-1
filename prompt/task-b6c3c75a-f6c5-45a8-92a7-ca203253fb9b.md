---
task_id: b6c3c75a-f6c5-45a8-92a7-ca203253fb9b
title: 'endpoint بک‌اند بلااستفاده: POST /api/v1/encyclopedia/articles'
type: audit
priority: medium
...
created_at: '2026-06-13T18:15:55.307631+00:00'
updated_at: '2026-06-13T20:21:09.561303+00:00'
target_files:
- backend/tests/test_encyclopedia.py
---

# endpoint بک‌اند بلااستفاده: POST /api/v1/encyclopedia/articles

## Raw Idea

## 📋 شرح
endpoint `POST /api/v1/encyclopedia/articles` در `backend/tests/test_encyclopedia.py` تعریف شده ولی هیچ `fetch`، `axios`، `apiClient.*`، یا frontend call به آن دیده نمی‌شود.

## 🔍 جزئیات
- method: `POST`
- path: `/api/v1/encyclopedia/articles`
- فایل: `backend/tests/test_encyclopedia.py`
...
---
[scan #2 at 2026-06-13T18:15:55.386028+00:00]
## 📋 شرح
endpoint `GET /api/v1/encyclopedia/articles/{article_id}` در `backend/tests/test_encyclopedia.py` تعریف شده ولی هیچ `fetch`، `axios`، `apiClient.*`، یا frontend call به آن دیده نمی‌شود.

## 🔍 جزئیات
- method: `GET`
- path: `/api/v1/encyclopedia/articles/{article_id}`
- فایل: `backend/tests/
---
[scan #3 at 2026-06-13T18:15:55.434314+00:00]
## 📋 شرح
endpoint `GET /api/v1/encyclopedia/articles` در `backend/tests/test_encyclopedia.py` تعریف شده ولی هیچ `fetch`، `axios`، `apiClient.*`، یا frontend call به آن دیده نمی‌شود.
...
- عل
---
[scan #4 at 2026-06-13T18:15:55.470425+00:00]
## 📋 شرح
endpoint `GET /api/v1/encyclopedia/search` در `backend/tests/test_encyclopedia.py` تعریف شده ولی هیچ `fetch`، `axios`، `apiClient.*`، یا frontend call به آن دیده نمی‌شود.

## 🔍 جزئیات
- method: `GET`
- path: `/api/v1/encyclopedia/search`
- فایل: `backend/tests/test_encyclopedia.py`
- علت: n
---
[scan #5 at 2026-06-13T18:15:55.507267+00:00]
## 📋 شرح
endpoint `GET /api/v1/encyclopedia/audit-log` در `backend/tests/test_encyclopedia.py` تعریف شده ولی هیچ `fetch`، `axios`، `apiClient.*`، یا frontend call به آن دیده نمی‌شود.

## 🔍 جزئیات
- method: `GET`
- path: `/api/v1/encyclopedia/audit-log`
- فایل: `backend/tests/test_encyclopedia.py`
- 
...
- [ ] مشخص شد endpoint `POST /api/v1/encyclopedia/articles` در کدام دسته است (orphan/internal/deprecated)
...
1. گام ۱: grep روی `/api/v1/encyclopedia/articles` در frontend + scripts/ + docs/ — اگر caller هست، اتصال را drop شده اصلاح کن.
...
## 🧪 دستورات اعتبارسنجی
- `python -m py_compile backend/tests/test_encyclopedia.py`
- `ruff check backend/tests/test_encyclopedia.py`
- `pytest -x`
...
1. مشخص شد endpoint `POST /api/v1/encyclopedia/articles` در کدام دسته است (orphan/internal/deprecated) _(verify: static)_

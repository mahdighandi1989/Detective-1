---
task_id: 196cd143-054f-4084-93ef-d26ea5175090
title: Duplicated Frontend API Fetch and Authentication Logic
type: refactor
priority: medium
...
created_at: '2026-06-13T18:15:53.794147+00:00'
updated_at: '2026-06-13T20:20:03.149318+00:00'
target_files:
...
# Duplicated Frontend API Fetch and Authentication Logic

## Raw Idea

The logic for retrieving the authentication token from `localStorage` and making authenticated API calls is duplicated across multiple frontend pages (`encyclopedia/page.tsx`, `graph/page.tsx`, `persons/page.tsx`). This violates the DRY (Don't Repeat Yourself) principle, increases maintenance overhead, and makes it harder to implement consistent error handling or token refresh strategies. The `frontend/src/lib/api.ts` file is the ideal place for such shared utility functions.
...
- `frontend/src/app/encyclopedia/page.tsx:58-95` — `getAuthToken` — Duplicated token retrieval and API fetch logic.
  ```tsx
  ...
  ```
- `frontend/src/app/graph/page.tsx:252-260` — `loadGraph` — Duplicated token retrieval and direct fetch call.
  ```tsx
  const token =
  ...
  ```
- `frontend/src/app/persons/page.tsx:108-113` — `getToken` — Duplicated token retrieval logic.
  ```tsx
...
Next.js 14, React, TypeScript, Web Fetch API.

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `frontend/src/lib/api.ts` — This file should contain the centralized API utility.
- `frontend/src/app/layout.tsx` — Potentially affected if global API context is introduced.

## 🌐 نقشهٔ وابستگی‌ها
This refactoring improves code quality and maintainability across the frontend. It affects how all API calls are made from the client side.
...
- [ ] A single `apiFetch` utility exists in `frontend/src/lib/api.ts`.
- [ ] All frontend pages use this `apiFetch` utility for authenticated calls.
- [ ] The `getAuthToken` (or `getToken`) logic is encapsulated within `apiFetch` or a helper in `api.ts`.
...
1. Create a centralized `apiFetch` utility function in `frontend/src/lib/api.ts` that handles token retrieval, adds `Authorization` headers, and performs error handling. All pages should then import and use this single utility.

## 💡 نمونه‌های قبل/بعد
**Before (e.g., encyclopedia/page.tsx)**
...
// ...
const data = await apiFetch<Article[]>(`/api/encyclopedia/articles`);
```

_بعد:_
```
import { apiFetch } from '@/lib/api'; // Assuming api.ts exports it
// ...
const data = await apiFetch<Article[]>(`/api/encyclopedia/articles`);
```
...
## 🧪 دستورات اعتبارسنجی
- `Manually test all frontend pages (encyclopedia, graph, persons) to ensure data loads correctly after login.`
- `Check network requests in browser dev tools to confirm `Authorization` headers are sent.`

## ⚠️ ریسک‌ها و موارد احتیاط
Potential for introducing regressions in API calls if the refactoring is not thorough.
...
1. A single `apiFetch` utility exists in `frontend/src/lib/api.ts`. _(verify: static)_
2. All frontend pages use this `apiFetch` utility for authenticated calls. _(verify: static)_
3. The `getAuthToken` (or `getToken`) logic is encapsulated within `apiFetch` or a helper in `api.ts`. _(verify: static)_
4. All existing API calls continue to function correctly. _(verify: static)_

## Task Steps

### Step 1: Analyze existing token retrieval and API call logic duplication
**Status:** `pending` (0%)
**Scope:** This foundational step involves a thorough review of the current implementation of authentication token retrieval from `localStorage` and subsequent authenticated API calls within the specified frontend pages. The goal is to identify the exact duplicated code patterns, understand their current behavior, and determine the necessary parameters and return types for the new shared utility functions. This step does not involve any code modification but is crucial for designing the refactoring correctly.
**Excerpt:**
```
The logic for retrieving the authentication token from `localStorage` and making authenticated API calls is duplicated across multiple frontend pages (`encyclopedia/page.tsx`, `graph/page.tsx`, `persons/page.tsx`). This violates the DRY (Don't Repeat Yourself) principle, increases maintenance overhead, and makes it harder to implement consistent error handling or token refresh strategies. The `frontend/src/lib/api.ts` file is the ideal place for such shared utility functions.
```

### Step 2: Create or update utility function for token retrieval in `frontend/src/lib/api.ts`
**Status:** `pending` (0%)
**Scope:** This step focuses on creating a dedicated utility function within `frontend/src/lib/api.ts` responsible solely for retrieving the authentication token from `localStorage`. This function should encapsulate the logic for accessing `localStorage` and handling cases where the token might be missing or invalid. It should return the token or null/undefined, allowing calling functions to handle its presence.
**Excerpt:**
```
The logic for retrieving the authentication token from `localStorage` and making authenticated API calls is duplicated across multiple frontend pages (...). The `frontend/src/lib/api.ts` file is the ideal place for such shared utility functions.
```

### Step 3: Create or update utility function for authenticated API calls in `frontend/src/lib/api.ts`
**Status:** `pending` (0%)
**Scope:** This step involves creating a shared utility function in `frontend/src/lib/api.ts` that handles making authenticated API requests. This function will take the necessary API endpoint, method, and body, retrieve the authentication token using the utility created in the previous step, and include it in the request headers. It should also provide a basic structure for consistent error handling, as mentioned in the problem statement, without fully implementing complex error strategies yet.
**Excerpt:**
```
The logic for retrieving the authentication token from `localStorage` and making authenticated API calls is duplicated across multiple frontend pages (...). This violates the DRY (Don't Repeat Yourself) principle, increases maintenance overhead, and makes it harder to implement consistent error handling or token refresh strategies. The `frontend/src/lib/api.ts` file is the ideal place for such shared utility functions.
```

### Step 4: Refactor `encyclopedia/page.tsx` to use shared API utilities
**Status:** `pending` (0%)
**Scope:** This step involves modifying the `encyclopedia/page.tsx` file to remove its duplicated logic for token retrieval and authenticated API calls. Instead, it will import and utilize the newly created or updated utility functions from `frontend/src/lib/api.ts`. This ensures that `encyclopedia/page.tsx` adheres to the DRY principle and benefits from centralized authentication logic.
— [merged] This step focuses on refactoring `graph/page.tsx` to remove its duplicated logic for authentication token retrieval and authenticated API calls. It will be updated to import and use the shared utility functions from `frontend/src/lib/api.ts`, ensuring consistency and adherence to the DRY principle across the application's frontend pages.
— [merged] This step involves modifying the `persons/page.tsx` file to remove its duplicated logic for token retrieval and authenticated API calls. It will be updated to import and use the shared utility functions from `frontend/src/lib/api.ts`, completing the refactoring of all identified pages to use the centralized authentication and API calling mechanisms.
**Excerpt:**
```
The logic for retrieving the authentication token from `localStorage` and making authenticated API calls is duplicated across multiple frontend pages (`encyclopedia/page.tsx`, `graph/page.tsx`, `persons/page.tsx`). This violates the DRY (Don't Repeat Yourself) principle, increases maintenance overhead, and makes it harder to implement consistent error handling or token refresh strategies.
```

---
task_id: 8b67a619-396d-4ec8-8602-a1891455d6b5
title: کامپوننت `Header` فرانت‌اند وجود ندارد
type: bug
priority: critical
...
created_at: '2026-06-13T17:57:16.683880+00:00'
updated_at: '2026-06-13T20:16:07.427420+00:00'
target_files:
...
# کامپوننت `Header` فرانت‌اند وجود ندارد

## Raw Idea

فایل `frontend/src/app/layout.tsx` (خط 5) کامپوننت `Header` را از `@/components/Header` وارد می‌کند. با این حال، محتوای فایل `frontend/src/components/Header.tsx` در اطلاعات ارائه شده موجود نیست. این کامپوننت برای ناوبری سراسری و UI ثابت در صفحات مختلف حیاتی است.
...
- `frontend/src/app/layout.tsx:5` — `Header` — وارد کردن کامپوننت Header در layout اصلی
  ```tsx
  import { Header } from "@/components/Header";
  ```
- `frontend/src/app/layout.tsx:28` — `Header` — استفاده از کامپوننت Header در layout اصلی
  ```tsx
  <Header />
  ```
- `frontend/src/components/Header.tsx:1` — فایل Header.tsx که محتوای آن ارائه نشده است
...
- `frontend/src/app/page.tsx` (سطر -1) — صفحه اصلی که از Header استفاده می‌کند
- `frontend/src/app/encyclopedia/page.tsx` (سطر -1) — صفحه دانشنامه که از Header استفاده می‌کند
- `frontend/src/app/persons/page.tsx` (سطر -1) — صفحه اشخاص که از Header استفاده می‌کند
- `frontend/src/app/graph/page.tsx` (سطر -1) — صفحه گراف که از Header استفاده می‌کند
- `frontend/src/app/globals.css` — `layout.tsx` این فایل را import می‌کند

## 🌐 نقشهٔ وابستگی‌ها
این کامپوننت یک بخش اساسی از رابط کاربری فرانت‌اند است و بدون آن، ناوبری و تجربهٔ کاربری به شدت مختل می‌شود.
...
- [ ] کامپوننت `Header` در `frontend/src/components/Header.tsx` پیاده‌سازی شده است.
- [ ] هدر در تمام صفحات فرانت‌اند نمایش داده می‌شود.
- [ ] لینک‌های ناوبری در هدر کار می‌کنند.
- [ ] وضعیت احراز هویت کاربر در هدر نمایش داده می‌شود.
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. کامپوننت `frontend/src/components/Header.tsx` را پیاده‌سازی کنید. این کامپوننت باید شامل لینک‌های ناوبری اصلی (مانند به `/encyclopedia`, `/persons`, `/graph`, `/auth`)، نمایش وضعیت احراز هویت کاربر (مثلاً 'Logged in as User' یا 'Login') و احتمالاً یک دکمهٔ خروج باشد.

## 💡 نمونه‌های قبل/بعد
**بعد (frontend/src/components/Header.tsx - نمونه)**
...
export function Header() {
  // Logic for user session/auth status
    ...
    <header className="sticky top-0 z-40 w-full border-b bg-background/80 backdrop-blur-sm">
      <div className="container flex h-16 items-center justify-between">
        <Link href="/" className="mr-6 flex items-center space-x-2">
          <span className="font-bold">Detective-1</span>
        </Link>
        <nav className="flex items-center space-x-6 text-sm font-medium">
          <Link href="/encyclopedia" className="transition-colors hover:text-primary">دانشنامه</Link>
          <Link href="/persons" className="transition-colors hover:text-primary">اشخاص</Link>
          <Link href="/graph" className="transition-colors hover:text-primary">نمودار</Link>
          {isAuthenticated ? (
            <Button variant="ghost">خروج</Button>
          ) : (
            <Link href="/auth" className="transition-colors hover:text-primary">ورود</Link>
          )}
        </nav>
      </div>
    </header>
...
## 🧪 دستورات اعتبارسنجی
- `بررسی دستی ناوبری در تمام صفحات فرانت‌اند.`

## ⚠️ ریسک‌ها و موارد احتیاط
عدم پیاده‌سازی منجر به UI ناقص می‌شود.
...
1. کامپوننت `Header` در `frontend/src/components/Header.tsx` پیاده‌سازی شده است. _(verify: static)_
2. هدر در تمام صفحات فرانت‌اند نمایش داده می‌شود. _(verify: static)_
3. لینک‌های ناوبری در هدر کار می‌کنند. _(verify: static)_
4. وضعیت احراز هویت کاربر در هدر نمایش داده می‌شود. _(verify: static)_

## Task Steps

### Step 1: پیاده‌سازی یا تعریف کامپوننت Header برای ناوبری سراسری
**Status:** `pending` (0%)
**Scope:** این مرحله بر ایجاد یا تعریف محتوای کامپوننت `Header` که در مسیر `frontend/src/components/Header.tsx` قرار دارد، تمرکز دارد. هدف اصلی ارائه یک کامپوننت کاربردی است که ناوبری سراسری و عناصر UI ثابت را مدیریت کند، زیرا این کامپوننت برای تجربه کاربری یکپارچه در صفحات مختلف حیاتی است. این مرحله شامل تغییر فایل `frontend/src/app/layout.tsx` نمی‌شود، زیرا عبارت import از قبل در آنجا موجود است. همچنین، طراحی یا محتوای خاص لینک‌های ناوبری در این مرحله پوشش داده نمی‌شود و به مراحل بعدی طراحی UI موکول می‌گردد.
**Excerpt:**
```
فایل `frontend/src/app/layout.tsx` (خط 5) کامپوننت `Header` را از `@/components/Header` وارد می‌کند. با این حال، محتوای فایل `frontend/src/components/Header.tsx` در اطلاعات ارائه شده موجود نیست. این کامپوننت برای ناوبری سراسری و UI ثابت در صفحات مختلف حیاتی است.
```

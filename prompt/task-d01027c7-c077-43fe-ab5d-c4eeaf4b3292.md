---
task_id: d01027c7-c077-43fe-ab5d-c4eeaf4b3292
title: 'High: JWT Access Tokens Stored in Browser Local Storage'
type: security
priority: high
...
created_at: '2026-06-13T18:08:44.701965+00:00'
updated_at: '2026-06-13T20:18:04.056233+00:00'
target_files:
...
# High: JWT Access Tokens Stored in Browser Local Storage

## Raw Idea

The frontend application, specifically in `frontend/src/app/encyclopedia/page.tsx` (line 61), `frontend/src/app/graph/page.tsx` (line 254), and `frontend/src/app/persons/page.tsx` (line 111), retrieves JWT access tokens from `window.localStorage`. Storing JWTs in `localStorage` makes them vulnerable to Cross-Site Scripting (XSS) attacks. If an attacker successfully injects a malicious script into the application, they can easily access and steal the user's token, leading to session hijacking and unauthorized access to the user's account.
...
- `frontend/src/app/encyclopedia/page.tsx:58-64` — `getAuthToken` — This function is the primary point of token retrieval from local storage.
  ```tsx
  ...
  ```
- `frontend/src/app/graph/page.tsx:252-255` — `loadGraph` — Direct access to `localStorage` for token.
  ```tsx
  const token =
          typeof window !== 'undefined'
            ...
            : null;
  ```
- `frontend/src/app/persons/page.tsx:108-113` — `getToken` — Another instance of token retrieval from local storage.
  ```tsx
...
Next.js, FastAPI, JWT, browser security.

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `backend/app/api/routes/auth.py` (سطر 90) — The `/login` endpoint issues tokens. It needs to be modified to set `HttpOnly` cookies.
- `backend/app/core/security.py` — Token creation logic, might need adjustments for cookie-based auth.
- `frontend/src/lib/api.ts` — Central API utility, might need adjustments for cookie-based auth.

## 🌐 نقشهٔ وابستگی‌ها
This change affects the entire authentication flow for both frontend and backend.
...
- [ ] JWT access tokens are no longer stored in `localStorage`.
- [ ] Authentication works correctly using `HttpOnly` cookies.
- [ ] The backend `/login` endpoint sets `HttpOnly` cookies.
- [ ] The frontend correctly sends cookies with API requests.
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. Migrate JWT storage from `localStorage` to `HttpOnly` cookies. `HttpOnly` cookies are not accessible via client-side JavaScript, significantly mitigating XSS risks for token theft. The backend should be configured to issue these cookies, and the frontend should rely on the browser automatically sending them with requests.

## 💡 نمونه‌های قبل/بعد
**Frontend `getAuthToken` (conceptual after)**
...
_بعد:_
```
// With HttpOnly cookies, no client-side retrieval is needed. Browser sends automatically.
```

**Backend `/login` response (conceptual after)**
...
_بعد:_
```
response = JSONResponse(content=Token(access_token=access_token, refresh_token=refresh_token, token_type="bearer").model_dump())
...
## 🧪 دستورات اعتبارسنجی
- `Inspect browser `Application` tab -> `Local Storage` to confirm tokens are gone.`
- `Inspect browser `Application` tab -> `Cookies` to confirm `HttpOnly` cookies are present.`
- `Verify all authenticated frontend pages function correctly.`

## ⚠️ ریسک‌ها و موارد احتیاط
Breaking authentication if cookie handling is not implemented correctly on both sides.
...
1. JWT access tokens are no longer stored in `localStorage`. _(verify: static)_
2. Authentication works correctly using `HttpOnly` cookies. _(verify: static)_
3. The backend `/login` endpoint sets `HttpOnly` cookies. _(verify: static)_
4. The frontend correctly sends cookies with API requests. _(verify: static)_

## Task Steps

### Step 1: بررسی و برنامه‌ریزی برای رفع آسیب‌پذیری XSS ناشی از ذخیره JWT در localStorage
**Status:** `pending` (0%)
**Scope:** این مرحله شامل بررسی دقیق آسیب‌پذیری XSS گزارش‌شده به دلیل ذخیره توکن‌های دسترسی JWT در `window.localStorage` در فایل‌های فرانت‌اند مشخص‌شده است. دامنه این مرحله شامل درک پیاده‌سازی فعلی بازیابی توکن در `frontend/src/app/encyclopedia/page.tsx`، `frontend/src/app/encyclopedia/page.tsx` و `frontend/src/app/encyclopedia/page.tsx` می‌شود. همچنین، تحقیق و پیشنهاد یک جایگزین امن برای ذخیره‌سازی و بازیابی توکن، مانند استفاده از کوکی‌های HttpOnly، برای کاهش خطر XSS را در بر می‌گیرد. این فاز تغییرات لازم در هر دو کامپوننت فرانت‌اند و احتمالاً بک‌اند را برای پیاده‌سازی مکانیسم امن انتخابی شناسایی خواهد کرد. این مرحله صراحتاً پیاده‌سازی واقعی اصلاحیه را که مراحل بعدی خواهد بود، شامل نمی‌شود.
**Excerpt:**
```
The frontend application, specifically in `frontend/src/app/encyclopedia/page.tsx` (line 61), `frontend/src/app/encyclopedia/page.tsx` (line 254), and `frontend/src/app/encyclopedia/page.tsx` (line 111), retrieves JWT access tokens from `window.localStorage`. Storing JWTs in `localStorage` makes them vulnerable to Cross-Site Scripting (XSS) attacks. If an attacker successfully injects a malicious script into the application, they can easily access and steal the user's token, leading to session hijacking and unauthorized access to the user's account.
```

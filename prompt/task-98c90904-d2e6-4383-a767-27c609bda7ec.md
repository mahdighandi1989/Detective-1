---
task_id: 98c90904-d2e6-4383-a767-27c609bda7ec
title: افزودن تست‌های یکپارچه‌سازی برای چرخه حیات و میان‌افزار برنامه FastAPI
type: refactor
priority: high
...
created_at: '2026-06-13T18:10:52.045992+00:00'
updated_at: '2026-06-13T20:18:54.822265+00:00'
target_files:
- backend/app/main.py
---

# افزودن تست‌های یکپارچه‌سازی برای چرخه حیات و میان‌افزار برنامه FastAPI

## Raw Idea

فایل `app.main.py` نقطه ورود برنامه FastAPI است که مسئول پیکربندی CORS، میان‌افزار لاگ‌برداری درخواست و مدیریت چرخه حیات برنامه (راه‌اندازی/خاموش کردن اتصال پایگاه داده و Neo4j) است. مدیر زمینه `lifespan` و `request_logging_middleware` برای سلامت عملیاتی برنامه، نظارت بر عملکرد و اتصال به سرویس‌های خارجی حیاتی هستند. فقدان تست‌های صریح برای این اجزا به این معنی است که مشکلات احتمالی در راه‌اندازی، خاموش کردن یا پردازش درخواست ممکن است تا زمان استقرار مورد توجه قرار نگیرند.
...
- `backend/app/main.py:115-154, 183-217, 220-232` — `lifespan, request_logging_middleware, health, ready` — اجزای اصلی چرخه حیات و میان‌افزار برنامه
  ```python
  @asynccontextmanager
...
FastAPI, ASGI Middleware, `asynccontextmanager`, Mocking

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `backend/app/db/session.py` — شامل هوک‌های اتصال پایگاه داده است
- `backend/app/db/neo4j.py` — شامل هوک‌های اتصال Neo4j است (ضمنی)
- `backend/app/core/config.py` (سطر 54) — تنظیمات CORS و پیشوند API را فراهم می‌کند
- `backend/app/api/routes/auth.py` — `main.py` این فایل را import می‌کند
- `backend/app/api/routes/persons.py` — `main.py` این فایل را import می‌کند
- `backend/app/api/routes/encyclopedia.py` — `main.py` این فایل را import می‌کند
- `backend/tests/test_persons.py` — این فایل `main.py` را import می‌کند (caller)
- `backend/tests/conftest.py` — این فایل `main.py` را import می‌کند (caller)

## 🌐 نقشهٔ وابستگی‌ها
این اجزا برای کل برنامه FastAPI اساسی هستند و بر راه‌اندازی، خاموش کردن و هر درخواست HTTP ورودی تأثیر می‌گذارند.
...
- [ ] `lifespan` با موفقیت `_DB_CONNECT` و `_NEO4J_CONNECT` را در هنگام راه‌اندازی و همتایان `_DISCONNECT` آن‌ها را در هنگام خاموش کردن فراخوانی کند (در صورت وجود هوک‌ها).
- [ ] `request_logging_middleware` هدرهای `X-Request-ID` و `X-Process-Time-Ms` را به پاسخ‌ها اضافه کند.
- [ ] `request_logging_middleware` شروع/پایان درخواست و خطاها را به درستی لاگ کند.
- [ ] نقطه پایانی `/health` مقدار `{"status": "ok", "service": "detective1-api"}` را برگرداند.
- [ ] نقطه پایانی `/ready` مقدار `{"status": "ready", "database": bool, "neo4j": bool}` را با مقادیر بولی صحیح بر اساس حل هوک برگرداند.
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. یک فایل تست جدید به نام `backend/tests/test_main_app.py` ایجاد کنید. تست‌هایی را برای موارد زیر پیاده‌سازی کنید:
1. مدیر زمینه `lifespan`، اطمینان از اینکه اتصالات پایگاه داده و Neo4j به درستی راه‌اندازی و قطع می‌شوند (با استفاده از mock برای سرویس‌های خارجی).
2. میان‌افزار `request_logging_middleware`، تأیید اینکه شناسه‌های درخواست تولید/انتشار می‌یابند و لاگ‌برداری طبق انتظار انجام می‌شود.
3. نقاط پایانی `/health` و `/ready` وضعیت و اطلاعات صحیح را برگردانند.

## 📤 خروجی مورد انتظار
تغییر کد در فایل‌های مرتبط، commit یا PR جدید با پیام واضح، و عبور تمام معیارهای پذیرش.

## 🧪 دستورات اعتبارسنجی
- `pytest backend/tests/test_main_app.py`

## ⚠️ ریسک‌ها و موارد احتیاط
نیاز به mocking دقیق توابع اتصال پایگاه داده و Neo4j برای جلوگیری از فراخوانی‌های واقعی خارجی در طول تست‌ها.
...
1. `lifespan` با موفقیت `_DB_CONNECT` و `_NEO4J_CONNECT` را در هنگام راه‌اندازی و همتایان `_DISCONNECT` آن‌ها را در هنگام خاموش کردن فراخوانی کند (در صورت وجود هوک‌ها). _(verify: static)_
2. `request_logging_middleware` هدرهای `X-Request-ID` و `X-Process-Time-Ms` را به پاسخ‌ها اضافه کند. _(verify: static)_
3. `request_logging_middleware` شروع/پایان درخواست و خطاها را به درستی لاگ کند. _(verify: static)_
4. نقطه پایانی `/health` مقدار `{"status": "ok", "service": "detective1-api"}` را برگرداند. _(verify: static)_
5. نقطه پایانی `/ready` مقدار `{"status": "ready", "database": bool, "neo4j": bool}` را با مقادیر بولی صحیح بر اساس حل هوک برگرداند. _(verify: static)_

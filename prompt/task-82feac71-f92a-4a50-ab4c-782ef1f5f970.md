---
task_id: 82feac71-f92a-4a50-ab4c-782ef1f5f970
title: ناسازگاری در تعریف `ClassificationLevel` بین فرانت‌اند و بک‌اند
type: bug
priority: medium
...
created_at: '2026-06-13T18:15:54.001514+00:00'
updated_at: '2026-06-13T20:20:28.997891+00:00'
target_files:
- frontend/src/app/persons/page.tsx
- frontend/src/app/persons/page.tsx
---

# ناسازگاری در تعریف `ClassificationLevel` بین فرانت‌اند و بک‌اند

## Raw Idea

در فرانت‌اند، `PersonSummary.classification_level` (در `frontend/src/app/persons/page.tsx`) و `CLASSIFICATION_LABEL` فقط شامل `public`, `restricted`, `confidential`, `secret` هستند. این در حالی است که در بک‌اند، `backend/app/models/user.py` و `backend/app/models/article.py` شامل `TOP_SECRET` نیز می‌شوند. این ناسازگاری می‌تواند منجر به عدم نمایش صحیح یا مدیریت نادرست محتوای `TOP_SECRET` در رابط کاربری شود.
...
- `frontend/src/app/persons/page.tsx:35-36` — تعریف ناقص ClassificationLevel در PersonSummary
  ```tsx
  classification_level: 'public' | 'restricted' | 'confidential' | 'secret';
  ```
- `frontend/src/app/persons/page.tsx:100-106` — تعریف ناقص CLASSIFICATION_LABEL
  ```tsx
...
TypeScript Types, React Frontend, Backend Enums

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `backend/app/models/user.py` (سطر 91) — مدل User شامل ClearanceLevel.TOP_SECRET است.
- `backend/app/models/article.py` (سطر 70) — مدل Article شامل ClassificationLevel.TOP_SECRET است.
- `backend/app/core/enums.py` (سطر N/A) — منبع اصلی تعریف ClassificationLevel.

## 🌐 نقشهٔ وابستگی‌ها
این تغییر بر روی نمایش و مدیریت سطوح محرمانگی در فرانت‌اند تأثیر می‌گذارد.
...
- [ ] نوع `ClassificationLevel` در فرانت‌اند شامل `top_secret` باشد.
- [ ] `CLASSIFICATION_LABEL` در فرانت‌اند شامل برچسب فارسی برای `top_secret` باشد.
- [ ] محتوای `TOP_SECRET` (در صورت دسترسی کاربر) به درستی در UI نمایش داده شود.
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. تعریف `ClassificationLevel` در فرانت‌اند (`frontend/src/app/persons/page.tsx`) باید به‌روزرسانی شود تا شامل `top_secret` باشد و با تعاریف بک‌اند (`backend/app/core/enums.py` یا `backend/app/models/user.py`) همخوانی داشته باشد. همچنین، منطق نمایش و فیلترینگ در UI باید برای مدیریت این سطح جدید به‌روزرسانی شود.
...
## 🧪 دستورات اعتبارسنجی
- `بررسی دستی UI برای اطمینان از نمایش صحیح سطوح محرمانگی.`
...
1. نوع `ClassificationLevel` در فرانت‌اند شامل `top_secret` باشد. _(verify: static)_
2. `CLASSIFICATION_LABEL` در فرانت‌اند شامل برچسب فارسی برای `top_secret` باشد. _(verify: static)_
3. محتوای `TOP_SECRET` (در صورت دسترسی کاربر) به درستی در UI نمایش داده شود. _(verify: static)_

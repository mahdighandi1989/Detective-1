---
task_id: 628c5d71-dcce-4a86-a256-8ca253d847a6
title: حذف آبشاری (cascade delete) نادرست در روابط Person به Article و Source
type: bug
priority: critical
...
created_at: '2026-06-13T18:04:48.177154+00:00'
updated_at: '2026-06-13T20:17:20.920039+00:00'
target_files:
- backend/app/models/person.py
---

# حذف آبشاری (cascade delete) نادرست در روابط Person به Article و Source

## Raw Idea

در مدل `Person`، روابط `articles` و `sources` با `cascade="all, delete-orphan"` تعریف شده‌اند. این تنظیم به این معنی است که با حذف یک `Person`، تمام `Article`ها و `Source`های مرتبط نیز حذف خواهند شد. این رفتار احتمالاً نادرست است، زیرا مقالات دانشنامه و منابع اطلاعاتی معمولاً موجودیت‌های مستقلی هستند که نباید با حذف یک شخص از بین بروند. این یک مشکل جدی در یکپارچگی داده‌ها و احتمال از دست رفتن اطلاعات است.
...
- `backend/app/models/person.py:144-151` — تعریف روابط sources و articles با cascade نادرست در مدل Person
  ```python
...
SQLAlchemy ORM, Many-to-Many Relationships, Cascade Options

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `backend/app/models/article.py` (سطر 166) — مدل Article که به Person مرتبط است.
- `backend/app/models/source.py` (سطر 175) — مدل Source که به Person مرتبط است.
...
## 🌐 نقشهٔ وابستگی‌ها
این تغییر بر روی عملیات حذف `Person` و یکپارچگی داده‌های `Article` و `Source` تأثیر می‌گذارد.
...
- [ ] حذف یک `Person` فقط رکوردهای مربوطه را از جداول `article_person_association` و `person_source_association` حذف کند.
- [ ] حذف یک `Person` منجر به حذف `Article`ها یا `Source`های اصلی نشود.
- [ ] تست‌های واحد برای تأیید رفتار صحیح حذف اضافه شود.
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. تنظیم `cascade` برای روابط `articles` و `sources` در مدل `Person` باید اصلاح شود. از آنجایی که اینها روابط چند به چند (many-to-many) هستند که از طریق جداول ارتباطی (`article_person_association_table` و `person_source_association`) مدیریت می‌شوند، حذف یک `Person` باید فقط رکوردهای مربوطه را از جداول ارتباطی حذف کند، نه خود `Article`ها یا `Source`ها را. `cascade="all, delete-orphan"` برای روابط یک به چند (one-to-many) که موجودیت فرزند کاملاً به والد وابسته است، مناسب است، نه برای روابط چند به چند.
...
**بعد: cascade صحیح برای M2M (حذف از جدول واسط)**

_بعد:_
```
articles: Mapped[List["Article"]] = relationship(
    "Article", secondary=article_person_association_table, back_populates="persons"
)
sources: Mapped[List["Source"]] = relationship(
    "Source", secondary=person_source_association, back_populates="persons"
)
```
...
## 🧪 دستورات اعتبارسنجی
- `ایجاد یک شخص، یک مقاله و یک منبع. ارتباط دادن شخص به مقاله و منبع. حذف شخص و بررسی عدم حذف مقاله و منبع.`

## ⚠️ ریسک‌ها و موارد احتیاط
احتمال ایجاد خطاهای Foreign Key یا عدم حذف صحیح ارتباطات در صورت عدم دقت.
...
1. حذف یک `Person` فقط رکوردهای مربوطه را از جداول `article_person_association` و `person_source_association` حذف کند. _(verify: static)_
2. حذف یک `Person` منجر به حذف `Article`ها یا `Source`های اصلی نشود. _(verify: static)_
3. تست‌های واحد برای تأیید رفتار صحیح حذف اضافه شود. _(verify: static)_

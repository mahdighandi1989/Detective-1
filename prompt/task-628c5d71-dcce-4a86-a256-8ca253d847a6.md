...
updated_at: '2026-06-13T18:04:48.177156+00:00'
target_files:
...
1. حذف یک `Person` فقط رکوردهای مربوطه را از جداول `article_person_association` و `person_source_association` حذف کند.
2. حذف یک `Person` منجر به حذف `Article`ها یا `Source`های اصلی نشود.
3. تست‌های واحد برای تأیید رفتار صحیح حذف اضافه شود.

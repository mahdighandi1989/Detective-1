---
task_id: 58fa3e70-eaf9-4bf4-85dc-48c2525e5a6c
...
created_at: '2026-06-13T18:10:52.551021+00:00'
updated_at: '2026-06-13T20:19:20.509913+00:00'
target_files:
...
The `article_person_association_table` is defined, and `Person` is imported for type checking, indicating a clear intent for a many-to-many relationship. However, the `Article` model itself is missing the corresponding `persons` relationship mapping (e.g., `persons: Mapped[list['Person']] = relationship(...)`). This leaves the relationship incomplete and unusable from the `Article` model, creating

📁 file: backend/app/models/article.py (line 120)

🎯 پیشنهاد: این الگو معمولاً منطق سیستم را در شرایط لبه می‌شکند.
---
[scan #2 at 2026-06-13T18:10:52.588517+00:00]
The `EMBEDDING_DIM` constant (1536) is hardcoded. This value is specific to a particular embedding model (e.g., OpenAI text-embedding-3-small). If the underlying embedding model changes in the future (e.g., to a newer version or a different provider with a different output dimension), this hardcoded
...
- `backend/app/models/article.py:120`
...
## 🧪 دستورات اعتبارسنجی
- `python -m py_compile backend/app/models/article.py`
- `ruff check backend/app/models/article.py`
- `pytest -x`

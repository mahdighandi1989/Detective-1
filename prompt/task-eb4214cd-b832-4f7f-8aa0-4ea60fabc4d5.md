---
task_id: eb4214cd-b832-4f7f-8aa0-4ea60fabc4d5
title: Redundant and Problematic `backend/app/api/routes/risk_assessment.py` Module
type: refactor
priority: high
...
created_at: '2026-06-13T18:07:20.977959+00:00'
updated_at: '2026-06-13T20:17:51.252761+00:00'
target_files:
- backend/app/api/routes/risk_assessment.py
---

# Redundant and Problematic `backend/app/api/routes/risk_assessment.py` Module

## Raw Idea

The file `backend/app/api/routes/risk_assessment.py` contains API routes for risk assessments that largely overlap with the functionality provided in `backend/app/api/routes/risk.py`. Furthermore, it uses defensive `try/except` imports for core dependencies (e.g., `get_db`, `get_current_user`, `RiskAssessment` model), which explicitly violates the project's stated 'fail-fast' wiring philosophy as documented in `backend/app/main.py` and `backend/app/api/routes/persons.py`. Its enum definitions for `RiskCategory` and `RiskLevel` are also inconsistent with other modules, adding to the overall enum inconsistency problem.
...
- `backend/app/api/routes/risk_assessment.py:1-250` — `router` — The entire file is problematic due to defensive imports and redundancy.
  ```python
...
FastAPI, SQLAlchemy ORM, Pydantic, Celery

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `backend/app/main.py` (سطر 20) — Explicitly states fail-fast philosophy
- `backend/app/api/routes/persons.py` (سطر 24) — Explicitly states fail-fast philosophy
- `backend/app/api/routes/risk.py` (سطر 1) — Overlapping functionality for risk assessment
- `backend/app/api/__init__.py` (سطر 17) — Should not include this router if removed
- `backend/app/api/deps.py` — `risk_assessment.py` این فایل را import می‌کند
- `backend/app/models/risk_assessment.py` — `risk_assessment.py` این فایل را import می‌کند
- `backend/app/models/person.py` — `risk_assessment.py` این فایل را import می‌کند
- `backend/app/services/risk_engine.py` — `risk_assessment.py` این فایل را import می‌کند

## 🌐 نقشهٔ وابستگی‌ها
This file is not currently included in the main API router, which is good. However, its presence and inconsistent design principles pose a risk for future development and maintainability.
...
- [ ] The file `backend/app/api/routes/risk_assessment.py` is removed.
- [ ] Any unique and valuable functionality from `risk_assessment.py` is safely merged into `backend/app/api/routes/risk.py`.
- [ ] No new regressions or inconsistencies are introduced in `backend/app/api/routes/risk.py`.
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. Remove the entire `backend/app/api/routes/risk_assessment.py` file. Before removal, carefully review its unique functionalities (if any, such as `RecomputeRequest`/`Response` or specific evidence handling) and merge them into `backend/app/api/routes/risk.py` after resolving enum inconsistencies. Ensure that `backend/app/api/__init__.py` does not attempt to include this route.

## 📤 خروجی مورد انتظار
تغییر کد در فایل‌های مرتبط، commit یا PR جدید با پیام واضح، و عبور تمام معیارهای پذیرش.

## 🧪 دستورات اعتبارسنجی
- `ls backend/app/api/routes/ | grep risk_assessment.py`
- `git diff --stat backend/app/api/routes/risk.py`

## ⚠️ ریسک‌ها و موارد احتیاط
Potential for data loss or functional regressions if unique features are not properly migrated.
...
1. The file `backend/app/api/routes/risk_assessment.py` is removed. _(verify: static)_
2. Any unique and valuable functionality from `risk_assessment.py` is safely merged into `backend/app/api/routes/risk.py`. _(verify: static)_
3. No new regressions or inconsistencies are introduced in `backend/app/api/routes/risk.py`. _(verify: static)_

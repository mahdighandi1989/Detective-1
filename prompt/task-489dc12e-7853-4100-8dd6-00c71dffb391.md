...
updated_at: '2026-06-13T20:16:40.134944+00:00'
target_files:
...
1. A single, canonical `ClassificationLevel` enum exists in `app.core.enums.py`. _(verify: static)_
2. All backend models, schemas, and API routes import and use this canonical enum. _(verify: static)_
3. `encyclopedia.py` uses `require_roles` and `UserRole` enum for authorization. _(verify: static)_
4. Frontend displays classification labels consistently with backend. _(verify: static)_

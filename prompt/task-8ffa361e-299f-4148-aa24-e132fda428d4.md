---
task_id: 8ffa361e-299f-4148-aa24-e132fda428d4
title: 'Anti-pattern: Stale assumption / Under-engineering / Conditional'
type: bug_fix
priority: high
...
created_at: '2026-06-13T18:10:52.588809+00:00'
updated_at: '2026-06-13T20:19:25.412501+00:00'
target_files:
- backend/app/db/base.py
---

# Anti-pattern: Stale assumption / Under-engineering / Conditional

## Raw Idea

این ماژول به صورت صریح نیازمند به‌روزرسانی دستی برای هر مدل جدیدی است که به پروژه اضافه می‌شود. این طراحی چندین anti-pattern منطقی را معرفی می‌کند:

- **Stale Assumption (فرض منسوخ)**: فرض بر این است که توسعه‌دهندگان همواره به یاد خواهند داشت که این فایل را به‌روز کنند، که مستعد خطای انسانی است. این فرض با تکامل پروژه به راحتی می‌تواند منسوخ شود.
- **Under-engineering (مهندسی ناکافی)**: برای وظیفه

📁 file: backend/app/db/base.py (line 38)
...
- `backend/app/db/base_class.py` — `base.py` این فایل را import می‌کند
- `backend/app/models/user.py` — `base.py` این فایل را import می‌کند
- `backend/app/models/person.py` — `base.py` این فایل را import می‌کند
- `backend/app/models/relationship.py` — `base.py` این فایل را import می‌کند
- `backend/app/models/audit_log.py` — این فایل `base.py` را import می‌کند (caller)
- `backend/app/models/risk_assessment.py` — این فایل `base.py` را import می‌کند (caller)
...
## 🧪 دستورات اعتبارسنجی
- `python -m py_compile backend/app/db/base.py`
- `ruff check backend/app/db/base.py`
- `pytest -x`

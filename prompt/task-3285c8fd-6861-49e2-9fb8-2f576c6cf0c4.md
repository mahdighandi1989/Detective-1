---
task_id: 3285c8fd-6861-49e2-9fb8-2f576c6cf0c4
title: 'endpoint بک‌اند بلااستفاده: POST /articles'
type: audit
priority: medium
...
created_at: '2026-06-13T18:15:54.897085+00:00'
updated_at: '2026-06-13T20:20:44.371385+00:00'
target_files:
- backend/app/api/routes/encyclopedia.py
---

# endpoint بک‌اند بلااستفاده: POST /articles

## Raw Idea

## 📋 شرح
endpoint `POST /articles` در `backend/app/api/routes/encyclopedia.py` تعریف شده ولی هیچ `fetch`، `axios`، `apiClient.*`، یا frontend call به آن دیده نمی‌شود.

## 🔍 جزئیات
- method: `POST`
- path: `/articles`
- فایل: `backend/app/api/routes/encyclopedia.py`
...
---
[scan #2 at 2026-06-13T18:15:54.962706+00:00]
## 📋 شرح
endpoint `GET /articles` در `backend/app/api/routes/encyclopedia.py` تعریف شده ولی هیچ `fetch`، `axios`، `apiClient.*`، یا frontend call به آن دیده نمی‌شود.
...
- `backend/app/api/deps.py` — `encyclopedia.py` این فایل را import می‌کند
...
- [ ] مشخص شد endpoint `POST /articles` در کدام دسته است (orphan/internal/deprecated)
...
1. گام ۱: grep روی `/articles` در frontend + scripts/ + docs/ — اگر caller هست، اتصال را drop شده اصلاح کن.
...
## 🧪 دستورات اعتبارسنجی
- `python -m py_compile backend/app/api/routes/encyclopedia.py`
- `ruff check backend/app/api/routes/encyclopedia.py`
- `pytest -x`
...
1. مشخص شد endpoint `POST /articles` در کدام دسته است (orphan/internal/deprecated) _(verify: static)_

---
task_id: 096d70e3-f18c-4aae-b4d6-97a1113aa0a6
title: Frontend Enum Mismatches with Backend
type: bug
priority: high
...
created_at: '2026-06-13T18:09:36.395424+00:00'
updated_at: '2026-06-13T20:18:31.855308+00:00'
target_files:
...
# Frontend Enum Mismatches with Backend

## Raw Idea

The frontend components define their own TypeScript types and enums that are intended to mirror backend definitions, but several inconsistencies exist. For example, `frontend/src/app/graph/page.tsx` and `frontend/src/app/persons/page.tsx` use `suspect` for risk levels, while the backend often uses `SUSPICIOUS`. Additionally, `frontend/src/app/persons/page.tsx`'s `classification_level` type (`public | restricted | confidential | secret`) is missing `top_secret`, which is present in backend models. These mismatches will lead to incorrect UI rendering (e.g., wrong labels, colors), filtering issues, and potential runtime type errors when integrating with the API, degrading the user experience and data accuracy.
...
- `frontend/src/app/graph/page.tsx:26-26` — Frontend `RiskLevel` uses 'suspect'.
  ```tsx
  type RiskLevel = 'clean' | 'suspect' | 'infiltrator' | 'transformed' | 'unknown';
  ```
- `frontend/src/app/persons/page.tsx:11-16` — Frontend `RiskCategory` uses 'suspect'.
  ```tsx
  export type RiskCategory =
    ...
    | 'unknown';
  ```
- `frontend/src/app/persons/page.tsx:100-105` — Frontend `classification_level` is missing 'top_secret'.
  ```tsx
...
Next.js, TypeScript, FastAPI, Enums

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `backend/app/api/routes/graph.py` (سطر 36) — Backend defines `RiskLevel` with `SUSPICIOUS`.
- `backend/app/models/person.py` (سطر 50) — Backend defines `RiskCategory` with `SUSPECT` (but `app.api.routes.graph.py` uses `SUSPICIOUS`).
- `backend/app/models/user.py` (سطر 84) — Backend defines `ClearanceLevel` including `TOP_SECRET`.
- `backend/app/models/article.py` (سطر 64) — Backend defines `ClassificationLevel` including `TOP_SECRET`.

## 🌐 نقشهٔ وابستگی‌ها
This affects the frontend's ability to correctly display and filter data based on backend classifications and risk levels.
...
- [ ] Frontend `RiskLevel` and `RiskCategory` types use consistent naming (e.g., `suspicious`) with backend.
- [ ] Frontend `classification_level` type includes all levels defined in the backend (e.g., `top_secret`).
- [ ] UI elements (badges, filters, graph nodes) correctly reflect backend enum values and associated metadata (e.g., colors, labels).
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. Synchronize frontend TypeScript enum definitions with the canonical backend Python enums (once the backend enums are unified in `app.core.enums.py`). Specifically, ensure consistent naming (e.g., `SUSPICIOUS` everywhere) and complete value sets (e.g., include `TOP_SECRET`). Consider generating frontend types from backend schemas if a tool is available, or maintain them meticulously manually.

## 💡 نمونه‌های قبل/بعد
**Before (frontend RiskLevel)**
...
_بعد:_
```
type RiskLevel = 'clean' | 'suspicious' | 'infiltrator' | 'transformed' | 'unknown';
```

**Before (frontend ClassificationLabel)**
...
## 🧪 دستورات اعتبارسنجی
- `Manually inspect frontend UI for correct display of risk levels and classification levels.`
- `Test filtering functionality in persons and graph pages.`

## ⚠️ ریسک‌ها و موارد احتیاط
Minor risk of UI breakage during refactoring; requires careful testing of all affected components.
...
1. Frontend `RiskLevel` and `RiskCategory` types use consistent naming (e.g., `suspicious`) with backend. _(verify: static)_
2. Frontend `classification_level` type includes all levels defined in the backend (e.g., `top_secret`). _(verify: static)_
3. UI elements (badges, filters, graph nodes) correctly reflect backend enum values and associated metadata (e.g., colors, labels). _(verify: static)_

## Task Steps

### Step 1: استانداردسازی نوع/enum سطح ریسک در فرانت‌اند برای تطابق با 'SUSPICIOUS' بک‌اند
**Status:** `pending` (0%)
**Scope:** این مرحله شامل شناسایی تعریف نوع یا enum TypeScript در کدبیس فرانت‌اند است که سطوح ریسک را نشان می‌دهد. تعریف فعلی که از 'suspect' استفاده می‌کند، باید به 'SUSPICIOUS' به‌روزرسانی شود تا با اصطلاحات بک‌اند همسو شود. این مرحله صرفاً بر روی خود تعریف نوع تمرکز دارد و نه بر استفاده از آن در فایل‌های خاص. هدف ایجاد یک منبع واحد و سازگار برای سطوح ریسک در فرانت‌اند است.
**Excerpt:**
```
The frontend components define their own TypeScript types and enums that are intended to mirror backend definitions, but several inconsistencies exist. For example, `frontend/src/app/graph/page.tsx` and `frontend/src/app/graph/page.tsx` use `suspect` for risk levels, while the backend often uses `SUSPICIOUS`.
```

### Step 2: به‌روزرسانی `frontend/src/app/graph/page.tsx` برای استفاده از سطح ریسک استانداردشده
**Status:** `pending` (0%)
**Scope:** این مرحله بر روی اصلاح `frontend/src/app/graph/page.tsx` تمرکز دارد. شامل یافتن تمام مواردی است که سطح ریسک 'suspect' در این فایل خاص استفاده یا ارجاع داده شده است و به‌روزرسانی آن‌ها برای استفاده از نوع/enum سطح ریسک 'SUSPICIOUS' که به تازگی استاندارد شده است. این کار تضمین می‌کند که کامپوننت گراف داده‌های ریسک را به درستی تفسیر و نمایش می‌دهد.
— [merged] این مرحله شامل اصلاح `frontend/src/app/graph/page.tsx` است. هدف شناسایی و به‌روزرسانی تمام مواردی است که سطح ریسک 'suspect' در حال حاضر در این فایل استفاده یا ارجاع داده شده است و جایگزینی آن‌ها با نوع/enum سطح ریسک 'SUSPICIOUS' که استاندارد شده است. این کار تضمین می‌کند که کامپوننت لیست افراد داده‌های ریسک را به درستی مدیریت و نمایش می‌دهد.
**Excerpt:**
```
For example, `frontend/src/app/graph/page.tsx` and `frontend/src/app/graph/page.tsx` use `suspect` for risk levels, while the backend often uses `SUSPICIOUS`.
```

### Step 3: افزودن 'top_secret' به نوع `classification_level` در `frontend/src/app/graph/page.tsx`
**Status:** `pending` (0%)
**Scope:** این مرحله به طور خاص تعریف نوع `classification_level` را در `frontend/src/app/graph/page.tsx` هدف قرار می‌دهد. وظیفه این است که این نوع union TypeScript را گسترش دهیم تا شامل literal 'top_secret' شود و اطمینان حاصل شود که به طور کامل سطوح طبقه‌بندی بک‌اند را منعکس می‌کند. این کار به UI اجازه می‌دهد تا آیتم‌های دارای بالاترین طبقه‌بندی را به درستی مدیریت و نمایش دهد.
**Excerpt:**
```
Additionally, `frontend/src/app/graph/page.tsx`'s `classification_level` type (`public | restricted | confidential | secret`) is missing `top_secret`, which is present in backend models.
```

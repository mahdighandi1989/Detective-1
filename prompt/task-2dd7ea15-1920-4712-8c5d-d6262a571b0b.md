---
task_id: 2dd7ea15-1920-4712-8c5d-d6262a571b0b
title: Missing RBAC for `list_articles` and `list_sources` Endpoints
type: security
priority: critical
...
created_at: '2026-06-13T17:58:47.903820+00:00'
updated_at: '2026-06-13T20:16:17.652481+00:00'
target_files:
...
# Missing RBAC for `list_articles` and `list_sources` Endpoints

## Raw Idea

The `list_articles` endpoint in `backend/app/api/routes/encyclopedia.py` and the `list_sources`, `get_source` endpoints in `backend/app/api/routes/sources.py` currently only check for user authentication (`get_current_user`) but do not enforce any role-based access control (RBAC) or classification level filtering. This means any authenticated user, regardless of their assigned role or clearance level, can view all encyclopedia articles and all OSINT sources, potentially exposing sensitive or classified information to unauthorized individuals.
...
- `backend/app/api/routes/encyclopedia.py:302-345` — `list_articles` — Missing classification filtering for articles.
  ```python
  ...
  ```
- `backend/app/api/routes/sources.py:193-230` — `list_sources` — Missing role-based access control for listing sources.
  ```python
  ...
  ```
- `backend/app/api/routes/sources.py:239-249` — `get_source` — Missing role-based access control for retrieving a single source.
  ```python
...
FastAPI, SQLAlchemy, RBAC, Classification Levels

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `backend/app/api/deps.py` (سطر 291) — Contains RBAC helpers like `require_roles` and `can_access_classification`
- `backend/app/models/user.py` (سطر 52) — Defines `UserRole` and `ClearanceLevel`
- `backend/app/models/article.py` (سطر 64) — Defines `ClassificationLevel` for articles
- `frontend/src/app/encyclopedia/page.tsx` (سطر 129) — Calls `list_articles`
- `frontend/src/app/graph/page.tsx` (سطر 256) — Calls `/api/graph` which might indirectly use sources
- `backend/app/services/llm_adapter.py` — `encyclopedia.py` این فایل را import می‌کند
- `backend/app/services/embeddings.py` — `encyclopedia.py` این فایل را import می‌کند
- `backend/app/main.py` — این فایل `encyclopedia.py` را import می‌کند (caller)
- `backend/app/models/source.py` — `sources.py` این فایل را import می‌کند

## 🌐 نقشهٔ وابستگی‌ها
These endpoints are fundamental for displaying intelligence data. Lack of RBAC affects the entire security posture of the application.
...
- [ ] A user with `VIEWER` role and `UNCLASSIFIED` clearance can only see `UNCLASSIFIED` articles.
- [ ] A user with `ANALYST` role and `SECRET` clearance can see `UNCLASSIFIED`, `RESTRICTED`, `CONFIDENTIAL`, `SECRET` articles.
- [ ] Only users with `ANALYST` or `ADMIN` roles can create/update sources.
- [ ] Users with `VIEWER` role can list and get sources, but cannot modify them.
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. Implement proper RBAC and classification-level filtering for these endpoints:
1.  **For `list_articles`:** Modify the SQLAlchemy query to filter articles based on the `current_user`'s `clearance` level, using the `can_access_classification` helper from `app.api.deps` or a similar logic. Only articles with a classification level accessible to the user should be returned.
2.  **For `list_sources` and `get_source`:** Apply the `require_roles` dependency (e.g., `Depends(require_roles(UserRole.ANALYST, UserRole.ADMIN, UserRole.VIEWER))`) to restrict access to authorized roles. If sources themselves can have classification levels, implement similar filtering.

## 💡 نمونه‌های قبل/بعد
**Add classification filter to list_articles**
...
_بعد:_
```
from app.api.deps import can_access_classification # Assuming this is the correct path

# ... inside list_articles
    accessible_levels = [level.value for level in ClassificationLevel if can_access_classification(user, level)]
    stmt = select(Article).where(Article.classification_level.in_(accessible_levels))
```

**Add role dependency to list_sources**
...
_بعد:_
```
current_user=Depends(require_roles(UserRole.ANALYST, UserRole.ADMIN, UserRole.VIEWER)),
```
...
## 🧪 دستورات اعتبارسنجی
- `pytest backend/tests/test_encyclopedia.py -k 'rbac'`
- `pytest backend/tests/test_sources.py -k 'rbac'`

## ⚠️ ریسک‌ها و موارد احتیاط
Potential for accidental over-filtering or under-filtering if classification logic is complex. Requires thorough testing with different user roles/clearances.
...
1. A user with `VIEWER` role and `UNCLASSIFIED` clearance can only see `UNCLASSIFIED` articles. _(verify: static)_
2. A user with `ANALYST` role and `SECRET` clearance can see `UNCLASSIFIED`, `RESTRICTED`, `CONFIDENTIAL`, `SECRET` articles. _(verify: static)_
3. Only users with `ANALYST` or `ADMIN` roles can create/update sources. _(verify: static)_
4. Users with `VIEWER` role can list and get sources, but cannot modify them. _(verify: static)_

## Task Steps

### Step 1: اعمال فیلترینگ RBAC/Classification بر روی endpoint `list_articles`
**Status:** `pending` (0%)
**Scope:** این مرحله بر روی اصلاح تابع `list_articles` در فایل `backend/app/api/routes/encyclopedia.py` تمرکز دارد. هدف، ادغام منطقی است که مقالات بازگردانده شده را بر اساس نقش‌ها و سطوح دسترسی کاربر احراز هویت شده فیلتر کند و اطمینان حاصل شود که فقط افراد مجاز می‌توانند مقالات خاص را مشاهده کنند. این مرحله شامل اصلاح سایر endpointها یا تعریف سیستم اصلی RBAC نیست، بلکه اعمال قوانین RBAC موجود یا مفروض است.
— [merged] این مرحله شامل اصلاح تابع `list_sources` در فایل `backend/app/api/routes/sources.py` است. هدف، فیلتر کردن لیست منابع OSINT بازگردانده شده است، به طوری که فقط منابع مناسب برای نقش‌ها و سطوح دسترسی کاربر احراز هویت شده قابل مشاهده باشند. این مرحله به طور جداگانه بر روی `list_sources` متمرکز است و سایر endpointها را تحت تأثیر قرار نمی‌دهد.
— [merged] این مرحله بر روی اصلاح تابع `get_source` در فایل `backend/app/api/routes/sources.py` تمرکز دارد. هدف، اطمینان از این است که وقتی کاربر یک منبع خاص را درخواست می‌کند، دسترسی فقط در صورتی اعطا شود که نقش‌ها و سطوح دسترسی او اجازه مشاهده آن منبع خاص را بدهد. در صورت عدم مجوز، یک خطای مناسب (مثلاً 403 Forbidden) باید بازگردانده شود.
**Excerpt:**
```
The `list_articles` endpoint in `backend/app/api/routes/encyclopedia.py` ... currently only check for user authentication (`get_current_user`) but do not enforce any role-based access control (RBAC) or classification level filtering. This means any authenticated user, regardless of their assigned role or clearance level, can view all encyclopedia articles and all OSINT sources, potentially exposing sensitive or classified information to unauthorized individuals.
```

### Step 2: افزودن تست‌های واحد/یکپارچه‌سازی برای اعمال RBAC/Classification
**Status:** `pending` (0%)
**Scope:** این مرحله شامل ایجاد یا به‌روزرسانی تست‌های واحد و/یا یکپارچه‌سازی برای تأیید عملکرد صحیح فیلترینگ RBAC و سطح دسترسی تازه پیاده‌سازی شده برای endpointهای `list_articles`، `list_sources` و `get_source` است. تست‌ها باید سناریوهای دسترسی مجاز و غیرمجاز، نقش‌های مختلف و سطوح دسترسی گوناگون را پوشش دهند.
**Excerpt:**
```
The `list_articles` endpoint in `backend/app/api/routes/encyclopedia.py` and the `list_sources`, `get_source` endpoints in `backend/app/api/routes/sources.py` currently only check for user authentication (`get_current_user`) but do not enforce any role-based access control (RBAC) or classification level filtering. This means any authenticated user, regardless of their assigned role or clearance level, can view all encyclopedia articles and all OSINT sources, potentially exposing sensitive or classified information to unauthorized individuals.
```

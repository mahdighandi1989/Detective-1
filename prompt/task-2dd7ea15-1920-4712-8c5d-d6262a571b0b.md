...
updated_at: '2026-06-13T17:58:47.903825+00:00'
target_files:
...
1. A user with `VIEWER` role and `UNCLASSIFIED` clearance can only see `UNCLASSIFIED` articles.
2. A user with `ANALYST` role and `SECRET` clearance can see `UNCLASSIFIED`, `RESTRICTED`, `CONFIDENTIAL`, `SECRET` articles.
3. Only users with `ANALYST` or `ADMIN` roles can create/update sources.
4. Users with `VIEWER` role can list and get sources, but cannot modify them.

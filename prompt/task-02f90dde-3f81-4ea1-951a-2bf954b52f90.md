...
updated_at: '2026-06-13T17:59:49.074943+00:00'
target_files:
...
1. A single Python enum definition for `RiskCategory` is used by all backend models, schemas, and API routes.
2. A single Python enum definition for `ClassificationLevel` is used by all backend models, schemas, and API routes.
3. Frontend TypeScript types for risk and classification match the canonical backend definitions.
4. All UI components (graph, persons list, encyclopedia form) correctly display and interact with the standardized enum values and associated colors.
5. RBAC logic correctly applies the canonical `ClassificationLevel` for access control.

---
task_id: 02f90dde-3f81-4ea1-951a-2bf954b52f90
title: Widespread Enum Inconsistencies for Risk Classification and Confidentiality Levels
type: bug
priority: critical
...
created_at: '2026-06-13T17:59:49.074940+00:00'
updated_at: '2026-06-13T20:16:20.241211+00:00'
target_files:
...
# Widespread Enum Inconsistencies for Risk Classification and Confidentiality Levels

## Raw Idea

The project suffers from severe inconsistencies in defining and using enums for 'Risk Classification' and 'Confidentiality Levels' across both frontend and backend components. Multiple files define their own versions of these critical enums, leading to conflicting string values, different numeric mappings, and disparate color codes. This makes cross-stack communication unreliable, complicates RBAC enforcement, and causes visual inconsistencies in the UI. For example, 'suspect' vs 'suspicious', 'transformed' vs 'degenerated', 'public' vs 'unclassified' vs 'internal', and varying hex color codes for the same logical risk level.
...
- `frontend/src/app/graph/page.tsx:26-27, 57-91, 93-107, 132-175, 309-315, 348-350` — `RiskLevel, RISK_META, normalizeRisk` — Frontend graph page's risk enum and its metadata.
  ```tsx
  ...
  ```
- `frontend/src/app/persons/page.tsx:11-16, 60-98, 100-106, 350-372` — `RiskCategory, RISK_META, CLASSIFICATION_LABEL` — Frontend persons page's risk and classification enums and their metadata.
  ```tsx
  export type RiskCategory = 'clean' | 'suspect' | 'infiltrator' | 'transformed' | 'unknown';
  const RISK_META: Record<RiskCategory, { ... }> = { ... };
  const CLASSIFICATION_LABEL: Record<PersonSummary['classification_level'], string> = { ... };
  ```
- `backend/app/models/person.py:50-58, 60-67, 90, 116-120` — `RiskCategory, RiskLevel, ClassificationLevel` — Person model's risk enums and imported classification enum.
  ```python
  class RiskCategory(str, enum.Enum): ...
  class RiskLevel(str, enum.Enum): ...
  from app.core.enums import ClassificationLevel
  ```
- `backend/app/models/risk_assessment.py:67-77, 80-86, 106` — `RiskLevel, AssessmentType` — Risk assessment model's risk and assessment type enums.
  ```python
  class RiskLevel(str, enum.Enum): ...
  class AssessmentType(str, enum.Enum): ...
  risk_level: Mapped[RiskLevel] = mapped_column(SAEnum(RiskLevel), nullable=False)
  ```
- `backend/app/models/user.py:85-91, 165-175` — `ClearanceLevel, can_access` — User model's canonical clearance level enum and access logic.
  ```python
  class ClearanceLevel(str, enum.Enum): ...
  def can_access(self, required: "ClearanceLevel") -> bool: ...
  ```
- `backend/app/api/routes/graph.py:36-44, 47-53, 88, 134-143` — `RiskLevel, RISK_COLOR_MAP, _normalize_risk` — Backend graph route's risk enum and color mapping.
  ```python
  class RiskLevel(str, Enum): ...
  RISK_COLOR_MAP: dict[RiskLevel, str] = { ... }
  def _normalize_risk(value: Any) -> RiskLevel: ...
  ```
- `backend/app/api/routes/risk.py:53-60, 63-68, 80-81` — `RiskCategory, RISK_COLOR_MAP` — Backend risk route's risk category enum and color mapping.
  ```python
  class RiskCategory(str, enum.Enum): ...
  RISK_COLOR_MAP: dict[str, str] = { ... }
  ```
- `backend/app/services/__init__.py:44-55, 57-66` — `RiskCategory, color property` — Services package's risk category enum and color property.
  ```python
...
FastAPI, SQLAlchemy ORM, Pydantic, Next.js React components, TypeScript.

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `backend/app/api/deps.py` (سطر 34) — Imports and uses ClassificationLevel and UserRole, with fallback logic.
- `backend/app/api/routes/encyclopedia.py` (سطر 66) — Defines its own ClassificationLevel and uses it for article ingestion.
- `backend/app/api/routes/risk_assessment.py` (سطر 76) — Defines its own RiskCategory and RiskLevel enums with color mappings.
- `backend/app/core/enums.py` — Intended single source of truth for enums, but not consistently used.
...
## 🌐 نقشهٔ وابستگی‌ها
These enums are fundamental to data modeling, API contracts, UI rendering, and authorization logic across the entire application. Changes will impact almost every layer.
...
- [ ] A single Python enum definition for `RiskCategory` is used by all backend models, schemas, and API routes.
- [ ] A single Python enum definition for `ClassificationLevel` is used by all backend models, schemas, and API routes.
- [ ] Frontend TypeScript types for risk and classification match the canonical backend definitions.
- [ ] All UI components (graph, persons list, encyclopedia form) correctly display and interact with the standardized enum values and associated colors.
- [ ] RBAC logic correctly applies the canonical `ClassificationLevel` for access control.
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. Establish a single source of truth for all critical enums (RiskCategory, ClassificationLevel) in the backend (e.g., `app.core.enums.py` or a dedicated `app.models.enums.py`). All other backend modules (models, API routes, services) must import and use these canonical enums. The frontend interfaces and mappings must then be updated to strictly adhere to these canonical definitions, including string values and associated metadata like colors. This will require a comprehensive refactor to ensure consistency.

## 💡 نمونه‌های قبل/بعد
**Standardize RiskCategory enum (example)**
...
```
class RiskCategory(str, enum.Enum):
    SUSPICIOUS = "suspicious" # Consistent casing and value
```

**Standardize ClassificationLevel enum (example)**
...
_بعد:_
```
type Classification = 'unclassified' | 'restricted' | 'confidential' | 'secret' | 'top_secret' | 'internal';
```
...
## 🧪 دستورات اعتبارسنجی
- `Review all enum definitions and their usages in code.`
- `Run `npm run dev` and manually verify UI elements (graph node colors, person risk badges, encyclopedia classification dropdowns).`
- `Run `pytest backend/tests/` to ensure no existing tests break due to enum changes.`

## ⚠️ ریسک‌ها و موارد احتیاط
High risk of breaking existing functionality due to widespread changes. Requires careful, phased refactoring and extensive testing. Potential for data migration if enum values stored in DB change.
...
1. A single Python enum definition for `RiskCategory` is used by all backend models, schemas, and API routes. _(verify: static)_
2. A single Python enum definition for `ClassificationLevel` is used by all backend models, schemas, and API routes. _(verify: static)_
3. Frontend TypeScript types for risk and classification match the canonical backend definitions. _(verify: static)_
4. All UI components (graph, persons list, encyclopedia form) correctly display and interact with the standardized enum values and associated colors. _(verify: static)_
5. RBAC logic correctly applies the canonical `ClassificationLevel` for access control. _(verify: static)_

## Task Steps

### Step 1: Initial Discovery and Inventory of Existing Enum Definitions
**Status:** `pending` (0%)
**Scope:** این مرحله شامل شناسایی و مستندسازی تمام تعاریف موجود برای 'Risk Classification' و 'Confidentiality Levels' در سراسر codebase است. هدف، جمع‌آوری اطلاعات دقیق در مورد مقادیر رشته‌ای، نگاشت‌های عددی و کدهای رنگی مرتبط با هر تعریف، هم در بخش فرانت‌اند و هم در بک‌اند است. این مرحله صرفاً جمع‌آوری اطلاعات است و هیچ تغییری در کد ایجاد نمی‌کند.
**Excerpt:**
```
The project suffers from severe inconsistencies in defining and using enums for 'Risk Classification' and 'Confidentiality Levels' across both frontend and backend components. Multiple files define their own versions of these critical enums, leading to conflicting string values, different numeric mappings, and disparate color codes. This makes cross-stack communication unreliable, complicates RBAC enforcement, and causes visual inconsistencies in the UI. For example, 'suspect' vs 'suspicious', 'transformed' vs 'degenerated', 'public' vs 'unclassified' vs 'internal', and varying hex color codes for the same logical risk level.
```

### Step 2: Define Canonical 'Risk Classification' Enum
**Status:** `pending` (0%)
**Scope:** این مرحله بر ایجاد یک تعریف واحد و معتبر برای enum 'Risk Classification' تمرکز دارد. این تعریف شامل مقادیر رشته‌ای استاندارد شده، نگاشت‌های عددی و کدهای رنگی هگز مرتبط است. هدف، ایجاد 'منبع حقیقت' برای Risk Classification است که تمام ناهماهنگی‌های فعلی را حل کند و مبنایی برای رفتار و UI سازگار سیستم باشد. این مرحله شامل پیاده‌سازی کد یا مهاجرت کامپوننت‌های موجود نیست.
**Excerpt:**
```
The project suffers from severe inconsistencies in defining and using enums for 'Risk Classification' and 'Confidentiality Levels' across both frontend and backend components. Multiple files define their own versions of these critical enums, leading to conflicting string values, different numeric mappings, and disparate color codes. ... For example, 'suspect' vs 'suspicious', 'transformed' vs 'degenerated', ... and varying hex color codes for the same logical risk level.
```

### Step 3: Define Canonical 'Confidentiality Levels' Enum
**Status:** `pending` (0%)
**Scope:** این مرحله بر ایجاد یک تعریف واحد و معتبر برای enum 'Confidentiality Levels' تمرکز دارد. این تعریف شامل مقادیر رشته‌ای استاندارد شده، نگاشت‌های عددی و کدهای رنگی هگز مرتبط است. هدف، ایجاد 'منبع حقیقت' برای Confidentiality Levels است که تمام ناهماهنگی‌های فعلی را حل کند و مبنایی برای رفتار و UI سازگار سیستم، به ویژه در مورد دسترسی به داده‌ها و RBAC باشد. این مرحله شامل پیاده‌سازی کد یا مهاجرت کامپوننت‌های موجود نیست.
**Excerpt:**
```
The project suffers from severe inconsistencies in defining and using enums for 'Risk Classification' and 'Confidentiality Levels' across both frontend and backend components. Multiple files define their own versions of these critical enums, leading to conflicting string values, different numeric mappings, and disparate color codes. ... For example, 'public' vs 'unclassified' vs 'internal', and varying hex color codes for the same logical risk level.
```

### Step 4: Implement Shared Backend Enum Module/Utility
**Status:** `pending` (0%)
**Scope:** این مرحله شامل ایجاد یک ماژول یا فایل ابزاری مشترک در بک‌اند است که تعاریف enum canonical 'Risk Classification' و 'Confidentiality Levels' را در خود جای می‌دهد. این فایل به عنوان یک مکان مرکزی برای دسترسی تمام سرویس‌های بک‌اند به تعاریف استاندارد شده عمل خواهد کرد. این مرحله صرفاً بر ایجاد این artifact قابل استفاده مجدد تمرکز دارد.
**Excerpt:**
```
The project suffers from severe inconsistencies in defining and using enums for 'Risk Classification' and 'Confidentiality Levels' across both frontend and backend components. Multiple files define their own versions of these critical enums, leading to conflicting string values, different numeric mappings, and disparate color codes.
```

### Step 5: Migrate Backend Components to Use Canonical 'Risk Classification' Enum
**Status:** `pending` (0%)
**Scope:** این مرحله شامل بازسازی تمام کامپوننت‌های بک‌اند است که در حال حاضر تعاریف enum 'Risk Classification' را تعریف یا استفاده می‌کنند. هدف، import و استفاده از enum canonical تازه تعریف شده از ماژول مشترک بک‌اند است. این کار شامل به‌روزرسانی منطق موجود بک‌اند، تعاملات پایگاه داده و نقاط پایانی API برای اطمینان از استفاده مداوم از منبع حقیقت واحد برای Risk Classification است.
— [merged] این مرحله شامل بازسازی تمام کامپوننت‌های فرانت‌اند است که در حال حاضر تعاریف enum 'Risk Classification' را تعریف یا استفاده می‌کنند. هدف، import و استفاده از enum canonical تازه تعریف شده از ماژول مشترک فرانت‌اند است. این کار شامل به‌روزرسانی کامپوننت‌های UI، منطق نمایش داده و ورودی‌های فرم برای اطمینان از استفاده مداوم از منبع حقیقت واحد برای Risk Classification است.
**Excerpt:**
```
The project suffers from severe inconsistencies in defining and using enums for 'Risk Classification' and 'Confidentiality Levels' across both frontend and backend components. Multiple files define their own versions of these critical enums, leading to conflicting string values, different numeric mappings, and disparate color codes. This makes cross-stack communication unreliable, complicates RBAC enforcement...
```

### Step 6: Migrate Backend Components to Use Canonical 'Confidentiality Levels' Enum
**Status:** `pending` (0%)
**Scope:** این مرحله شامل بازسازی تمام کامپوننت‌های بک‌اند است که در حال حاضر تعاریف enum 'Confidentiality Levels' را تعریف یا استفاده می‌کنند. هدف، import و استفاده از enum canonical تازه تعریف شده از ماژول مشترک بک‌اند است. این کار شامل به‌روزرسانی منطق موجود بک‌اند، تعاملات پایگاه داده و نقاط پایانی API برای اطمینان از استفاده مداوم از منبع حقیقت واحد برای Confidentiality Levels است.
— [merged] این مرحله شامل بازسازی تمام کامپوننت‌های فرانت‌اند است که در حال حاضر تعاریف enum 'Confidentiality Levels' را تعریف یا استفاده می‌کنند. هدف، import و استفاده از enum canonical تازه تعریف شده از ماژول مشترک فرانت‌اند است. این کار شامل به‌روزرسانی کامپوننت‌های UI، منطق نمایش داده و ورودی‌های فرم برای اطمینان از استفاده مداوم از منبع حقیقت واحد برای Confidentiality Levels است.
**Excerpt:**
```
The project suffers from severe inconsistencies in defining and using enums for 'Risk Classification' and 'Confidentiality Levels' across both frontend and backend components. Multiple files define their own versions of these critical enums, leading to conflicting string values, different numeric mappings, and disparate color codes. This makes cross-stack communication unreliable, complicates RBAC enforcement...
```

### Step 7: Implement Shared Frontend Enum Module/Utility
**Status:** `pending` (0%)
**Scope:** این مرحله شامل ایجاد یک ماژول یا فایل ابزاری مشترک در فرانت‌اند است که تعاریف enum canonical 'Risk Classification' و 'Confidentiality Levels' را در خود جای می‌دهد. این ماژول باید با فریم‌ورک‌های فرانت‌اند (مانند TypeScript enums یا JavaScript objects) سازگار باشد و به عنوان یک منبع مرکزی برای کامپوننت‌های UI عمل کند. این مرحله صرفاً بر ایجاد این artifact قابل استفاده مجدد تمرکز دارد.
**Excerpt:**
```
The project suffers from severe inconsistencies in defining and using enums for 'Risk Classification' and 'Confidentiality Levels' across both frontend and backend components. Multiple files define their own versions of these critical enums, leading to conflicting string values, different numeric mappings, and disparate color codes.
```

### Step 8: Update UI for Consistent Display of Enum Values and Colors
**Status:** `pending` (0%)
**Scope:** این مرحله شامل بررسی و به‌روزرسانی تمام عناصر UI است که 'Risk Classification' و 'Confidentiality Levels' را نمایش می‌دهند. هدف، اطمینان از استفاده از مقادیر رشته‌ای استاندارد شده و کدهای رنگی هگز صحیح از enumهای canonical است تا ناهماهنگی‌های بصری از بین برود. این مرحله بر لایه ارائه بصری تمرکز دارد.
**Excerpt:**
```
Multiple files define their own versions of these critical enums, leading to conflicting string values, different numeric mappings, and disparate color codes. ... and causes visual inconsistencies in the UI. For example, ... varying hex color codes for the same logical risk level.
```

### Step 9: Verify and Update RBAC Enforcement Logic
**Status:** `pending` (0%)
**Scope:** این مرحله شامل بررسی و به‌روزرسانی تمام منطق اجرای کنترل دسترسی مبتنی بر نقش (RBAC) است. هدف، اطمینان از اینکه RBAC به درستی مجوزها را بر اساس enumهای canonical 'Risk Classification' و 'Confidentiality Levels' تفسیر و اعمال می‌کند. این مرحله به پیامدهای امنیتی می‌پردازد و اطمینان می‌دهد که قوانین دسترسی به طور مداوم در سراسر سیستم با استفاده از مقادیر enum استاندارد شده اعمال می‌شوند.
**Excerpt:**
```
This makes cross-stack communication unreliable, complicates RBAC enforcement, and causes visual inconsistencies in the UI.
```

### Step 10: Implement Unit and Integration Tests for Enum Consistency
**Status:** `pending` (0%)
**Scope:** این مرحله شامل افزودن تست‌های واحد و یکپارچه‌سازی جامع برای تأیید سازگاری enumهای 'Risk Classification' و 'Confidentiality Levels' در سراسر کامپوننت‌های بک‌اند و فرانت‌اند است. همچنین، این تست‌ها باید اطمینان حاصل کنند که ارتباطات cross-stack به طور قابل اعتماد کار می‌کند. این مرحله بر تضمین صحت و استحکام تغییرات از طریق تست خودکار تمرکز دارد.
**Excerpt:**
```
The project suffers from severe inconsistencies in defining and using enums for 'Risk Classification' and 'Confidentiality Levels' across both frontend and backend components. Multiple files define their own versions of these critical enums, leading to conflicting string values, different numeric mappings, and disparate color codes. This makes cross-stack communication unreliable...
```

### Step 11: Remove Obsolete Enum Definitions and Code
**Status:** `pending` (0%)
**Scope:** این مرحله شامل شناسایی و حذف تمام تعاریف enum اضافی، منسوخ یا ناسازگار و کد مرتبط است که پس از مهاجرت به enumهای canonical دیگر استفاده نمی‌شوند. این یک مرحله پاکسازی است تا اطمینان حاصل شود که codebase عاری از ناهماهنگی‌های قدیمی و منابع بالقوه خطاهای آینده است.
**Excerpt:**
```
Multiple files define their own versions of these critical enums, leading to conflicting string values, different numeric mappings, and disparate color codes.
```

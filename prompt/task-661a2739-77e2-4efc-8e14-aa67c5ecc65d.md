---
task_id: 661a2739-77e2-4efc-8e14-aa67c5ecc65d
...
created_at: '2026-06-13T18:10:52.099458+00:00'
updated_at: '2026-06-13T20:19:07.705356+00:00'
---
...
## 🔍 جزئیات
- علت: field finally has different defaults: ['Annotated[AsyncSession, Depends(get_db)]', 'Depends(get_async_db)):', 'None  # type: ignore']
---
[scan #2 at 2026-06-13T18:10:52.138807+00:00]
## 📋 شرح
فیلد `user` در `` در دو یا چند جای مختلف default value متفاوت دارد.

## 🤔 چرا مهم است
defaults متناقض باعث می‌شود رفتار سیستم به ترتیب اجرا/import وابسته شود — bug های غیرقابل reproduce.

## 🔍 جزئیات
- علت: field user has different defaults: ['Depends(require_roles(*WRITE_ROLES)),', 'Depend
---
[scan #3 at 2026-06-13T18:10:52.157497+00:00]
## 📋 شرح
فیلد `payload` در `` در دو یا چند جای مختلف default value متفاوت دارد.

## 🤔 چرا مهم است
defaults متناقض باعث می‌شود رفتار سیستم به ترتیب اجرا/import وابسته شود — bug های غیرقابل reproduce.

## 🔍 جزئیات
- علت: field payload has different defaults: ['Depends(get_async_db),', 'Depends(get_db)
---
[scan #4 at 2026-06-13T18:10:52.176977+00:00]
## 📋 شرح
فیلد `title` در `` در دو یا چند جای مختلف default value متفاوت دارد.

## 🤔 چرا مهم است
defaults متناقض باعث می‌شود رفتار سیستم به ترتیب اجرا/import وابسته شود — bug های غیرقابل reproduce.

## 🔍 جزئیات
- علت: field title has different defaults: ['mapped_column(String)', 'Field(..., min_lengt
---
[scan #5 at 2026-06-13T18:10:52.199204+00:00]
## 📋 شرح
فیلد `content` در `` در دو یا چند جای مختلف default value متفاوت دارد.

## 🤔 چرا مهم است
defaults متناقض باعث می‌شود رفتار سیستم به ترتیب اجرا/import وابسته شود — bug های غیرقابل reproduce.

## 🔍 جزئیات
- علت: field content has different defaults: ['Field(..., min_length=1, description="Raw
---
[scan #6 at 2026-06-13T18:10:52.218963+00:00]
## 📋 شرح
فیلد `form` در `` در دو یا چند جای مختلف default value متفاوت دارد.

## 🤔 چرا مهم است
defaults متناقض باعث می‌شود رفتار سیستم به ترتیب اجرا/import وابسته شود — bug های غیرقابل reproduce.

## 🔍 جزئیات
- علت: field form has different defaults: ['Field(', 'None']
...
2. تست fixture رفتار پیش‌فرض را تأیید می‌کند _(verify: static)_

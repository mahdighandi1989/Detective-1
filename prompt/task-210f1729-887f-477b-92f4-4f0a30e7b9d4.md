---
task_id: 210f1729-887f-477b-92f4-4f0a30e7b9d4
title: ارجاع نادرست Foreign Key در مدل RiskAssessment به جدول Person
type: bug
priority: critical
...
created_at: '2026-06-13T18:04:48.196107+00:00'
updated_at: '2026-06-13T18:04:48.196110+00:00'
target_files:
- backend/app/models/risk_assessment.py
---

# ارجاع نادرست Foreign Key در مدل RiskAssessment به جدول Person

## Raw Idea

در مدل `RiskAssessment`، ستون `person_id` به عنوان Foreign Key به `person.id` ارجاع داده شده است. با این حال، نام جدول مدل `Person` در `backend/app/models/person.py` به صورت `__tablename__ = "persons"` (جمع) تعریف شده است. این ناسازگاری در نام جدول باعث شکست migrationهای دیتابیس یا خطاهای زمان اجرا هنگام تلاش برای ایجاد یا بازیابی ارزیابی‌های ریسک می‌شود.
...
- `backend/app/models/risk_assessment.py:100-102` — Foreign Key نادرست به 'person.id'
  ```python
...
SQLAlchemy ORM, PostgreSQL Foreign Keys, Alembic Migrations

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `backend/app/models/person.py` (سطر 99) — مدل Person که نام جدول آن 'persons' است.
- `backend/alembic/versions/0001_initial.py` (سطر N/A) — فایل migration اولیه که ممکن است نیاز به به‌روزرسانی داشته باشد.
...
- `backend/app/api/routes/risk.py` — این فایل `risk_assessment.py` را import می‌کند (caller)

## 🌐 نقشهٔ وابستگی‌ها
این تغییر بر روی ساختار دیتابیس و قابلیت ایجاد و مدیریت ارزیابی‌های ریسک تأثیر می‌گذارد.
...
- [ ] migrationهای Alembic بدون خطا اجرا شوند.
- [ ] ارزیابی‌های ریسک جدید با موفقیت ایجاد شوند و به اشخاص مرتبط شوند.
- [ ] بازیابی ارزیابی‌های ریسک برای یک شخص به درستی کار کند.
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. تعریف Foreign Key در مدل `RiskAssessment` باید اصلاح شود تا به نام صحیح جدول `persons` ارجاع دهد.
...
## 🧪 دستورات اعتبارسنجی
- `alembic revision --autogenerate -m "fix_risk_assessment_fk"`
- `alembic upgrade head`
- `pytest backend/tests/test_risk.py`

## ⚠️ ریسک‌ها و موارد احتیاط
احتمال شکست migrationهای موجود در صورت عدم دقت در اصلاح.
...
1. migrationهای Alembic بدون خطا اجرا شوند.
2. ارزیابی‌های ریسک جدید با موفقیت ایجاد شوند و به اشخاص مرتبط شوند.
3. بازیابی ارزیابی‌های ریسک برای یک شخص به درستی کار کند.

---
task_id: ca521c32-060e-4777-8a10-4dfe69fa4ef4
title: اعتبارسنجی ناکافی URL در مدل Source
type: bug
priority: medium
...
created_at: '2026-06-13T18:15:53.942463+00:00'
updated_at: '2026-06-13T20:20:21.191901+00:00'
target_files:
- backend/app/models/source.py
- backend/app/api/routes/sources.py
---

# اعتبارسنجی ناکافی URL در مدل Source

## Raw Idea

فیلد `url` در مدل SQLAlchemy `Source` (`backend/app/models/source.py`) به‌عنوان `Text` (یک نوع رشته) تعریف شده است و استفاده از آن در `backend/app/api/routes/sources.py` (مثلاً در `create_source`) `payload.url` را به `str` تبدیل می‌کند. این رویکرد فاقد اعتبارسنجی قوی برای فرمت URL است و به‌طور بالقوه اجازه می‌دهد URLهای بدشکل یا نامعتبر در پایگاه داده ذخیره شوند. ذخیره URLهای نامعتبر می‌تواند منجر به لینک‌های شکسته، خطاها در سرویس‌های پایین‌دستی (مانند عامل‌های OSINT که سعی در اسکرپ کردن آن‌ها دارند) یا رفتار غیرمنتظره شود. نوع `HttpUrl` Pydantic اعتبارسنجی داخلی را برای این منظور ارائه می‌دهد.
...
- `backend/app/models/source.py:117-119` — `Source.url` — تعریف فیلد `url` در مدل SQLAlchemy.
  ```python
  url: Mapped[str] = mapped_column(
          Text, unique=True, index=True, nullable=False, comment="آدرس URL دقیق منبع"
      )
  ```
- `backend/app/api/routes/sources.py:158-159` — `create_source` — تبدیل `payload.url` به رشته در زمان ایجاد منبع.
  ```python
...
- `backend/app/schemas/source.py` — اسکیماهای Pydantic `SourceCreate` و `SourceUpdate` باید برای اعتبارسنجی URL به‌روزرسانی شوند.
- `backend/app/services/osint_agent.py` — این سرویس URLها را مصرف می‌کند و از صحت آن‌ها بهره‌مند می‌شود.
- `backend/app/services/credibility.py` — از URL برای عامل اعتبار دامنه استفاده می‌کند.
- `backend/app/models/relationship.py` — `source.py` این فایل را import می‌کند
- `backend/app/db/base_class.py` — `source.py` این فایل را import می‌کند
- `backend/app/db/base.py` — `source.py` این فایل را import می‌کند
- `backend/app/models/person.py` — `source.py` این فایل را import می‌کند
- `backend/app/api/routes/sources.py` — این فایل `source.py` را import می‌کند (caller)
- `backend/app/models/__init__.py` — این فایل `source.py` را import می‌کند (caller)

## 🌐 نقشهٔ وابستگی‌ها
بر کیفیت داده‌ها برای منابع و قابلیت اطمینان سرویس‌هایی که URLهای منبع را مصرف می‌کنند، تأثیر می‌گذارد.
...
- [ ] اسکیماهای `SourceCreate` و `SourceUpdate` از `pydantic.HttpUrl` برای فیلد `url` استفاده می‌کنند.
- [ ] تلاش برای ایجاد یک منبع با URL نامعتبر (مثلاً 'not-a-url') منجر به خطای 422 Unprocessable Entity می‌شود.
- [ ] URLهای معتبر به‌درستی در پایگاه داده ذخیره می‌شوند.
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. اسکیماهای Pydantic `SourceCreate` و `SourceUpdate` (که در `backend/app/schemas/source.py` قرار دارند، اما ارائه نشده‌اند) را به‌روزرسانی کنید تا از `pydantic.HttpUrl` برای فیلد `url` استفاده کنند. اطمینان حاصل کنید که مدل SQLAlchemy می‌تواند نمایش رشته‌ای `HttpUrl` را به‌درستی ذخیره کند.

## 💡 نمونه‌های قبل/بعد
**استفاده از Pydantic HttpUrl برای اعتبارسنجی**
...
## 🧪 دستورات اعتبارسنجی
- `pytest backend/tests/test_sources.py (اضافه کردن موارد تست جدید برای URLهای نامعتبر)`
- `به‌طور دستی endpoint API `POST /api/v1/sources` را با URLهای معتبر و نامعتبر تست کنید.`

## ⚠️ ریسک‌ها و موارد احتیاط
جزئی، عمدتاً مربوط به یکپارچگی داده‌ها.
...
1. اسکیماهای `SourceCreate` و `SourceUpdate` از `pydantic.HttpUrl` برای فیلد `url` استفاده می‌کنند. _(verify: static)_
2. تلاش برای ایجاد یک منبع با URL نامعتبر (مثلاً 'not-a-url') منجر به خطای 422 Unprocessable Entity می‌شود. _(verify: static)_
3. URLهای معتبر به‌درستی در پایگاه داده ذخیره می‌شوند. _(verify: static)_

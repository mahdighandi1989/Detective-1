...
updated_at: '2026-06-13T17:59:49.078686+00:00'
target_files:
...
1. A user with `VIEWER` role and `UNCLASSIFIED` clearance cannot access a `SECRET` person profile.
2. A user with `ANALYST` role and `SECRET` clearance can access a `SECRET` person profile.
3. The `_can_access_classification` function correctly uses `user.clearance`.

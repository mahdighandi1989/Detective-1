---
task_id: 3151b8b6-947e-4e2c-a9af-4f3b5bb2de62
title: 'RBAC Bypass: Incorrect Classification Level Check in Persons API'
type: security
priority: critical
...
created_at: '2026-06-13T17:59:49.078684+00:00'
updated_at: '2026-06-13T20:16:25.060793+00:00'
target_files:
- backend/app/api/routes/persons.py
---

# RBAC Bypass: Incorrect Classification Level Check in Persons API

## Raw Idea

The `_can_access_classification` helper function in `backend/app/api/routes/persons.py` is intended to enforce access control based on a user's clearance level and a person's classification. However, it attempts to read `user.clearance_level` (line 106) which does not exist on the `User` model. The correct attribute is `user.clearance`. This likely causes `getattr` to return the default `ClassificationLevel.PUBLIC`, effectively bypassing the intended classification checks for all users. This allows unauthorized users to potentially view sensitive person profiles (e.g., `SECRET`, `TOP_SECRET`) that they are not cleared for.
...
- `backend/app/api/routes/persons.py:104-114` — `_can_access_classification` — The `user_clearance` assignment is incorrect.
  ```python
...
FastAPI, SQLAlchemy ORM, RBAC.

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `backend/app/models/user.py` (سطر 165) — Defines the `User` model with `clearance` attribute.
- `backend/app/core/enums.py` — Defines the canonical `ClassificationLevel`.
- `backend/app/api/routes/persons.py` (سطر 174) — Calls `_can_access_classification` for filtering.
- `frontend/src/app/persons/page.tsx` (سطر 100) — Displays `classification_level` and relies on backend filtering.
...
## 🌐 نقشهٔ وابستگی‌ها
This helper is used by `list_persons` and `get_person` endpoints. A fix will correctly enforce access control for person profiles.
...
- [ ] A user with `VIEWER` role and `UNCLASSIFIED` clearance cannot access a `SECRET` person profile.
- [ ] A user with `ANALYST` role and `SECRET` clearance can access a `SECRET` person profile.
- [ ] The `_can_access_classification` function correctly uses `user.clearance`.
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. Correct the attribute access in `_can_access_classification` from `user.clearance_level` to `user.clearance`. Ensure the `ClassificationLevel` enum used in this helper is consistent with `app.core.enums.ClassificationLevel`.

## 💡 نمونه‌های قبل/بعد
**Correct `user_clearance` retrieval**
...
## 🧪 دستورات اعتبارسنجی
- `Manual test: Log in as a low-clearance user, try to access a high-classification person profile. It should return 403 Forbidden.`
- `Add unit/integration tests for `backend/app/api/routes/persons.py` covering classification access.`

## ⚠️ ریسک‌ها و موارد احتیاط
Incorrectly restricting access for authorized users if the fix is flawed.
...
1. A user with `VIEWER` role and `UNCLASSIFIED` clearance cannot access a `SECRET` person profile. _(verify: static)_
2. A user with `ANALYST` role and `SECRET` clearance can access a `SECRET` person profile. _(verify: static)_
3. The `_can_access_classification` function correctly uses `user.clearance`. _(verify: static)_

---
task_id: 3eaa56fe-2b43-448c-8254-649baf17930b
title: ذخیره‌سازی تاریخچهٔ تغییرات (Audit Log) به صورت JSON در مدل RiskAssessment
type: refactor
priority: high
...
created_at: '2026-06-13T18:10:52.063428+00:00'
updated_at: '2026-06-13T20:18:59.772942+00:00'
target_files:
- backend/app/models/risk_assessment.py
---

# ذخیره‌سازی تاریخچهٔ تغییرات (Audit Log) به صورت JSON در مدل RiskAssessment

## Raw Idea

مدل `RiskAssessment` دارای فیلد `audit_log` از نوع JSON است که برای ذخیرهٔ تاریخچهٔ تغییرات این ارزیابی استفاده می‌شود. این رویکرد یک ضدالگو در طراحی دیتابیس است. ذخیرهٔ لاگ‌های ممیزی به صورت JSON در یک ستون، قابلیت جستجو، فیلترینگ، گزارش‌گیری و حفظ یکپارچگی داده‌ها را به شدت کاهش می‌دهد. یک مدل `AuditLog` اختصاصی (که در `backend/app/models/audit_log.py` وجود دارد) باید برای ثبت تمام رویدادهای ممیزی در سراسر سیستم استفاده شود و ارزیابی‌های ریسک را از طریق `entity_id` به آن مرتبط کند.
...
- `backend/app/models/risk_assessment.py:133-134` — فیلد audit_log در مدل RiskAssessment
  ```python
...
SQLAlchemy ORM, Data Modeling Best Practices, Audit Logging

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `backend/app/models/audit_log.py` (سطر 116) — مدل اختصاصی AuditLog برای ثبت رویدادهای ممیزی.
- `backend/app/api/routes/risk.py` (سطر 166) — روت‌های API که RiskAssessment را ایجاد/به‌روزرسانی می‌کنند و باید از AuditLog استفاده کنند.
- `backend/app/models/relationship.py` — `risk_assessment.py` این فایل را import می‌کند
- `backend/app/db/base_class.py` — `risk_assessment.py` این فایل را import می‌کند
- `backend/app/db/base.py` — `risk_assessment.py` این فایل را import می‌کند
- `backend/app/models/person.py` — `risk_assessment.py` این فایل را import می‌کند
- `backend/app/api/routes/persons.py` — این فایل `risk_assessment.py` را import می‌کند (caller)

## 🌐 نقشهٔ وابستگی‌ها
این تغییر بر روی مدل‌سازی داده‌ها، نحوهٔ ثبت لاگ‌های ممیزی و قابلیت‌های گزارش‌گیری تأثیر می‌گذارد.
...
- [ ] فیلد `audit_log` از مدل `RiskAssessment` حذف شود.
- [ ] هر عملیات ایجاد یا به‌روزرسانی `RiskAssessment` یک رکورد جدید در مدل `AuditLog` ایجاد کند.
- [ ] قابلیت بازیابی لاگ‌های ممیزی برای یک `RiskAssessment` خاص از طریق مدل `AuditLog` فراهم شود.
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. فیلد `audit_log` از مدل `RiskAssessment` حذف شود. به جای آن، هر زمان که یک `RiskAssessment` ایجاد یا به‌روزرسانی می‌شود، یک رکورد جدید در مدل `AuditLog` ایجاد شود. این رکورد باید شامل `entity_type="RiskAssessment"` و `entity_id=risk_assessment.id` باشد تا ارتباط حفظ شود. این کار امکان ردیابی جامع‌تر و کارآمدتر تغییرات را فراهم می‌کند.

## 💡 نمونه‌های قبل/بعد
**قبل: ذخیره لاگ در JSON**

_قبل:_
```
assessment.audit_log.append({"action": "update", "timestamp": str(datetime.now())})
db.add(assessment)
```

**بعد: استفاده از مدل AuditLog**
...
## 🧪 دستورات اعتبارسنجی
- `alembic revision --autogenerate -m "remove_audit_log_from_risk_assessment"`
- `alembic upgrade head`
- `تست دستی ایجاد/به‌روزرسانی RiskAssessment و بررسی رکوردهای AuditLog در دیتابیس.`

## ⚠️ ریسک‌ها و موارد احتیاط
نیاز به بازنگری منطق ثبت لاگ در روت‌های API و سرویس‌ها.
...
1. فیلد `audit_log` از مدل `RiskAssessment` حذف شود. _(verify: static)_
2. هر عملیات ایجاد یا به‌روزرسانی `RiskAssessment` یک رکورد جدید در مدل `AuditLog` ایجاد کند. _(verify: static)_
3. قابلیت بازیابی لاگ‌های ممیزی برای یک `RiskAssessment` خاص از طریق مدل `AuditLog` فراهم شود. _(verify: static)_

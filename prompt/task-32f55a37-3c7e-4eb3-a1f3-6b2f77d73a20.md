---
task_id: 32f55a37-3c7e-4eb3-a1f3-6b2f77d73a20
title: Redundant and Conflicting Backend Risk Assessment API Routes
type: refactor
priority: high
...
created_at: '2026-06-13T18:08:25.706202+00:00'
updated_at: '2026-06-13T20:17:59.034090+00:00'
target_files:
...
# Redundant and Conflicting Backend Risk Assessment API Routes

## Raw Idea

The backend contains two separate modules, `backend/app/api/routes/risk.py` and `backend/app/api/routes/risk_assessment.py`, that both provide API endpoints for managing risk assessments. They define their own, conflicting `RiskCategory` (or `RiskLevel`) enums and color mappings, and offer overlapping functionality (e.g., evaluating risk, retrieving assessments). This redundancy leads to confusion, inconsistent API design, and makes maintenance and future development more difficult.
...
- `backend/app/api/routes/risk.py:1-267` — `router` — Current risk assessment endpoints.
  ```python
  router = APIRouter(prefix="/risk", tags=["risk"])
  @router.post("/persons/{person_id}/evaluate", ...)
  ```
- `backend/app/api/routes/risk_assessment.py:1-287` — `router` — Current risk assessment endpoints with different prefix and schemas.
  ```python
  router = APIRouter(prefix="/risk-assessments", tags=["risk-assessment"])
  @router.get("", response_model=RiskAssessmentListOut)
  ```
- `backend/app/api/__init__.py:25-26, 40-41` — `api_router` — API router aggregation, currently including both modules.
  ```python
...
- `backend/app/models/risk_assessment.py` — The ORM model used by both, but with inconsistent enum definitions.
- `backend/app/services/risk_engine.py` — The service used by both modules to perform risk assessment.
- `backend/app/schemas/risk_assessment.py` — This file (not provided) would define Pydantic schemas for risk assessments and needs to be unified.
- `backend/app/api/deps.py` — `risk.py` این فایل را import می‌کند
- `backend/app/models/person.py` — `risk.py` این فایل را import می‌کند
- `backend/app/api/routes/__init__.py` — `__init__.py` این فایل را import می‌کند
- `backend/app/main.py` — این فایل `__init__.py` را import می‌کند (caller)

## 🌐 نقشهٔ وابستگی‌ها
This is an internal backend architectural issue. Consolidating these routes will simplify the API, improve maintainability, and ensure consistent behavior for risk assessment operations. It will also simplify the enum standardization effort.
...
- [ ] All risk assessment endpoints are exposed through a single API router module.
- [ ] No duplicate or conflicting endpoints for risk assessment exist.
- [ ] All risk assessment endpoints use the canonical risk classification enum and color mappings.
- [ ] The API documentation (`/docs`) reflects the unified risk assessment API.
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. Consolidate all risk assessment related API routes into a single, unified module (e.g., `backend/app/api/routes/risk_management.py`). Choose one canonical set of enums and color mappings (as part of the broader enum standardization in Finding 1) and apply it consistently across all endpoints in the consolidated module. Refactor the endpoints to provide a clear, non-redundant API surface for risk assessment operations.
...
**Risk assessment endpoint (simplified)**
...
_بعد:_
```
backend/app/api/routes/risk_management.py:
@router.post("/persons/{person_id}/assess", ...)
```
...
## 🧪 دستورات اعتبارسنجی
- `Run `uvicorn app.main:app --reload` and check `/docs` for unified risk endpoints.`
- `Run `pytest backend/tests/` to ensure existing tests for risk assessment are adapted and pass.`

## ⚠️ ریسک‌ها و موارد احتیاط
Requires careful migration of existing endpoint logic and potential merging of schemas. Risk of breaking existing frontend calls if API paths change without corresponding frontend updates.
...
1. All risk assessment endpoints are exposed through a single API router module. _(verify: static)_
2. No duplicate or conflicting endpoints for risk assessment exist. _(verify: static)_
3. All risk assessment endpoints use the canonical risk classification enum and color mappings. _(verify: static)_
4. The API documentation (`/docs`) reflects the unified risk assessment API. _(verify: static)_

## Task Steps

### Step 1: تحلیل و شناسایی تضادها در Enums و نگاشت رنگی
**Status:** `pending` (0%)
**Scope:** این مرحله شامل بررسی دقیق فایل‌های `backend/app/api/routes/risk.py` و `backend/app/api/routes/risk_assessment.py` برای شناسایی تعاریف متضاد یا تکراری Enums مانند `RiskCategory` یا `RiskLevel` و نگاشت‌های رنگی مرتبط با آن‌ها است. هدف، درک کامل تفاوت‌ها و نقاط اشتراک قبل از هرگونه تغییر کد است. خارج از این مرحله، هیچ تغییری در کد یا ایجاد فایل جدید انجام نمی‌شود.
**Excerpt:**
```
The backend contains two separate modules, `backend/app/api/routes/risk.py` and `backend/app/api/routes/risk_assessment.py`, that both provide API endpoints for managing risk assessments. They define their own, conflicting `RiskCategory` (or `RiskLevel`) enums and color mappings, and offer overlapping functionality (e.g., evaluating risk, retrieving assessments). This redundancy leads to confusion, inconsistent API design, and makes maintenance and future development more difficult.
```

### Step 2: تعریف مدل یکپارچه Risk Enum و نگاشت رنگی
**Status:** `pending` (0%)
**Scope:** این مرحله بر ایجاد یک تعریف واحد و مرکزی برای `RiskCategory` (یا `RiskLevel`) Enum و نگاشت‌های رنگی مربوطه تمرکز دارد. این تعریف باید به گونه‌ای باشد که تمام نیازهای هر دو ماژول `risk.py` و `risk_assessment.py` را پوشش دهد و تضادهای شناسایی شده در مرحله قبل را حل کند. مکان این تعریف باید یک ماژول مشترک و قابل دسترس برای هر دو فایل باشد، بدون اینکه مسیر فایل جدیدی اختراع شود مگر اینکه کاربر صراحتاً آن را مشخص کند.
**Excerpt:**
```
They define their own, conflicting `RiskCategory` (or `RiskLevel`) enums and color mappings, and offer overlapping functionality (e.g., evaluating risk, retrieving assessments). This redundancy leads to confusion, inconsistent API design, and makes maintenance and future development more difficult.
```

### Step 3: مهاجرت تعاریف Enum و نگاشت رنگی به مدل یکپارچه
**Status:** `pending` (0%)
**Scope:** این مرحله شامل به‌روزرسانی فایل‌های `backend/app/api/routes/risk.py` و `backend/app/api/routes/risk_assessment.py` برای استفاده از تعریف یکپارچه `RiskCategory` (یا `RiskLevel`) Enum و نگاشت‌های رنگی که در مرحله قبل ایجاد شد، است. تعاریف قدیمی و متضاد باید از این فایل‌ها حذف شده و ارجاعات به آن‌ها با تعریف جدید جایگزین شوند. این کار شامل به‌روزرسانی هرگونه منطق یا مدل داده‌ای است که به این Enums وابسته است.
**Excerpt:**
```
The backend contains two separate modules, `backend/app/api/routes/risk.py` and `backend/app/api/routes/risk_assessment.py`, that both provide API endpoints for managing risk assessments. They define their own, conflicting `RiskCategory` (or `RiskLevel`) enums and color mappings...
```

### Step 4: شناسایی و تحلیل قابلیت‌های API تکراری
**Status:** `pending` (0%)
**Scope:** این مرحله شامل بررسی دقیق API endpoint ها در `backend/app/api/routes/risk.py` و `backend/app/api/routes/risk_assessment.py` برای شناسایی قابلیت‌های تکراری و همپوشان است. به طور خاص، باید به دنبال endpoint هایی بود که وظایفی مانند «ارزیابی ریسک» (evaluating risk) و «بازیابی ارزیابی‌ها» (retrieving assessments) را انجام می‌دهند. هدف، ایجاد یک لیست جامع از تمام endpoint های تکراری و درک دقیق عملکرد هر یک برای برنامه‌ریزی ادغام است.
**Excerpt:**
```
They define their own, conflicting `RiskCategory` (or `RiskLevel`) enums and color mappings, and offer overlapping functionality (e.g., evaluating risk, retrieving assessments). This redundancy leads to confusion, inconsistent API design, and makes maintenance and future development more difficult.
```

### Step 5: پیاده‌سازی API Endpoint های یکپارچه برای قابلیت‌های تکراری
**Status:** `pending` (0%)
**Scope:** این مرحله شامل انتخاب یک ماژول اصلی (مثلاً `backend/app/api/routes/risk_assessment.py` به دلیل نام‌گذاری خاص‌تر) یا ایجاد یک ماژول جدید برای میزبانی API endpoint های یکپارچه است. قابلیت‌های تکراری شناسایی شده در مرحله قبل، مانند «ارزیابی ریسک» و «بازیابی ارزیابی‌ها»، باید به این ماژول منتقل یا در آن بازنویسی شوند تا یک مجموعه واحد و معتبر از endpoint ها برای این عملیات‌ها وجود داشته باشد. این کار شامل انتقال منطق اصلی و به‌روزرسانی مدل‌های داده‌ای مرتبط است.
**Excerpt:**
```
They define their own, conflicting `RiskCategory` (or `RiskLevel`) enums and color mappings, and offer overlapping functionality (e.g., evaluating risk, retrieving assessments). This redundancy leads to confusion, inconsistent API design, and makes maintenance and future development more difficult.
```

### Step 6: به‌روزرسانی فراخوان‌های داخلی به Endpoint های یکپارچه
**Status:** `pending` (0%)
**Scope:** این مرحله شامل شناسایی و به‌روزرسانی تمام نقاط در کد بک‌اند است که قبلاً از API endpoint های تکراری و قدیمی در `backend/app/api/routes/risk.py` یا `backend/app/api/routes/risk_assessment.py` برای قابلیت‌های «ارزیابی ریسک» و «بازیابی ارزیابی‌ها» استفاده می‌کردند. تمام این فراخوان‌ها باید به endpoint های یکپارچه و جدیدی که در مرحله قبل پیاده‌سازی شدند، تغییر مسیر داده شوند. این کار ممکن است شامل جستجوی سراسری در ریپو برای یافتن تمام ارجاعات باشد.
**Excerpt:**
```
The backend contains two separate modules, `backend/app/api/routes/risk.py` and `backend/app/api/routes/risk_assessment.py`, that both provide API endpoints for managing risk assessments. They define their own, conflicting `RiskCategory` (or `RiskLevel`) enums and color mappings, and offer overlapping functionality (e.g., evaluating risk, retrieving assessments). This redundancy leads to confusion, inconsistent API design, and makes maintenance and future development more difficult.
```

### Step 7: حذف Endpoint ها و ماژول‌های تکراری
**Status:** `pending` (0%)
**Scope:** این مرحله شامل حذف کامل API endpoint های تکراری و هرگونه کد مرتبط با آن‌ها از ماژول `backend/app/api/routes/risk.py` (یا ماژولی که به عنوان ثانویه در نظر گرفته شده است) است. پس از اطمینان از اینکه تمام فراخوان‌های داخلی به endpoint های یکپارچه منتقل شده‌اند، این endpoint های قدیمی و هرگونه منطق یا مدل داده‌ای که دیگر استفاده نمی‌شوند، باید حذف شوند. در صورت لزوم، ماژول `backend/app/api/routes/risk.py` نیز می‌تواند به طور کامل حذف یا به یک ماژول با مسئولیت‌های محدودتر تبدیل شود.
**Excerpt:**
```
The backend contains two separate modules, `backend/app/api/routes/risk.py` and `backend/app/api/routes/risk_assessment.py`, that both provide API endpoints for managing risk assessments. They define their own, conflicting `RiskCategory` (or `RiskLevel`) enums and color mappings, and offer overlapping functionality (e.g., evaluating risk, retrieving assessments). This redundancy leads to confusion, inconsistent API design, and makes maintenance and future development more difficult.
```

### Step 8: افزودن/به‌روزرسانی تست‌های واحد و یکپارچه‌سازی
**Status:** `pending` (0%)
**Scope:** این مرحله شامل ایجاد تست‌های واحد جدید برای منطق مربوط به مدل یکپارچه `RiskCategory` (یا `RiskLevel`) و نگاشت‌های رنگی، و همچنین تست‌های واحد برای منطق اصلی API endpoint های یکپارچه است. علاوه بر این، تست‌های یکپارچه‌سازی موجود باید به‌روزرسانی شوند تا از عملکرد صحیح API endpoint های جدید و حذف شده اطمینان حاصل شود. هدف، پوشش کامل تغییرات و جلوگیری از رگرسیون است.
**Excerpt:**
```
This redundancy leads to confusion, inconsistent API design, and makes maintenance and future development more difficult.
```

### Step 9: به‌روزرسانی مستندات API
**Status:** `pending` (0%)
**Scope:** این مرحله شامل به‌روزرسانی جامع مستندات API (مانند OpenAPI/Swagger) برای منعکس کردن تغییرات ایجاد شده در طول فرآیند بازسازی است. این شامل به‌روزرسانی تعاریف مدل `RiskCategory` (یا `RiskLevel`)، نگاشت‌های رنگی، و مهم‌تر از همه، به‌روزرسانی یا حذف توضیحات مربوط به API endpoint های تکراری و اضافه کردن توضیحات کامل برای API endpoint های یکپارچه است. هدف، اطمینان از اینکه مستندات API دقیق و به‌روز هستند.
**Excerpt:**
```
This redundancy leads to confusion, inconsistent API design, and makes maintenance and future development more difficult.
```

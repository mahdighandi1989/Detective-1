---
task_id: 0f90ebdc-c37a-4c83-a2e4-beaf5cc6aa91
title: Missing .env.example files and Undocumented Environment Variables
type: bug
priority: high
...
created_at: '2026-06-13T18:09:06.118887+00:00'
updated_at: '2026-06-13T20:18:26.985698+00:00'
target_files:
- README.md
...
# Missing .env.example files and Undocumented Environment Variables

## Raw Idea

The `README.md` instructs users to copy `backend/.env.example` and `frontend/.env.example` to set up environment variables. However, these example files are not provided in the repository structure. Consequently, critical environment variables referenced in the code, such as `NEXT_PUBLIC_API_BASE_URL` in the frontend and `OPENAI_API_KEY`, `OPENAI_DEFAULT_MODEL`, `OPENAI_EMBEDDING_MODEL` in the backend, are not documented for users. This significantly hinders the setup process, making it difficult for new developers or deployers to configure the application correctly, leading to runtime errors related to missing configuration.
...
- `README.md:16-17` — Instructions referencing non-existent example files.
  ```markdown
  ...
  ```
- `frontend/src/app/encyclopedia/page.tsx:35-37` — Example of frontend environment variable usage.
  ```tsx
  const API_BASE =
    process.env.NEXT_PUBLIC_API_BASE_URL?.replace(//$/, '') ||
    'http://localhost:8000';
  ```
- `backend/app/services/llm_adapter.py:50-56` — Helper function for reading environment variables, indicating their usage.
  ```python
...
Next.js, FastAPI, Environment Variables

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `frontend/src/app/graph/page.tsx` (سطر 109) — Uses NEXT_PUBLIC_API_BASE_URL
- `frontend/src/app/persons/page.tsx` (سطر 53) — Uses NEXT_PUBLIC_API_BASE_URL
- `backend/app/core/config.py` — `llm_adapter.py` این فایل را import می‌کند
- `backend/app/api/routes/encyclopedia.py` — این فایل `llm_adapter.py` را import می‌کند (caller)
- `backend/app/services/__init__.py` — این فایل `llm_adapter.py` را import می‌کند (caller)
- `backend/app/services/osint_agent.py` — این فایل `llm_adapter.py` را import می‌کند (caller)
- `backend/app/workers/tasks.py` — این فایل `llm_adapter.py` را import می‌کند (caller)

## 🌐 نقشهٔ وابستگی‌ها
This affects the entire application's ability to connect to external services (API, LLMs) and run correctly.
...
- [ ] `backend/.env.example` and `frontend/.env.example` files exist.
- [ ] All environment variables referenced in the code are present in the respective example files with clear descriptions.
- [ ] The application can be successfully started and configured by following the updated `README.md` and using the example `.env` files.
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. Create `backend/.env.example` and `frontend/.env.example` files. Populate these files with all necessary environment variables, including `NEXT_PUBLIC_API_BASE_URL`, `OPENAI_API_KEY`, `OPENAI_DEFAULT_MODEL`, `OPENAI_EMBEDDING_MODEL`, and any other variables used by `app.core.config.settings` or `os.getenv` calls. Provide clear comments for each variable's purpose and example values.

## 📤 خروجی مورد انتظار
تغییر کد در فایل‌های مرتبط، commit یا PR جدید با پیام واضح، و عبور تمام معیارهای پذیرش.

## 🧪 دستورات اعتبارسنجی
- `Check for existence of `backend/.env.example` and `frontend/.env.example`.`
- `Manually review example files against code for completeness.`

## ⚠️ ریسک‌ها و موارد احتیاط
None, this is a documentation and configuration improvement.
...
1. `backend/.env.example` and `frontend/.env.example` files exist. _(verify: static)_
2. All environment variables referenced in the code are present in the respective example files with clear descriptions. _(verify: static)_
3. The application can be successfully started and configured by following the updated `README.md` and using the example `.env` files. _(verify: static)_

## Task Steps

### Step 1: ایجاد فایل `backend/.env.example` با متغیرهای محیطی بک‌اند
**Status:** `pending` (0%)
**Scope:** این مرحله شامل ایجاد فایل `backend/.env.example` در مسیر `backend/` است. هدف این فایل ارائه یک الگوی واضح برای توسعه‌دهندگان جهت تنظیم متغیرهای محیطی مورد نیاز برای بخش بک‌اند برنامه است. این فایل باید شامل متغیرهای حیاتی ذکر شده برای بک‌اند باشد و مقادیر پیش‌فرض یا توضیحات مختصری برای هر یک ارائه دهد. این مرحله هیچ تغییری در کد موجود بک‌اند یا فایل `README.md` ایجاد نمی‌کند.
**Excerpt:**
```
The `README.md` instructs users to copy `backend/.env.example` and `frontend/.env.example` to set up environment variables. However, these example files are not provided in the repository structure. Consequently, critical environment variables referenced in the code, such as `OPENAI_API_KEY`, `OPENAI_DEFAULT_MODEL`, `OPENAI_EMBEDDING_MODEL` in the backend, are not documented for users. This significantly hinders the setup process, making it difficult for new developers or deployers to configure the application correctly, leading to runtime errors related to missing configuration.
```

### Step 2: ایجاد فایل `frontend/.env.example` با متغیرهای محیطی فرانت‌اند
**Status:** `pending` (0%)
**Scope:** این مرحله شامل ایجاد فایل `frontend/.env.example` در مسیر `frontend/` است. این فایل به عنوان یک الگو برای کاربران عمل می‌کند تا متغیرهای محیطی خاص فرانت‌اند را پیکربندی کنند. این فایل باید شامل متغیر `NEXT_PUBLIC_API_BASE_URL` و هر متغیر حیاتی دیگری که برای عملکرد صحیح فرانت‌اند ضروری است، باشد. این مرحله هیچ تغییری در کد موجود فرانت‌اند یا فایل `README.md` ایجاد نمی‌کند.
**Excerpt:**
```
The `README.md` instructs users to copy `backend/.env.example` and `frontend/.env.example` to set up environment variables. However, these example files are not provided in the repository structure. Consequently, critical environment variables referenced in the code, such as `NEXT_PUBLIC_API_BASE_URL` in the frontend ... are not documented for users. This significantly hinders the setup process, making it difficult for new developers or deployers to configure the application correctly, leading to runtime errors related to missing configuration.
```

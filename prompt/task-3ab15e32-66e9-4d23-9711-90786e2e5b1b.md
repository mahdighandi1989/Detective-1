...
updated_at: '2026-06-13T20:16:12.291553+00:00'
target_files:
...
1. All backend modules import `ClassificationLevel`, `UserRole`, `RiskCategory`, `RiskLevel` from `backend/app/core/enums.py`. _(verify: static)_
2. No duplicate enum definitions exist outside `backend/app/core/enums.py`. _(verify: static)_
3. Frontend types for classification and risk levels align with `backend/app/core/enums.py`. _(verify: static)_
4. All UI elements correctly display and filter based on the canonical enum values. _(verify: static)_

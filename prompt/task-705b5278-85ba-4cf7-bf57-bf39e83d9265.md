---
task_id: 705b5278-85ba-4cf7-bf57-bf39e83d9265
title: استفاده از importهای دفاعی که خطاهای پیکربندی حیاتی را پنهان می‌کنند در `risk_assessment.py`
type: bug
priority: critical
...
created_at: '2026-06-13T18:04:48.186237+00:00'
updated_at: '2026-06-13T18:04:48.186239+00:00'
target_files:
- backend/app/api/routes/risk_assessment.py
---

# استفاده از importهای دفاعی که خطاهای پیکربندی حیاتی را پنهان می‌کنند در `risk_assessment.py`

## Raw Idea

فایل `backend/app/api/routes/risk_assessment.py` از بلوک‌های `try...except Exception` گسترده‌ای برای import کردن وابستگی‌های حیاتی (مانند `get_db`, `get_current_user`, مدل‌ها و سرویس‌ها) استفاده می‌کند. این رویکرد باعث می‌شود که اگر یک وابستگی مهم در زمان راه‌اندازی برنامه موجود نباشد، به جای شکست صریح و سریع (fail-fast) که در `main.py` و `api/__init__.py` تأکید شده، برنامه به صورت 'نرم' (fail-soft) عمل کند. این امر منجر به بازگرداندن خطای `HTTPException(503)` یا `HTTPException(401)` در زمان درخواست می‌شود که تشخیص و رفع مشکلات پیکربندی واقعی را بسیار دشوار می‌کند.
...
- `backend/app/api/routes/risk_assessment.py:28-67` — بلوک‌های try...except گسترده برای importهای حیاتی
  ```python
...
Python Import System, FastAPI Application Startup

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `backend/app/main.py` (سطر 20) — فایل اصلی برنامه که بر فلسفه fail-fast تأکید دارد.
- `backend/app/api/__init__.py` (سطر 11) — فایل اصلی API که بر فلسفه fail-fast تأکید دارد.
...
## 🌐 نقشهٔ وابستگی‌ها
این تغییر بر روی نحوهٔ راه‌اندازی برنامه و تشخیص خطاهای پیکربندی تأثیر می‌گذارد. پس از این تغییر، هرگونه مشکل در import وابستگی‌ها منجر به شکست برنامه در زمان راه‌اندازی خواهد شد.
...
- [ ] تمام بلوک‌های `try...except` پیرامون importهای حیاتی در `risk_assessment.py` حذف شوند.
- [ ] در صورت عدم وجود یک وابستگی حیاتی، برنامه در زمان راه‌اندازی با خطا مواجه شود.
- [ ] روت‌های `risk_assessment` در صورت عدم پیکربندی صحیح، خطای 500 (Internal Server Error) یا خطای import در زمان راه‌اندازی را نشان دهند، نه 503 یا 401.
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. تمام بلوک‌های `try...except` پیرامون importهای حیاتی در `backend/app/api/routes/risk_assessment.py` حذف شوند. این ماژول باید مانند سایر روت‌های API، وابستگی‌های خود را به صورت مستقیم import کند. اگر یک وابستگی حیاتی موجود نباشد، برنامه باید در زمان راه‌اندازی با `ImportError` یا `AttributeError` شکست بخورد تا مشکل پیکربندی به سرعت شناسایی و رفع شود.

## 📤 خروجی مورد انتظار
تغییر کد در فایل‌های مرتبط، commit یا PR جدید با پیام واضح، و عبور تمام معیارهای پذیرش.

## 🧪 دستورات اعتبارسنجی
- `تلاش برای راه‌اندازی برنامه با حذف موقت یکی از فایل‌های مدل یا سرویس‌های حیاتی (مثلاً `app/models/risk_assessment.py`) و اطمینان از شکست برنامه در زمان راه‌اندازی.`

## ⚠️ ریسک‌ها و موارد احتیاط
افزایش احتمال شکست برنامه در زمان راه‌اندازی در صورت وجود پیکربندی ناقص، اما با هدف تشخیص سریع‌تر مشکلات.
...
1. تمام بلوک‌های `try...except` پیرامون importهای حیاتی در `risk_assessment.py` حذف شوند.
2. در صورت عدم وجود یک وابستگی حیاتی، برنامه در زمان راه‌اندازی با خطا مواجه شود.
3. روت‌های `risk_assessment` در صورت عدم پیکربندی صحیح، خطای 500 (Internal Server Error) یا خطای import در زمان راه‌اندازی را نشان دهند، نه 503 یا 401.

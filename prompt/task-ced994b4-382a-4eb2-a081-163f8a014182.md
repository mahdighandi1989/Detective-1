...
updated_at: '2026-06-13T18:02:21.223730+00:00'
target_files:
...
1. تنها یک تعریف برای هر Enum اصلی (مانند RiskLevel, ClassificationLevel) در بک‌اند وجود دارد.
2. تمام ماژول‌های بک‌اند Enumها را از منبع کانونی خود import می‌کنند.
3. مدل RiskAssessment دیگر فیلد color_code را ذخیره نمی‌کند.
4. `backend/app/api/routes/risk_assessment.py` در `backend/app/api/routes/risk.py` ادغام شده است.
5. صفحات فرانت‌اند (`/graph`, `/persons`, `/encyclopedia`) ریسک و طبقه‌بندی را بر اساس مقادیر کانونی بک‌اند به‌درستی نمایش می‌دهند.

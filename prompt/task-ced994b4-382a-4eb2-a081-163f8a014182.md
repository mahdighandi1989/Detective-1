---
task_id: ced994b4-382a-4eb2-a081-163f8a014182
title: ناسازگاری و تکرار تعاریف Enum برای ریسک و طبقه‌بندی در بک‌اند
type: bug
priority: critical
...
created_at: '2026-06-13T18:02:21.223727+00:00'
updated_at: '2026-06-13T20:16:50.014754+00:00'
target_files:
...
# ناسازگاری و تکرار تعاریف Enum برای ریسک و طبقه‌بندی در بک‌اند

## Raw Idea

چندین تعریف متناقض و تکراری برای Enumهای مرتبط با سطح ریسک (RiskCategory, RiskLevel) و طبقه‌بندی محرمانگی (ClassificationLevel) در ماژول‌های مختلف بک‌اند (models/person.py, models/risk_assessment.py, api/routes/risk.py, api/routes/risk_assessment.py, api/routes/encyclopedia.py) وجود دارد. این وضعیت اصل 'منبع واحد حقیقت' (Single Source of Truth) را نقض می‌کند و منجر به ناسازگاری داده‌ها، پاسخ‌های API غیرقابل پیش‌بینی و دشواری در نگهداری کد می‌شود. برای مثال، RiskLevel در person.py از مقادیر انگلیسی (none/low/medium/high/critical) استفاده می‌کند، در حالی که RiskLevel در risk_assessment.py از مقادیر فارسی (پاک/مشکوک/نفوذی/استحاله_یافته) بهره می‌برد. همچنین، فیلد color_code در مدل RiskAssessment، مدل را به جزئیات نمایش UI گره می‌زند.
...
- `backend/app/models/person.py:50-67` — تعاریف تکراری RiskCategory و RiskLevel
  ```python
  ...
  class RiskLevel(str, enum.Enum):
      ...
      ...
  ```
- `backend/app/models/risk_assessment.py:67-77` — تعریف متناقض RiskLevel با مقادیر فارسی
  ```python
  class RiskLevel(str, enum.Enum):
      CLEAN = "پاک"
      ...
  ```
- `backend/app/api/routes/risk.py:53-68` — تعریف تکراری RiskCategory و نگاشت رنگ
  ```python
  class RiskCategory(str, enum.Enum):
      CLEAN = "clean"
      ...
  ...
  ```
- `backend/app/api/routes/risk_assessment.py:76-93` — تعاریف تکراری و متناقض RiskCategory و RiskLevel
  ```python
  class RiskCategory(str, enum.Enum):
      CLEAN = "clean"
      ...
  class RiskLevel(str, enum.Enum):
      ...
      ...
  ```
- `backend/app/api/routes/encyclopedia.py:66-73` — تعریف تکراری ClassificationLevel
  ```python
...
FastAPI, SQLAlchemy ORM, Pydantic, Python Enums

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `backend/app/core/enums.py` — باید منبع واحد حقیقت برای Enumهای اصلی باشد
- `backend/app/api/deps.py` — از ClassificationLevel و UserRole استفاده می‌کند
- `backend/app/api/routes/graph.py` — از RiskLevel و RISK_COLOR_MAP استفاده می‌کند
- `frontend/src/app/graph/page.tsx` — از RiskLevel و RISK_META استفاده می‌کند
- `frontend/src/app/persons/page.tsx` — از RiskCategory و RISK_META استفاده می‌کند
- `backend/app/models/__init__.py` — مدل‌ها را re-export می‌کند
...
## 🌐 نقشهٔ وابستگی‌ها
این یک مشکل معماری هسته‌ای است که بر مدل‌های داده، قراردادهای API و منطق نمایش فرانت‌اند تأثیر می‌گذارد. تغییرات در چندین کامپوننت بک‌اند و فرانت‌اند منتشر خواهد شد.
...
- [ ] تنها یک تعریف برای هر Enum اصلی (مانند RiskLevel, ClassificationLevel) در بک‌اند وجود دارد.
- [ ] تمام ماژول‌های بک‌اند Enumها را از منبع کانونی خود import می‌کنند.
- [ ] مدل RiskAssessment دیگر فیلد color_code را ذخیره نمی‌کند.
- [ ] `backend/app/api/routes/risk_assessment.py` در `backend/app/api/routes/risk.py` ادغام شده است.
- [ ] صفحات فرانت‌اند (`/graph`, `/persons`, `/encyclopedia`) ریسک و طبقه‌بندی را بر اساس مقادیر کانونی بک‌اند به‌درستی نمایش می‌دهند.
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. 1. یک مجموعهٔ واحد و کانونی از Enumها برای RiskCategory و ClassificationLevel در فایل `app.core.enums.py` تعریف شود. 2. Enumهای `AssessmentType` و `RelationshipType` در ماژول‌های مدل مربوطه (مانند `app.models.risk_assessment.py` و `app.models.relationship.py`) تعریف شوند. 3. تمام مدل‌ها (Person, RiskAssessment, Article) و روت‌های API (risk.py, risk_assessment.py, persons.py, encyclopedia.py) به‌روزرسانی شوند تا این Enumهای کانونی را از منبع واحد import و استفاده کنند. 4. فیلد `color_code` از مدل `RiskAssessment` حذف شود؛ نگاشت رنگ باید در لایهٔ پاسخ API یا فرانت‌اند انجام شود. 5. روت‌های `backend/app/api/routes/risk.py` و `backend/app/api/routes/risk_assessment.py` در یک ماژول واحد (مثلاً `backend/app/api/routes/risk.py`) ادغام شوند تا تکرار منطق و تعاریف از بین برود.

## 💡 نمونه‌های قبل/بعد
**قبل: تعاریف پراکنده RiskLevel**
...
## 🧪 دستورات اعتبارسنجی
- `grep -r "class RiskLevel" backend/app/models/ backend/app/api/routes/`
- `grep -r "class RiskCategory" backend/app/models/ backend/app/api/routes/`
- `pytest backend/tests/`
- `npm test`
- `uvicorn app.main:app --reload (و بررسی دستی صفحات فرانت‌اند)`

## ⚠️ ریسک‌ها و موارد احتیاط
ریسک بالا برای ایجاد باگ‌های جدید در صورت عدم بازسازی دقیق. نیاز به تست کامل تمام endpointهای API و صفحات فرانت‌اند.
...
1. تنها یک تعریف برای هر Enum اصلی (مانند RiskLevel, ClassificationLevel) در بک‌اند وجود دارد. _(verify: static)_
2. تمام ماژول‌های بک‌اند Enumها را از منبع کانونی خود import می‌کنند. _(verify: static)_
3. مدل RiskAssessment دیگر فیلد color_code را ذخیره نمی‌کند. _(verify: static)_
4. `backend/app/api/routes/risk_assessment.py` در `backend/app/api/routes/risk.py` ادغام شده است. _(verify: static)_
5. صفحات فرانت‌اند (`/graph`, `/persons`, `/encyclopedia`) ریسک و طبقه‌بندی را بر اساس مقادیر کانونی بک‌اند به‌درستی نمایش می‌دهند. _(verify: static)_

## Task Steps

### Step 1: Consolidate RiskCategory Enum Definition in models/risk_assessment.py
**Status:** `pending` (0%)
**Scope:** این مرحله بر روی فایل `models/risk_assessment.py` متمرکز است. هدف، تعریف یا انتقال تعریف canonical و استاندارد `RiskCategory` Enum به این فایل است تا به عنوان منبع واحد حقیقت برای این Enum عمل کند. این کار شامل حذف هرگونه تعریف تکراری یا متناقض `RiskCategory` از داخل همین فایل و اطمینان از صحت تعریف نهایی است. این مرحله شامل به‌روزرسانی سایر فایل‌ها یا ماژول‌ها که از `RiskCategory` استفاده می‌کنند، نمی‌شود.
**Excerpt:**
```
چندین تعریف متناقض و تکراری برای Enumهای مرتبط با سطح ریسک (RiskCategory, RiskLevel) و طبقه‌بندی محرمانگی (ClassificationLevel) در ماژول‌های مختلف بک‌اند (models/person.py, models/risk_assessment.py, api/routes/risk.py, api/routes/risk_assessment.py, api/routes/encyclopedia.py) وجود دارد. این وضعیت اصل 'منبع واحد حقیقت' (Single Source of Truth) را نقض می‌کند و منجر به ناسازگاری داده‌ها، پاسخ‌های API غیرقابل پیش‌بینی و دشواری در نگهداری کد می‌شود.
```

### Step 2: Consolidate RiskLevel Enum Definition with Canonical Values in models/risk_assessment.py
**Status:** `pending` (0%)
**Scope:** این مرحله بر روی فایل `models/risk_assessment.py` متمرکز است. هدف، تعریف یا انتقال تعریف canonical و استاندارد `RiskLevel` Enum به این فایل است. این شامل انتخاب یک مجموعه مقادیر استاندارد (مثلاً انگلیسی: `none`/`low`/`medium`/`high`/`critical`) و حذف هرگونه تعریف تکراری یا متناقض `RiskLevel` از داخل همین فایل است. این مرحله شامل به‌روزرسانی سایر فایل‌ها یا ماژول‌ها که از `RiskLevel` استفاده می‌کنند، نمی‌شود.
**Excerpt:**
```
RiskLevel در person.py از مقادیر انگلیسی (none/low/medium/high/critical) استفاده می‌کند، در حالی که RiskLevel در risk_assessment.py از مقادیر فارسی (پاک/مشکوک/نفوذی/استحاله_یافته) بهره می‌برد. چندین تعریف متناقض و تکراری برای Enumهای مرتبط با سطح ریسک (RiskCategory, RiskLevel) و طبقه‌بندی محرمانگی (ClassificationLevel) در ماژول‌های مختلف بک‌اند (models/person.py, models/risk_assessment.py, api/routes/risk.py, api/routes/risk_assessment.py, api/routes/encyclopedia.py) وجود دارد.
```

### Step 3: Consolidate ClassificationLevel Enum Definition in models/risk_assessment.py
**Status:** `pending` (0%)
**Scope:** این مرحله بر روی فایل `models/risk_assessment.py` متمرکز است. هدف، تعریف یا انتقال تعریف canonical و استاندارد `ClassificationLevel` Enum به این فایل است تا به عنوان منبع واحد حقیقت برای این Enum عمل کند. این کار شامل حذف هرگونه تعریف تکراری یا متناقض `ClassificationLevel` از داخل همین فایل و اطمینان از صحت تعریف نهایی است. این مرحله شامل به‌روزرسانی سایر فایل‌ها یا ماژول‌ها که از `ClassificationLevel` استفاده می‌کنند، نمی‌شود.
**Excerpt:**
```
چندین تعریف متناقض و تکراری برای Enumهای مرتبط با سطح ریسک (RiskCategory, RiskLevel) و طبقه‌بندی محرمانگی (ClassificationLevel) در ماژول‌های مختلف بک‌اند (models/person.py, models/risk_assessment.py, api/routes/risk.py, api/routes/risk_assessment.py, api/routes/encyclopedia.py) وجود دارد. این وضعیت اصل 'منبع واحد حقیقت' (Single Source of Truth) را نقض می‌کند و منجر به ناسازگاری داده‌ها، پاسخ‌های API غیرقابل پیش‌بینی و دشواری در نگهداری کد می‌شود.
```

### Step 4: Remove color_code Field from RiskAssessment Model in models/risk_assessment.py
**Status:** `pending` (0%)
**Scope:** این مرحله به طور خاص بر روی مدل `RiskAssessment` در فایل `models/risk_assessment.py` تمرکز دارد. هدف، حذف فیلد `color_code` از تعریف این مدل است. این اقدام برای جداسازی نگرانی‌های مربوط به نمایش UI از مدل داده‌ای بک‌اند انجام می‌شود. این مرحله شامل به‌روزرسانی منطق وابسته به `color_code` یا تغییرات UI نمی‌شود.
**Excerpt:**
```
فیلد color_code در مدل RiskAssessment، مدل را به جزئیات نمایش UI گره می‌زند.
```

### Step 5: Update models/person.py to Use Centralized Enums
**Status:** `pending` (0%)
**Scope:** این مرحله بر روی فایل `models/person.py` متمرکز است. هدف، حذف هرگونه تعریف محلی و تکراری از `RiskLevel` و `ClassificationLevel` (در صورت وجود) در این فایل و جایگزینی آن‌ها با وارد کردن (import) تعاریف استاندارد شده از `models/risk_assessment.py` است. این کار تضمین می‌کند که `models/person.py` از منبع واحد حقیقت برای این Enumها پیروی می‌کند. این مرحله شامل به‌روزرسانی سایر فایل‌ها یا انجام مهاجرت داده‌ای نمی‌شود.
**Excerpt:**
```
RiskLevel در person.py از مقادیر انگلیسی (none/low/medium/high/critical) استفاده می‌کند, در حالی که RiskLevel در risk_assessment.py از مقادیر فارسی (پاک/مشکوک/نفوذی/استحاله_یافته) بهره می‌برد. چندین تعریف متناقض و تکراری برای Enumهای مرتبط با سطح ریسک (RiskCategory, RiskLevel) و طبقه‌بندی محرمانگی (ClassificationLevel) در ماژول‌های مختلف بک‌اند (models/person.py, models/risk_assessment.py, api/routes/risk.py, api/routes/risk_assessment.py, api/routes/encyclopedia.py) وجود دارد.
```

### Step 6: Update api/routes/risk.py to Use Centralized Enums
**Status:** `pending` (0%)
**Scope:** این مرحله بر روی فایل `api/routes/risk.py` متمرکز است. هدف، حذف هرگونه تعریف محلی و تکراری از `RiskCategory` و `RiskLevel` (در صورت وجود) در این فایل و جایگزینی آن‌ها با وارد کردن (import) تعاریف استاندارد شده از `models/risk_assessment.py` است. این کار تضمین می‌کند که این API route از منبع واحد حقیقت برای این Enumها پیروی می‌کند و پاسخ‌های API سازگار خواهند بود. این مرحله شامل به‌روزرسانی سایر فایل‌ها یا انجام مهاجرت داده‌ای نمی‌شود.
— [merged] این مرحله بر روی فایل `api/routes/risk_assessment.py` متمرکز است. هدف، حذف هرگونه تعریف محلی و تکراری از `RiskCategory` و `RiskLevel` (در صورت وجود) در این فایل و جایگزینی آن‌ها با وارد کردن (import) تعاریف استاندارد شده از `models/risk_assessment.py` است. این کار تضمین می‌کند که این API route از منبع واحد حقیقت برای این Enumها پیروی می‌کند و پاسخ‌های API سازگار خواهند بود. این مرحله شامل به‌روزرسانی سایر فایل‌ها یا انجام مهاجرت داده‌ای نمی‌شود.
— [merged] این مرحله بر روی فایل `api/routes/encyclopedia.py` متمرکز است. هدف، حذف هرگونه تعریف محلی و تکراری از `ClassificationLevel` (و در صورت وجود، `RiskCategory`/`RiskLevel`) در این فایل و جایگزینی آن‌ها با وارد کردن (import) تعاریف استاندارد شده از `models/risk_assessment.py` است. این کار تضمین می‌کند که این API route از منبع واحد حقیقت برای این Enumها پیروی می‌کند و پاسخ‌های API سازگار خواهند بود. این مرحله شامل به‌روزرسانی سایر فایل‌ها یا انجام مهاجرت داده‌ای نمی‌شود.
**Excerpt:**
```
چندین تعریف متناقض و تکراری برای Enumهای مرتبط با سطح ریسک (RiskCategory, RiskLevel) و طبقه‌بندی محرمانگی (ClassificationLevel) در ماژول‌های مختلف بک‌اند (models/person.py, models/risk_assessment.py, api/routes/risk.py, api/routes/risk_assessment.py, api/routes/encyclopedia.py) وجود دارد. این وضعیت اصل 'منبع واحد حقیقت' (Single Source of Truth) را نقض می‌کند و منجر به ناسازگاری داده‌ها، پاسخ‌های API غیرقابل پیش‌بینی و دشواری در نگهداری کد می‌شود.
```

### Step 7: Implement Data Migration for RiskLevel Values
**Status:** `pending` (0%)
**Scope:** این مرحله شامل ایجاد و اجرای یک اسکریپت مهاجرت پایگاه داده است. هدف، تبدیل مقادیر موجود `RiskLevel` در پایگاه داده از نمایش‌های قدیمی و ناسازگار (مانند مقادیر فارسی) به مقادیر جدید و canonical (مانند مقادیر انگلیسی) است که در `models/risk_assessment.py` تعریف شده‌اند. این اسکریپت باید به طور ایمن داده‌ها را به‌روزرسانی کند تا از از دست رفتن داده‌ها یا ناسازگاری پس از تغییرات کد جلوگیری شود. این مرحله شامل تغییرات در منطق برنامه یا UI نمی‌شود.
**Excerpt:**
```
این وضعیت اصل 'منبع واحد حقیقت' (Single Source of Truth) را نقض می‌کند و منجر به ناسازگاری داده‌ها، پاسخ‌های API غیرقابل پیش‌بینی و دشواری در نگهداری کد می‌شود. برای مثال، RiskLevel در person.py از مقادیر انگلیسی (none/low/medium/high/critical) استفاده می‌کند، در حالی که RiskLevel در risk_assessment.py از مقادیر فارسی (پاک/مشکوک/نفوذی/استحاله_یافته) بهره می‌برد.
```

### Step 8: Update Logic Dependent on risk_assessment Function/Variable
**Status:** `pending` (0%)
**Scope:** این مرحله شامل بازبینی و به‌روزرسانی هر بخش از کد است که از تابع یا متغیر `risk_assessment` استفاده می‌کند. هدف، اطمینان از اینکه این منطق به درستی با تعاریف جدید و متمرکز Enumها (مانند `RiskCategory` و `RiskLevel`) تعامل دارد و هرگونه تغییر در مقادیر `RiskLevel` را به درستی مدیریت می‌کند. این ممکن است شامل به‌روزرسانی امضای توابع، منطق داخلی، یا نحوه فراخوانی باشد. این مرحله شامل تغییرات UI یا مهاجرت داده‌ای نمی‌شود.
**Excerpt:**
```
⚙ نام توابع/متغیرهای snake_case ذکرشده: • risk_assessment
```

### Step 9: Update Logic Dependent on color_code Function/Variable
**Status:** `pending` (0%)
**Scope:** این مرحله شامل بازبینی و به‌روزرسانی هر بخش از کد است که از تابع یا متغیر `color_code` استفاده می‌کند. هدف، حذف یا بازآرایی این منطق است، به طوری که دیگر به فیلد `color_code` که از مدل `RiskAssessment` حذف شده، وابسته نباشد. این ممکن است شامل انتقال منطق تولید رنگ به لایه UI، یا بازنویسی آن برای استخراج اطلاعات رنگ از `RiskLevel` یا سایر ویژگی‌های مدل باشد. این مرحله شامل تغییرات مستقیم در UI نمی‌شود.
**Excerpt:**
```
فیلد color_code در مدل RiskAssessment، مدل را به جزئیات نمایش UI گره می‌زند. ⚙ نام توابع/متغیرهای snake_case ذکرشده: • color_code
```

### Step 10: Add/Update Unit and Integration Tests for Centralized Enums and Decoupling
**Status:** `pending` (0%)
**Scope:** این مرحله شامل ایجاد تست‌های واحد و یکپارچه‌سازی جدید یا به‌روزرسانی تست‌های موجود است. هدف، تأیید صحت پیاده‌سازی 'منبع واحد حقیقت' برای Enumهای `RiskCategory`، `RiskLevel` و `ClassificationLevel` در فایل‌های `models/person.py`، `models/risk_assessment.py`، `api/routes/risk.py`، `api/routes/risk_assessment.py` و `api/routes/encyclopedia.py` است. همچنین، این تست‌ها باید اطمینان حاصل کنند که حذف فیلد `color_code` از مدل `RiskAssessment` هیچ رگرسیونی ایجاد نکرده و منطق وابسته به درستی عمل می‌کند. این مرحله شامل هیچ تغییر کد عملکردی نمی‌شود، بلکه فقط به تست‌ها می‌پردازد.
**Excerpt:**
```
این وضعیت اصل 'منبع واحد حقیقت' (Single Source of Truth) را نقض می‌کند و منجر به ناسازگاری داده‌ها، پاسخ‌های API غیرقابل پیش‌بینی و دشواری در نگهداری کد می‌شود.
```

---
task_id: 40e28b0f-74ec-48ce-a50a-ea08f0f6fd35
title: عدم تطابق نسخه‌های وابستگی بین pyproject.toml و requirements.txt
type: refactor
priority: medium
...
created_at: '2026-06-13T18:15:53.913520+00:00'
updated_at: '2026-06-13T20:20:16.175196+00:00'
target_files:
- pyproject.toml
- requirements.txt
---

# عدم تطابق نسخه‌های وابستگی بین pyproject.toml و requirements.txt

## Raw Idea

فایل‌های `pyproject.toml` و `requirements.txt`، که هر دو وابستگی‌های پروژه را تعریف می‌کنند، نسخه‌های متفاوتی را برای چندین بسته (مانند `fastapi`، `uvicorn`، `pydantic`، `celery`، `openai`) مشخص می‌کنند. `requirements.txt` به‌صراحت بیان می‌کند که «منبع حقیقت: pyproject.toml» است و باید همگام‌سازی شود. این عدم تطابق می‌تواند منجر به محیط‌های ساخت ناسازگار، رفتار غیرمنتظره در زمان اجرا و به‌طور بالقوه از دست دادن وصله‌های امنیتی یا رفع اشکال در نسخه‌های جدیدتر شود، یا در صورت به‌روزتر بودن `pyproject.toml`، تغییرات ناسازگار را معرفی کند.
...
- `pyproject.toml:10-24` — `tool.poetry.dependencies` — نسخه‌های `fastapi`, `uvicorn`, `pydantic`, `sqlalchemy`, `celery`, `openai` و دیگران در `pyproject.toml` با `requirements.txt` متفاوت است.
  ```
  ...
  ```
- `requirements.txt:16-20` — نسخه‌های دقیق در `requirements.txt`.
  ```
  fastapi==0.111.0
...
Poetry, pip, Docker builds

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `docker-compose.yml` — از `requirements.txt` برای ساخت ایمیج بک‌اند استفاده می‌کند.
- `backend/Dockerfile` — از `requirements.txt` استفاده می‌کند.

## 🌐 نقشهٔ وابستگی‌ها
بر تمام وابستگی‌های پایتون تأثیر می‌گذارد و به‌طور بالقوه منجر به شکست‌های ساخت یا مشکلات زمان اجرا می‌شود.
...
- [ ] تمام وابستگی‌های مشترک دارای نسخه‌های پین‌شده یکسان در هر دو `pyproject.toml` و `requirements.txt` هستند.
- [ ] دستور `poetry export` اجرا شده و خروجی آن commit شده است.
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. نسخه‌های وابستگی را همگام‌سازی کنید. `pyproject.toml` را به‌روزرسانی کنید تا نسخه‌های دقیق استفاده شده در `requirements.txt` را منعکس کند (یا برعکس، اگر `pyproject.toml` واقعاً منبع حقیقت است و `requirements.txt` باید از آن تولید شود). اطمینان حاصل کنید که دستور `poetry export` ذکر شده در `requirements.txt` به‌طور منظم اجرا و commit می‌شود.

## 💡 نمونه‌های قبل/بعد
**همگام‌سازی نسخه FastAPI**
...
## 🧪 دستورات اعتبارسنجی
- `poetry install (در دایرکتوری backend)`
- `pip install -r backend/requirements.txt (در یک محیط تمیز)`
- `docker-compose build backend`

## ⚠️ ریسک‌ها و موارد احتیاط
شکست‌های ساخت در صورت عدم همگام‌سازی دقیق.
...
1. تمام وابستگی‌های مشترک دارای نسخه‌های پین‌شده یکسان در هر دو `pyproject.toml` و `requirements.txt` هستند. _(verify: static)_
2. دستور `poetry export` اجرا شده و خروجی آن commit شده است. _(verify: static)_

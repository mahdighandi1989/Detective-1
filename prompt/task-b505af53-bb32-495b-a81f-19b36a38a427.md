...
updated_at: '2026-06-06T17:39:01.342624+00:00'
target_files:
...
1. تمام فراخوانی‌های API از صفحات فرانت‌اند از کلاینت `api.ts` استفاده می‌کنند.
2. کلاینت `api.ts` به طور خودکار توکن‌های احراز هویت را در درخواست‌ها قرار می‌دهد.
3. پاسخ‌های خطا از بک‌اند به طور سازگار توسط `api.ts` تجزیه و تحلیل شده و به عنوان استثنا پرتاب می‌شوند.
4. کامپوننت‌های UI فرانت‌اند به طور مناسب خطاها را که توسط کلاینت `api.ts` برگردانده می‌شوند، مدیریت می‌کنند.

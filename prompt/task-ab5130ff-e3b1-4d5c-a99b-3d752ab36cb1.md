---
task_id: ab5130ff-e3b1-4d5c-a99b-3d752ab36cb1
...
created_at: '2026-06-13T18:15:54.762599+00:00'
updated_at: '2026-06-13T20:20:39.375018+00:00'
target_files:
...
## 📋 شرح
endpoint `POST /register` در `backend/app/api/routes/auth.py` تعریف شده ولی هیچ `fetch`، `axios`، `apiClient.*`، یا frontend call به آن دیده نمی‌شود.

## 🔍 جزئیات
- method: `POST`
- path: `/register`
- فایل: `backend/app/api/routes/auth.py`
...
---
[scan #2 at 2026-06-13T18:15:54.826692+00:00]
## 📋 شرح
endpoint `POST /login` در `backend/app/api/routes/auth.py` تعریف شده ولی هیچ `fetch`، `axios`، `apiClient.*`، یا frontend call به آن دیده نمی‌شود.
...
## 🤔 
---
[scan #3 at 2026-06-13T18:15:54.865540+00:00]
## 📋 شرح
endpoint `POST /refresh` در `backend/app/api/routes/auth.py` تعریف شده ولی هیچ `fetch`، `axios`، `apiClient.*`، یا frontend call به آن دیده نمی‌شود.
...
#
---
[scan #4 at 2026-06-13T18:15:54.896863+00:00]
## 📋 شرح
endpoint `GET /me` در `backend/app/api/routes/auth.py` تعریف شده ولی هیچ `fetch`، `axios`، `apiClient.*`، یا frontend call به آن دیده نمی‌شود.
...
- `backend/app/core/security.py` — `auth.py` این فایل را import می‌کند
- `backend/app/db/session.py` — `auth.py` این فایل را import می‌کند
- `backend/app/models/user.py` — `auth.py` این فایل را import می‌کند
- `backend/app/schemas/auth.py` — `auth.py` این فایل را import می‌کند
...
## 🧪 دستورات اعتبارسنجی
- `python -m py_compile backend/app/api/routes/auth.py`
- `ruff check backend/app/api/routes/auth.py`
- `pytest -x`
...
1. مشخص شد endpoint `POST /register` در کدام دسته است (orphan/internal/deprecated) _(verify: static)_

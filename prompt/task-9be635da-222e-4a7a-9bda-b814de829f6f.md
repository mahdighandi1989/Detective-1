---
task_id: 9be635da-222e-4a7a-9bda-b814de829f6f
title: 'Critical: `risk_assessment` Module Uses Unsafe Fallback Imports'
type: security
priority: critical
...
# Critical: `risk_assessment` Module Uses Unsafe Fallback Imports

## Raw Idea

The `backend/app/api/routes/risk_assessment.py` module employs extensive `try/except` blocks for importing critical dependencies such as `get_db`, `get_current_user`, `require_roles`, and even ORM models (`RiskAssessment`, `Person`). This practice, explicitly warned against in `backend/app/main.py` and `backend/app/api/routes/persons.py` as an anti-pattern, allows the application to boot with potentially broken or mocked security dependencies. For example, `get_current_user` and `require_roles` are mocked to raise `HTTPException(401)` or return the user without actual checks if the real dependencies are missing. This undermines the entire authentication and authorization framework for the risk assessment module, creating a critical security vulnerability or a highly unreliable system.
...
- `backend/app/api/routes/risk_assessment.py:28-68` — `(module imports)` — The entire block of defensive imports for core dependencies is problematic.
  ```python
...
FastAPI, Python import system, dependency injection.

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `backend/app/main.py` (سطر 20) — Explicitly warns against this pattern.
- `backend/app/api/routes/persons.py` (سطر 29) — Explicitly removed this pattern.
- `backend/app/api/deps.py` — Defines the canonical dependencies.
- `backend/app/models/risk_assessment.py` — The model that might be mocked.
- `backend/app/models/person.py` — The model that might be mocked.
- `backend/app/services/risk_engine.py` — `risk_assessment.py` این فایل را import می‌کند

## 🌐 نقشهٔ وابستگی‌ها
This issue affects the entire `risk_assessment` module, potentially bypassing authentication and authorization for all its endpoints.
...
- [ ] The `backend/app/api/routes/risk_assessment.py` module imports all its core dependencies directly without `try/except` fallbacks.
- [ ] If a core dependency is missing, the application fails to start with a clear `ImportError`.
- [ ] Authentication and authorization for risk assessment endpoints function as intended.
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. Remove all `try/except` blocks around critical imports in `backend/app/api/routes/risk_assessment.py`. Adopt the "fail-fast" principle consistently across the backend. Ensure all required dependencies are correctly wired and available at application startup.

## 💡 نمونه‌های قبل/بعد
**Direct, fail-fast imports**
...
## 🧪 دستورات اعتبارسنجی
- `Temporarily rename `backend/app/api/deps.py` and verify the application fails to start with an `ImportError`.`
- `Run existing tests for `risk_assessment` endpoints to ensure they still pass with direct imports.`

## ⚠️ ریسک‌ها و موارد احتیاط
Introducing startup errors if the environment is not correctly configured for all dependencies.

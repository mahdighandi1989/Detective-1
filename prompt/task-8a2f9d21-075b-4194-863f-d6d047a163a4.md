...
updated_at: '2026-06-13T18:04:23.418382+00:00'
target_files:
...
1. A single `RiskCategory` enum (e.g., in `app.core.enums.py`) is used consistently across all backend models and API routes.
2. The `RiskAssessment` model's `risk_level` field is renamed to `category` and uses the canonical `RiskCategory` enum.
3. The `risk.py` router successfully creates `RiskAssessment` records with correct category values.
4. The `graph.py` router correctly retrieves and displays risk categories.
5. Frontend `graph/page.tsx` and `persons/page.tsx` display consistent risk labels and colors based on the canonical categories.
6. All redundant local risk enums are removed.

---
task_id: 742b5bf5-63a2-421f-b811-a17235ce76f1
title: 'High: Missing API Endpoints for Creating, Updating, and Deleting Person Relationships'
...
created_at: '2026-06-13T18:10:52.012002+00:00'
updated_at: '2026-06-13T20:18:47.019087+00:00'
target_files:
- backend/app/models/relationship.py
- backend/app/api/__init__.py
---

# High: Missing API Endpoints for Creating, Updating, and Deleting Person Relationships

## Raw Idea

The `Relationship` model (`backend/app/models/relationship.py`) is defined, allowing for the representation of connections between persons. The `graph.py` router (`backend/app/api/routes/graph.py`) successfully reads and visualizes these relationships. However, there are no corresponding API endpoints (e.g., in `persons.py` or a new `relationships.py` router) to create, update, or delete these relationships. This means the relationship graph is read-only from the API perspective, severely limiting the 'Person Profiling & Tracking' module's ability to manage and enrich person connections.
...
- `backend/app/models/relationship.py:73-115` — `Relationship` — The model for relationships exists, but no API to manage it.
  ```python
  class Relationship(Base):
      ...
      context_url: Mapped[Optional[str]] = mapped_column(String(2048), nullable=True)
  ...
  ```
- `backend/app/api/__init__.py:36-41` — New router for relationships would need to be included here.
  ```python
...
- `backend/app/models/person.py` (سطر 135) — `Person` model has `outgoing_relationships` and `incoming_relationships` relationships.
- `backend/app/api/routes/graph.py` (سطر 182) — Reads relationships using `_extract_relationships`.
- `frontend/src/app/graph/page.tsx` (سطر 179) — Consumes relationship data for visualization.
- `backend/app/db/base_class.py` — `relationship.py` این فایل را import می‌کند
- `backend/app/db/base.py` — `relationship.py` این فایل را import می‌کند
- `backend/app/models/article.py` — این فایل `relationship.py` را import می‌کند (caller)
- `backend/app/models/audit_log.py` — این فایل `relationship.py` را import می‌کند (caller)
- `backend/app/models/risk_assessment.py` — این فایل `relationship.py` را import می‌کند (caller)

## 🌐 نقشهٔ وابستگی‌ها
Involves creating new API endpoints and schemas, and updating the main API router. This is a core feature for the graph visualization and person profiling module.
...
- [ ] New API endpoints exist for creating, updating, and deleting relationships.
- [ ] These endpoints are documented in OpenAPI.
- [ ] Relationships can be successfully managed via the API.
- [ ] RBAC is enforced for these operations.
- [ ] Audit logs are recorded for relationship changes.
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. 1. Create a new FastAPI router (e.g., `backend/app/api/routes/relationships.py`) to include endpoints for:
    *   `POST /relationships`: Create a new relationship between two persons.
    *   `PATCH /relationships/{relationship_id}`: Update an existing relationship.
    *   `DELETE /relationships/{relationship_id}`: Delete a relationship.
2. Define Pydantic schemas for `RelationshipCreate`, `RelationshipUpdate`, and `RelationshipRead` (similar to `PersonCreate`/`Update`/`Read`).
3. Implement RBAC checks for these new endpoints, likely requiring `ANALYST` or `ADMIN` roles.
4. Integrate audit logging for these actions.
5. Update `api/__init__.py` to include the new router.

## 💡 نمونه‌های قبل/بعد
**Conceptual `POST /relationships` endpoint**

_قبل:_
```
No API endpoint to create a relationship.
```
...
async def create_relationship(
    ...
    # ... logic to create relationship ...
    return RelationshipRead.model_validate(new_relationship)
```
...
## 🧪 دستورات اعتبارسنجی
- `Send POST/PATCH/DELETE requests to the new `/api/v1/relationships` endpoints.`
- `Verify relationship changes are reflected in the graph UI and database.`

## ⚠️ ریسک‌ها و موارد احتیاط
Ensuring data integrity for relationships (e.g., preventing self-loops, duplicate relationships); proper error handling for non-existent persons; potential for complex frontend integration.
...
1. New API endpoints exist for creating, updating, and deleting relationships. _(verify: static)_
2. These endpoints are documented in OpenAPI. _(verify: static)_
3. Relationships can be successfully managed via the API. _(verify: static)_
4. RBAC is enforced for these operations. _(verify: static)_
5. Audit logs are recorded for relationship changes. _(verify: static)_

## Task Steps

### Step 1: ایجاد فایل روتر جدید برای مدیریت روابط (Relationships)
**Status:** `pending` (0%)
**Scope:** این مرحله شامل ایجاد یک فایل پایتون جدید برای تعریف API router مخصوص روابط بین اشخاص است. هدف این است که عملیات CRUD برای مدل `Relationship` در یک ماژول جداگانه و سازمان‌یافته قرار گیرد، مشابه رویکرد `graph.py` برای خواندن روابط. این فایل در ساختار `backend/app/api/routes/` قرار خواهد گرفت و یک نمونه FastAPI `APIRouter` را مقداردهی اولیه می‌کند. خارج از این مرحله، پیاده‌سازی خود endpointهای CRUD است.
**Excerpt:**
```
However, there are no corresponding API endpoints (e.g., in `persons.py` or a new `relationships.py` router) to create, update, or delete these relationships.
```

### Step 2: پیاده‌سازی Endpoint 'ایجاد رابطه' (Create Relationship)
**Status:** `pending` (0%)
**Scope:** این مرحله بر پیاده‌سازی endpoint API برای ایجاد یک رابطه جدید بین اشخاص تمرکز دارد. این شامل تعریف یک مدل Pydantic برای داده‌های ورودی مورد نیاز برای ایجاد یک `Relationship` (مانند شناسه اشخاص مرتبط و نوع رابطه) و سپس افزودن یک endpoint از نوع POST به روتر `relationships.py` است. این endpoint باید داده‌ها را اعتبارسنجی کرده و از مدل `Relationship` برای ذخیره رابطه جدید در پایگاه داده استفاده کند. خارج از این مرحله، پیاده‌سازی endpointهای به‌روزرسانی یا حذف است.
**Excerpt:**
```
However, there are no corresponding API endpoints (e.g., in `persons.py` or a new `relationships.py` router) to create, update, or delete these relationships. This means the relationship graph is read-only from the API perspective, severely limiting the 'Person Profiling & Tracking' module's ability to manage and enrich person connections.
```

### Step 3: پیاده‌سازی Endpoint 'به‌روزرسانی رابطه' (Update Relationship)
**Status:** `pending` (0%)
**Scope:** این مرحله به پیاده‌سازی endpoint API برای به‌روزرسانی یک رابطه موجود می‌پردازد. این شامل تعریف یک مدل Pydantic برای داده‌های ورودی مورد نیاز برای به‌روزرسانی یک `Relationship` و افزودن یک endpoint از نوع PUT یا PATCH به روتر `relationships.py` است. این endpoint باید شناسه رابطه را دریافت کرده، رابطه موجود را در پایگاه داده پیدا کند و فیلدهای مشخص‌شده را به‌روزرسانی کند. خارج از این مرحله، پیاده‌سازی endpointهای ایجاد یا حذف است.
**Excerpt:**
```
However, there are no corresponding API endpoints (e.g., in `persons.py` or a new `relationships.py` router) to create, update, or delete these relationships. This means the relationship graph is read-only from the API perspective, severely limiting the 'Person Profiling & Tracking' module's ability to manage and enrich person connections.
```

### Step 4: پیاده‌سازی Endpoint 'حذف رابطه' (Delete Relationship)
**Status:** `pending` (0%)
**Scope:** این مرحله به پیاده‌سازی endpoint API برای حذف یک رابطه موجود اختصاص دارد. این شامل افزودن یک endpoint از نوع DELETE به روتر `relationships.py` است که شناسه رابطه مورد نظر برای حذف را دریافت می‌کند. این endpoint باید رابطه را از پایگاه داده حذف کند. خارج از این مرحله، پیاده‌سازی endpointهای ایجاد یا به‌روزرسانی است.
**Excerpt:**
```
However, there are no corresponding API endpoints (e.g., in `persons.py` or a new `relationships.py` router) to create, update, or delete these relationships. This means the relationship graph is read-only from the API perspective, severely limiting the 'Person Profiling & Tracking' module's ability to manage and enrich person connections.
```

### Step 5: یکپارچه‌سازی روتر جدید روابط با برنامه اصلی FastAPI
**Status:** `pending` (0%)
**Scope:** این مرحله شامل افزودن روتر `relationships.py` که در مراحل قبلی ایجاد و پیاده‌سازی شد، به برنامه اصلی FastAPI است. این کار معمولاً با فراخوانی متد `include_router` روی نمونه اصلی برنامه FastAPI انجام می‌شود تا endpointهای تعریف‌شده در `relationships.py` قابل دسترسی شوند. این یکپارچه‌سازی برای فعال‌سازی کامل قابلیت‌های CRUD جدید برای روابط ضروری است. خارج از این مرحله، هرگونه تغییر در روترهای موجود مانند `graph.py` است.
**Excerpt:**
```
The `Relationship` model (`backend/app/models/relationship.py`) is defined, allowing for the representation of connections between persons. The `graph.py` router (`backend/app/api/routes/graph.py`) successfully reads and visualizes these relationships. However, there are no corresponding API endpoints (e.g., in `persons.py` or a new `relationships.py` router) to create, update, or delete these relationships.
```

---
task_id: 2101e1f3-f621-4559-93a8-dcc1db6aa728
title: 'endpoint بک‌اند بلااستفاده: GET /ready'
type: audit
priority: medium
...
created_at: '2026-06-13T18:15:55.272488+00:00'
updated_at: '2026-06-13T20:21:04.626734+00:00'
target_files:
- backend/app/main.py
---

# endpoint بک‌اند بلااستفاده: GET /ready

## Raw Idea

## 📋 شرح
endpoint `GET /ready` در `backend/app/main.py` تعریف شده ولی هیچ `fetch`، `axios`، `apiClient.*`، یا frontend call به آن دیده نمی‌شود.
...
- [ ] مشخص شد endpoint `GET /ready` در کدام دسته است (orphan/internal/deprecated)
...
1. گام ۱: grep روی `/ready` در frontend + scripts/ + docs/ — اگر caller هست، اتصال را drop شده اصلاح کن.
...
1. مشخص شد endpoint `GET /ready` در کدام دسته است (orphan/internal/deprecated) _(verify: static)_

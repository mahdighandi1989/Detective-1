---
task_id: d1c8bdb2-c7e1-46d7-8765-5ef3c85a6cc9
title: استراتژی ناسازگار import Enum در `backend/app/api/deps.py`
type: bug
priority: medium
...
created_at: '2026-06-13T18:15:53.838629+00:00'
updated_at: '2026-06-13T20:20:08.030292+00:00'
target_files:
- backend/app/api/deps.py
---

# استراتژی ناسازگار import Enum در `backend/app/api/deps.py`

## Raw Idea

ماژول `backend/app/api/deps.py` که وابستگی‌های حیاتی احراز هویت و مجوز را مدیریت می‌کند، از یک بلوک `try/except` بیش از حد دفاعی (خطوط ۳۲-۵۴) برای import کردن `UserRole` و `ClassificationLevel` استفاده می‌کند. با وجود اینکه کامنت به صراحت `app.core.enums` را 'منبع واحد حقیقت' می‌داند و بیان می‌کند که فایل 'نباید کپی محلی/fallback خود را از `UserRole` تعریف کند'، اما در صورت شکست تمام importهای دیگر، یک Enum fallback برای `ClassificationLevel` تعریف می‌کند. این با فلسفه 'fail-fast' که در `main.py` و `persons.py` اتخاذ شده، در تضاد است و می‌تواند مشکلات پیکربندی را پنهان کند یا منجر به ناسازگاری‌های ظریف شود.
...
- `backend/app/api/deps.py:32-54` — بلوک try/except برای import Enumها
  ```python
  try:
      from app.core.enums import UserRole, ClassificationLevel
  except Exception: # pragma: no cover
      try:
          from app.models.enums import UserRole, ClassificationLevel
      except Exception: # pragma: no cover
          ...
...
Python, FastAPI dependencies, Enum

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `backend/app/core/enums.py` — منبع کانونی برای این Enumها
- `backend/app/models/user.py` — تعریف UserRole و ClearanceLevel که باید با app.core.enums همسو باشد
- `backend/app/main.py` — استراتژی fail-fast را اتخاذ می‌کند
- `backend/app/api/routes/persons.py` — استراتژی fail-fast را اتخاذ می‌کند
- `backend/app/api/routes/auth.py` — از deps.py استفاده می‌کند
...
## 🌐 نقشهٔ وابستگی‌ها
این تغییر یک ارائه‌دهنده وابستگی اصلی را ساده می‌کند. این به `app.core.enums` متکی است که به درستی پیکربندی و موجود باشد.
...
- [ ] `backend/app/api/deps.py` `UserRole` و `ClassificationLevel` را مستقیماً از `app.core.enums` import کند.
- [ ] برنامه در صورت وجود و تعریف صحیح `app.core.enums` با موفقیت راه‌اندازی شود.
- [ ] برنامه در زمان راه‌اندازی با `ImportError` شکست بخورد اگر `app.core.enums` یا Enumهای داخل آن موجود نباشند.
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. 1. کل بلوک `try/except` برای importهای `UserRole` و `ClassificationLevel` حذف شود. 2. `UserRole` و `ClassificationLevel` مستقیماً از `app.core.enums` import شوند. 3. اطمینان حاصل شود که `app.core.enums.py` منبع قطعی برای این Enumها است و همیشه در دسترس است. در صورت عدم وجود، برنامه باید در زمان راه‌اندازی با خطا متوقف شود، که با رفتار 'fail-fast' سایر importهای حیاتی سازگار است.

## 💡 نمونه‌های قبل/بعد
**قبل: import دفاعی با fallback**
...
اگر `app.core.enums` واقعاً گم شده یا اشتباه پیکربندی شده باشد، این تغییر باعث می‌شود برنامه در زمان راه‌اندازی با خطا مواجه شود، که رفتار 'fail-fast' مورد نظر است اما ممکن است نیاز به رفع فوری فایل Enum داشته باشد.
...
1. `backend/app/api/deps.py` `UserRole` و `ClassificationLevel` را مستقیماً از `app.core.enums` import کند. _(verify: static)_
2. برنامه در صورت وجود و تعریف صحیح `app.core.enums` با موفقیت راه‌اندازی شود. _(verify: static)_
3. برنامه در زمان راه‌اندازی با `ImportError` شکست بخورد اگر `app.core.enums` یا Enumهای داخل آن موجود نباشند. _(verify: static)_

---
task_id: 3284a6ca-b50b-4c8c-8413-6999b7a863ad
title: Inconsistent API Version Prefixing Between Frontend and Backend
type: bug
priority: high
...
created_at: '2026-06-13T18:08:44.740787+00:00'
updated_at: '2026-06-13T20:18:12.073062+00:00'
target_files:
- frontend/src/app/encyclopedia/page.tsx
- frontend/src/app/graph/page.tsx
---

# Inconsistent API Version Prefixing Between Frontend and Backend

## Raw Idea

The backend FastAPI application is configured to use `/api/v1` as its base prefix (from `settings.API_V1_PREFIX` in `backend/app/main.py`). However, the frontend makes inconsistent API calls: `frontend/src/app/encyclopedia/page.tsx` calls `/api/encyclopedia/articles` (missing `/v1`), and `frontend/src/app/graph/page.tsx` calls `/api/graph` (missing `/v1`). In contrast, `frontend/src/app/persons/page.tsx` correctly calls `/api/v1/persons`. This inconsistency leads to some frontend pages failing to connect to the correct backend endpoints, resulting in broken functionality or fallback to sample data.
...
- `frontend/src/app/encyclopedia/page.tsx:129-131` — `loadArticles` — API call missing '/v1' prefix.
  ```tsx
  const data = await apiFetch<Article[] | { items: Article[] }>(
          '/api/encyclopedia/articles'
        );
  ```
- `frontend/src/app/graph/page.tsx:256-256` — `loadGraph` — API call missing '/v1' prefix.
  ```tsx
...
FastAPI routing, Next.js client-side fetching.

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `backend/app/main.py` (سطر 236) — Defines the backend's API version prefix.
- `backend/app/api/routes/*.py` — Individual routers define their specific prefixes.
- `frontend/src/app/persons/page.tsx` (سطر 322) — Example of correct API version prefix usage.

## 🌐 نقشهٔ وابستگی‌ها
This is a critical integration issue between the frontend and backend, affecting the functionality of several core pages.
...
- [ ] All frontend API calls consistently use the `/api/v1` prefix.
- [ ] All frontend pages (encyclopedia, graph, persons) successfully fetch data from the backend.
- [ ] No more fallback to sample data due to API connection errors (unless backend is truly down).
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. Standardize the API base URL in the frontend to consistently include the `/api/v1` prefix. Update the paths in `encyclopedia/page.tsx` and `graph/page.tsx` to explicitly include `/v1` to match the backend's configured API version prefix.

## 💡 نمونه‌های قبل/بعد
**Before (frontend/src/app/encyclopedia/page.tsx)**
...
```
const data = await apiFetch<Article[] | { items: Article[] }>(
  '/api/v1/encyclopedia/articles' // Add /v1
);
```
...
## 🧪 دستورات اعتبارسنجی
- `Run the frontend and backend, then navigate to `/encyclopedia` and `/graph` pages. Verify data loads.`
- `Check browser network tab for successful API calls to `/api/v1/encyclopedia/articles` and `/api/v1/graph`.`

## ⚠️ ریسک‌ها و موارد احتیاط
Minor risk of breaking other API calls if not all paths are updated correctly.
...
1. All frontend API calls consistently use the `/api/v1` prefix. _(verify: static)_
2. All frontend pages (encyclopedia, graph, persons) successfully fetch data from the backend. _(verify: static)_
3. No more fallback to sample data due to API connection errors (unless backend is truly down). _(verify: static)_

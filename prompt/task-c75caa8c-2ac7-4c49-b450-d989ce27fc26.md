---
task_id: c75caa8c-2ac7-4c49-b450-d989ce27fc26
title: عدم اعمال فیلتر RBAC برای طبقه‌بندی محرمانگی در روت‌های Graph API
type: security
priority: high
...
created_at: '2026-06-13T18:10:52.081201+00:00'
updated_at: '2026-06-13T20:19:02.289799+00:00'
target_files:
- backend/app/api/routes/graph.py
- backend/app/api/routes/graph.py
---

# عدم اعمال فیلتر RBAC برای طبقه‌بندی محرمانگی در روت‌های Graph API

## Raw Idea

روت‌های `get_graph` و `get_person_neighbors` در `backend/app/api/routes/graph.py` از `Depends(get_current_user)` برای احراز هویت استفاده می‌کنند، اما هیچ فیلتر یا بررسی صریحی برای `ClassificationLevel` روی داده‌های `Person` یا `Relationship` اعمال نمی‌کنند. این بدان معناست که یک کاربر با سطح دسترسی پایین‌تر (مثلاً `UNCLASSIFIED` یا `RESTRICTED`) ممکن است بتواند اطلاعات مربوط به اشخاص یا روابطی با طبقه‌بندی‌های بالاتر (مثلاً `SECRET` یا `TOP_SECRET`) را از طریق نمودار مشاهده کند، حتی اگر نتواند به پروفایل کامل آن شخص دسترسی یابد. این یک نقص امنیتی است که می‌تواند منجر به افشای اطلاعات شود.
...
- `backend/app/api/routes/graph.py:264-278` — حلقه ساخت گره‌ها که فیلتر ClassificationLevel را ندارد.
  ```python
  ...
  ```
- `backend/app/api/routes/graph.py:317-346` — منطق `get_person_neighbors` که فیلتر ClassificationLevel را ندارد.
  ```python
...
FastAPI, RBAC, SQLAlchemy, Graph Data Filtering

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `backend/app/api/deps.py` (سطر 291) — ماژول مرکزی برای کنترل دسترسی بر اساس ClassificationLevel.
- `backend/app/models/user.py` (سطر 212) — مدل User دارای متد has_clearance است.
- `backend/app/models/person.py` (سطر N/A) — مدل Person دارای فیلد classification است.
- `backend/app/main.py` — این فایل `graph.py` را import می‌کند (caller)

## 🌐 نقشهٔ وابستگی‌ها
این تغییر بر روی داده‌های بازگردانده شده توسط API نمودار و امنیت اطلاعات تأثیر می‌گذارد.
...
- [ ] روت‌های `get_graph` و `get_person_neighbors` فقط اشخاص و روابطی را برگردانند که کاربر فعلی به سطح طبقه‌بندی آن‌ها دسترسی دارد.
- [ ] تست‌های امنیتی برای تأیید عدم افشای اطلاعات طبقه‌بندی‌شده در نمودار اضافه شود.
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. قبل از ساخت `GraphNode`ها و `GraphEdge`ها، باید بررسی `ClassificationLevel` انجام شود. فقط اشخاصی که کاربر فعلی به سطح طبقه‌بندی آن‌ها دسترسی دارد، باید در نمودار گنجانده شوند. این بررسی باید از مکانیزم مرکزی `current_user.has_clearance()` یا `app.api.deps.can_access_classification` استفاده کند.

## 💡 نمونه‌های قبل/بعد
**قبل: بدون فیلتر محرمانگی**
...
**بعد: با فیلتر محرمانگی**

_بعد:_
```
for person in persons:
    if not current_user.has_clearance(person.classification):
        continue # Skip person if user doesn't have clearance
    node = _person_to_node(person)
    # ... (add node to list)
```
...
## 🧪 دستورات اعتبارسنجی
- `ایجاد کاربران با سطوح دسترسی مختلف و بررسی خروجی API نمودار برای هر کاربر.`

## ⚠️ ریسک‌ها و موارد احتیاط
احتمال فیلتر کردن بیش از حد داده‌ها یا خطاهای منطقی در صورت عدم دقت.
...
1. روت‌های `get_graph` و `get_person_neighbors` فقط اشخاص و روابطی را برگردانند که کاربر فعلی به سطح طبقه‌بندی آن‌ها دسترسی دارد. _(verify: static)_
2. تست‌های امنیتی برای تأیید عدم افشای اطلاعات طبقه‌بندی‌شده در نمودار اضافه شود. _(verify: static)_

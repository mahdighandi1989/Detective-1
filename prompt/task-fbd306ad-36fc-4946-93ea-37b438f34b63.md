---
task_id: fbd306ad-36fc-4946-93ea-37b438f34b63
title: فایل سرویس LLM Adapter بیش از حد بزرگ
type: refactor
priority: high
...
created_at: '2026-06-13T18:08:44.782458+00:00'
updated_at: '2026-06-13T20:18:21.985309+00:00'
target_files:
- backend/app/services/llm_adapter.py
---

# فایل سرویس LLM Adapter بیش از حد بزرگ

## Raw Idea

فایل `backend/app/services/llm_adapter.py` با ۹۲۳ خط کد، بسیار بزرگ است و شامل اینترفیس `BaseLLMProvider`، منطق retry و پیاده‌سازی‌های مشخص برای OpenAI، Perplexity و Gemini است. این موضوع باعث می‌شود که پیمایش، درک و نگهداری فایل دشوار شود، به‌ویژه با اضافه شدن ارائه‌دهندگان LLM جدید در آینده. این فایل از دستورالعمل 'فایل‌های بالای ۵۰۰ خط' تخطی می‌کند.
...
- `backend/app/services/llm_adapter.py:1-923` — پیاده‌سازی‌های هر ارائه‌دهنده LLM
  ```python
...
Python, FastAPI, LLM SDKs (OpenAI, Google Generative AI, httpx)

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `backend/app/services/__init__.py` — از `get_llm_adapter` استفاده می‌کند
- `backend/app/api/routes/encyclopedia.py` — از `_get_llm_adapter` استفاده می‌کند
- `backend/app/services/osint_agent.py` — از `llm_adapter` استفاده می‌کند
- `backend/app/workers/tasks.py` — از `llm_adapter` برای وظایف پردازش استفاده می‌کند
- `backend/app/core/config.py` — `llm_adapter.py` این فایل را import می‌کند

## 🌐 نقشهٔ وابستگی‌ها
این بازسازی عمدتاً بر ساختار داخلی سرویس LLM تأثیر می‌گذارد. فراخواننده‌های خارجی فقط در صورت تغییر کارخانه `LLMAdapter` نیاز به به‌روزرسانی مسیرهای import خود خواهند داشت.
...
- [ ] `backend/app/services/llm_adapter.py` از نظر اندازه به طور قابل توجهی کاهش یابد.
- [ ] هر پیاده‌سازی ارائه‌دهنده LLM در فایل جداگانه خود قرار گیرد.
- [ ] تمام قابلیت‌ها (تکمیل چت، embeddings، جستجو) برای همه ارائه‌دهندگان پیکربندی‌شده به‌درستی کار کنند.
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. 1. هر پیاده‌سازی مشخص ارائه‌دهنده LLM (مانند `OpenAIProvider`, `PerplexityProvider`, `GeminiProvider`) به فایل جداگانه خود در یک دایرکتوری جدید `backend/app/services/llm_providers/` منتقل شود. 2. کلاس انتزاعی `BaseLLMProvider`، دکوراتور `retry_with_exponential_backoff` و کلاس‌های داده/Enumهای مشترک (`LLMProviderName`, `ChatMessage`, `LLMUsage`) در فایل اصلی `backend/app/services/llm_adapter.py` باقی بمانند. 3. تابع `LLMAdapter` (یا یک تابع کارخانه جدید) به‌روزرسانی شود تا ارائه‌دهنده صحیح را بر اساس پیکربندی به‌صورت پویا import و نمونه‌سازی کند.

## 💡 نمونه‌های قبل/بعد
**قبل: همه ارائه‌دهندگان در یک فایل**
...
**بعد: هر ارائه‌دهنده در فایل خود**
...
// backend/app/services/llm_adapter.py
# Only BaseLLMProvider and factory logic remain
```
...
- `uvicorn app.main:app --reload (و بررسی دستی قابلیت‌های LLM مانند تحلیل مقاله دانشنامه)`

## ⚠️ ریسک‌ها و موارد احتیاط
احتمال خطاهای import یا مشکلات زمان اجرا در صورت عدم به‌روزرسانی صحیح منطق نمونه‌سازی ارائه‌دهنده.
...
1. `backend/app/services/llm_adapter.py` از نظر اندازه به طور قابل توجهی کاهش یابد. _(verify: static)_
2. هر پیاده‌سازی ارائه‌دهنده LLM در فایل جداگانه خود قرار گیرد. _(verify: static)_
3. تمام قابلیت‌ها (تکمیل چت، embeddings، جستجو) برای همه ارائه‌دهندگان پیکربندی‌شده به‌درستی کار کنند. _(verify: static)_

---
task_id: edc9d8f1-52d4-489d-bf61-0ac6b4ee0672
title: صفحات فرانت‌اند 'جستجوی معنایی' و 'ارزیابی ریسک' وجود ندارند
...
created_at: '2026-06-13T17:56:24.284362+00:00'
updated_at: '2026-06-13T20:15:57.384052+00:00'
target_files:
- frontend/src/app/page.tsx
- frontend/src/app/page.tsx
---

# صفحات فرانت‌اند 'جستجوی معنایی' و 'ارزیابی ریسک' وجود ندارند

## Raw Idea

صفحهٔ اصلی (`frontend/src/app/page.tsx`) شامل لینک‌هایی به `/search` (خط 73) و `/risk-assessment` (خط 88) است. با این حال، فایل‌های `page.tsx` متناظر برای این مسیرها در لیست فایل‌های ارائه شده وجود ندارند. این در حالی است که بک‌اند دارای endpointهای API برای جستجوی معنایی (`/api/encyclopedia/search`) و ارزیابی ریسک (`/api/risk`, `/api/risk-assessments`) است که نشان می‌دهد این ویژگی‌ها در نظر گرفته شده‌اند.
...
- `frontend/src/app/page.tsx:73` — `Link` — لینک به صفحهٔ جستجوی معنایی
  ```tsx
  <Link href="/search">شروع جستجو</Link>
  ```
- `frontend/src/app/page.tsx:88` — `Link` — لینک به صفحهٔ ارزیابی ریسک
  ```tsx
...
Next.js 14 App Router, React, TypeScript, FastAPI

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `backend/app/api/routes/encyclopedia.py` (سطر 153) — endpoint جستجوی معنایی را فراهم می‌کند
- `backend/app/api/routes/risk.py` (سطر 144) — endpointهای ارزیابی ریسک را فراهم می‌کند
- `backend/app/api/routes/risk_assessment.py` (سطر 253) — endpointهای ارزیابی ریسک را فراهم می‌کند

## 🌐 نقشهٔ وابستگی‌ها
این صفحات جدید به endpointهای بک‌اند موجود متصل می‌شوند و نیاز به طراحی UI و منطق واکشی داده دارند.
...
- [ ] صفحات `/search` و `/risk-assessment` در فرانت‌اند قابل دسترسی هستند.
- [ ] صفحهٔ جستجوی معنایی به `/api/encyclopedia/search` متصل شده و نتایج را نمایش می‌دهد.
- [ ] صفحهٔ ارزیابی ریسک به `/api/risk` یا `/api/risk-assessments` متصل شده و امکان مشاهده/مدیریت ارزیابی‌ها را فراهم می‌کند.
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. صفحات فرانت‌اند گم‌شده را ایجاد کنید: `frontend/src/app/search/page.tsx` برای رابط کاربری جستجوی معنایی و `frontend/src/app/risk-assessment/page.tsx` برای رابط کاربری ارزیابی ریسک. این صفحات باید با endpointهای API موجود در بک‌اند ارتباط برقرار کنند.

## 📤 خروجی مورد انتظار
تغییر کد در فایل‌های مرتبط، commit یا PR جدید با پیام واضح، و عبور تمام معیارهای پذیرش.

## 🧪 دستورات اعتبارسنجی
- `به آدرس‌های `/search` و `/risk-assessment` در مرورگر بروید و از عملکرد آن‌ها اطمینان حاصل کنید.`

## ⚠️ ریسک‌ها و موارد احتیاط
نیاز به پیاده‌سازی کامل UI و منطق ارتباط با API.
...
1. صفحات `/search` و `/risk-assessment` در فرانت‌اند قابل دسترسی هستند. _(verify: static)_
2. صفحهٔ جستجوی معنایی به `/api/encyclopedia/search` متصل شده و نتایج را نمایش می‌دهد. _(verify: static)_
3. صفحهٔ ارزیابی ریسک به `/api/risk` یا `/api/risk-assessments` متصل شده و امکان مشاهده/مدیریت ارزیابی‌ها را فراهم می‌کند. _(verify: static)_

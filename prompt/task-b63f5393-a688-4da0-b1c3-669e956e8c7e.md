---
task_id: b63f5393-a688-4da0-b1c3-669e956e8c7e
title: 'endpoint بک‌اند بلااستفاده: GET /{source_id}'
type: audit
priority: medium
...
created_at: '2026-06-13T18:15:55.202392+00:00'
updated_at: '2026-06-13T20:20:59.309467+00:00'
target_files:
- backend/app/api/routes/sources.py
---

# endpoint بک‌اند بلااستفاده: GET /{source_id}

## Raw Idea

## 📋 شرح
endpoint `GET /{source_id}` در `backend/app/api/routes/sources.py` تعریف شده ولی هیچ `fetch`، `axios`، `apiClient.*`، یا frontend call به آن دیده نمی‌شود.
...
- [ ] مشخص شد endpoint `GET /{source_id}` در کدام دسته است (orphan/internal/deprecated)
...
1. گام ۱: grep روی `/{source_id}` در frontend + scripts/ + docs/ — اگر caller هست، اتصال را drop شده اصلاح کن.
...
## 🧪 دستورات اعتبارسنجی
- `python -m py_compile backend/app/api/routes/sources.py`
- `ruff check backend/app/api/routes/sources.py`
- `pytest -x`
...
1. مشخص شد endpoint `GET /{source_id}` در کدام دسته است (orphan/internal/deprecated) _(verify: static)_

...
updated_at: '2026-06-13T20:17:10.549056+00:00'
target_files:
...
1. `get_current_user` کاربران را از JWTهای معتبر به درستی شناسایی کند. _(verify: static)_
2. `get_current_user` برای توکن‌های نامعتبر/منقضی یا کاربران ناموجود، `CredentialsException` را برگرداند. _(verify: static)_
3. `require_roles` دسترسی را بر اساس نقش کاربر به درستی مجاز/غیرمجاز کند. _(verify: static)_
4. `require_min_role` سلسله مراتب نقش‌ها را به درستی اعمال کند. _(verify: static)_
5. `can_access_classification` دسترسی را بر اساس سطح دسترسی کاربر و طبقه‌بندی محتوا به درستی تعیین کند. _(verify: static)_
6. `get_db` نمونه‌های `AsyncSession` را فراهم کرده و به درستی ببندد. _(verify: static)_
